...
func TestDesiredWant_BrokerOK_ClaimedPlusHeadroom(t *testing.T) {
	...
	if got := l.desiredWant(context.Background()); got != 7 {
		t.Fatalf("want 7 (4 claimed + 3 headroom), got %d", got)
	}
...
func TestDesiredWant_NoClaimed_ScalesDownToHeadroom(t *testing.T) {
	fake := &fakeBrokerClient{counts: api.FleetTaskCountsResponse{Claimed: 0}}
	l := &Launcher{
