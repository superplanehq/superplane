...
INSERT INTO tasks (id, command_json, commands_json, webhook_url, status, created_at, execution_mode, docker_image, execution_timeout_seconds)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
SELECT id, command_json, commands_json, webhook_url, status, created_at, claimed_at, lease_until, runner_id,
	execution_mode, docker_image, execution_timeout_seconds, exit_code, output, error_message
...
func scanTask(row *sql.Row) (*models.Task, error) {
	...
	if err := row.Scan(&id, &cmdJSON, &commandsJSON, &webhook, &status, &createdAt, &claimedAt, &leaseUntil,
		&runnerID, &execMode, &dockerImage, &execTimeoutSec, &exitCode, &output, &errMsg); err != nil {
		return nil, err
	}
	var cmd []string
