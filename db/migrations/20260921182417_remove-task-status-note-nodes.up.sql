  ...
  SELECT
    ...
    COALESCE((
      ...
      FROM (
        ...
        SELECT
          jsonb_build_object(
            'source_id', incoming.edge -> 'source_id',
            'target_id', outgoing.edge -> 'target_id',
            'channel', incoming.edge -> 'channel'
          ),
          MIN(incoming.position)::numeric + 0.5
        ...
        JOIN jsonb_array_elements(version.edges) AS outgoing(edge)
          ON outgoing.edge ->> 'source_id' = incoming.edge ->> 'target_id'
        ...
        WHERE incoming.edge ->> 'target_id' = ANY (version.removed_node_ids)
          ...
          AND NOT (outgoing.edge ->> 'target_id' = ANY (version.removed_node_ids))
          AND NOT EXISTS (
            SELECT 1
            FROM jsonb_array_elements(version.edges) AS existing(edge)
            WHERE existing.edge ->> 'source_id' = incoming.edge ->> 'source_id'
              AND existing.edge ->> 'target_id' = outgoing.edge ->> 'target_id'
              AND existing.edge ->> 'channel' = incoming.edge ->> 'channel'
          )
        GROUP BY
