...
agent.evals.bootstrap:
	$(COMPOSE) run --rm -T --no-deps --user $$(id -u):$$(id -g) --env-from-file $(CURDIR)/agent/.env agent bash -lc "mkdir -p /tmp/uv-cache && export UV_CACHE_DIR=/tmp/uv-cache && cd /app/agent && uv sync --group dev && uv run --group dev python scripts/bootstrap_eval_env.py --merge-env /app/agent/.env $(ARGS)"
