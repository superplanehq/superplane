...
func (a accessAction) Execute(ctx context.Context, session agents.AgentSessionContext, _ Input) (any, error) {
	...
	return accessResult{
		...
		Notes: []string{
			...
			"Staging graph, Console, and repository file edits are allowed through canvases:update on this canvas; deleting the app and live app operations are not included in the agent token.",
		},
	}, nil
}
