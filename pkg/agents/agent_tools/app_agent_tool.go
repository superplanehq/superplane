...
func (t *AppAgentTool) Description() string {
	return "Inspect access, read, create drafts, update the current SuperPlane app canvas, and manage app repository files. This is the only way to reach the app; there is no command line or HTTP API to call. Use access to check the current session's interceptor-backed permissions, read for canvas YAML, read_runtime for memory/runs/events/executions/queues, list_files/read_file for app repository files and AGENTS.md context, create_draft when read returns live/no version_id or when intentionally creating another draft branch, write_file/delete_file to stage normal file changes, commit_files to commit staged draft file changes, list_integrations for connected integration IDs, list_resources for integration-backed resource values, update_draft to save full draft graph or Console YAML, and patch_draft to apply small graph edits without sending full canvas_yaml. The tool is bound to the current agent session's canvas and will reject attempts to access any other canvas. It never publishes drafts; update_draft, patch_draft, and file write actions require version_id and update that selected draft branch."
}
	...
	return agents.CustomToolInputSchema{
		...
		Properties: map[string]agents.CustomToolInputSchema{
			"action": {
				...
				Description: "Operation to run. Use access to inspect token-backed API capabilities, read for current YAML, read_runtime for memory/runs/events/executions/queues, list_files/read_file for app repository files and AGENTS.md context, create_draft when read returns live/no version_id or when intentionally creating another draft branch, write_file/delete_file to stage normal file changes, commit_files to commit staged draft file changes, update_draft to save full canvas_yaml and/or console_yaml, patch_draft to apply small graph edits without sending full canvas_yaml, list_integrations for connected integration IDs, and list_resources for integration-backed resource values.",
			},
			"canvas_id": {
				Type:        "string",
