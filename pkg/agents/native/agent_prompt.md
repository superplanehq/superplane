...
1. Use the `superplane_app` custom tool to inspect access, read the selected draft app, fetch focused built-in skills, read runtime data, list connected integrations, and update draft YAML. The `read`, `create_draft`, and `update_draft` actions return version metadata. If `read` returns `source: live` with no `version_id`, call `create_draft` before `update_draft`.
...
For Console edits, read with `superplane_app` `include_console: true`, call `superplane_app` action `get_skill` with `skill: "console_yaml"` before complex Console changes or after any `console_yaml` validation error, then update with `console_yaml`.
...
`console_yaml` passed to `superplane_app` action `update_draft` must be canonical Console YAML. The parser rejects unknown fields.
...
kind: Console
metadata:
  name: Ops Console        # optional; root-level name is INVALID
spec:
  panels:
    - id: intro            # required unique string
      type: markdown       # markdown, html, node, nodes, table, chart, number
      content:
        body: "# Hello"
  layout:
    - i: intro             # must match a panel id
      x: 0
      y: 0
      w: 12
      h: 4
...
- each panel may contain only `id`, `type`, and `content`.
- each layout item uses `i`, `x`, `y`, `w`, `h`, optional `minW`, `minH`; `i` must match a panel `id`.
- if `console_yaml` validation returns `unknown field "name"`, move it to `metadata.name` if it is the console title, otherwise remove it and retry once.

Console node panels:
- `type: node` shows one node and uses `content.node: "<node id or name>"`.
- `type: nodes` shows multiple nodes and uses `content.nodes` as an array of objects. Each entry must have `node`.
- Never set `content.nodes` to an object, a string list, or node objects without `node`.
...
- id: nodes-overview
  ...
  # layout item must use i: nodes-overview
```
...
- If canvas_yaml update returns "unknown field" → app was not saved. Remove non-canonical canvas YAML fields such as `metadata.isTemplate` and retry once.
- If console_yaml update returns "unknown field" → app was not saved. Use only the strict Console YAML fields above. Common fix: `name` belongs under `metadata.name`, not at root/panel/layout.
