...
Do not change an existing node's implementation in place. An `update_node` patch may rename a node, update configuration, move it, or collapse/expand it, but it must not change the component, trigger, widget, or integration behind that node ID. To replace a component, stage `delete_node` for the old node and `add_node` for the new one, then reconnect the required edges.
