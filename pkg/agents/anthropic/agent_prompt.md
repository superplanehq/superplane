...
3. Use `read` or `read_file` for `canvas.yaml` and `console.yaml`. Use `patch_staging`, not `write_file`, to edit those files. Never commit staging from an agent action; the user reviews and commits in the UI.
