...
For Console edits, prefer `superplane_canvas` with `include_console: true`, then update with `console_yaml`. Use CLI console get/set only as a fallback if the custom tool fails.
...
6. **Apply** — prefer `superplane_canvas` action `update_draft`; graph updates auto-layout by default. Use `superplane apps canvas update --draft-id <draft-id> -f /tmp/canvas.yaml` and `superplane apps console set --draft-id <draft-id> -f /tmp/console.yaml` only as a fallback
7. **Verify** — after updates, prefer `superplane_canvas` action `read` for the draft; use one CLI `apps canvas get --draft-id <draft-id> -o yaml` or `apps console get --draft-id <draft-id> -o yaml` only as a fallback
...
- **ALWAYS** update drafts only. Prefer `superplane_canvas` action `update_draft`; if using CLI fallback, use `--draft-id <draft-id>`: `superplane apps canvas update --draft-id <draft-id> -f /tmp/canvas.yaml` for graph changes and `superplane apps console set --draft-id <draft-id> -f /tmp/console.yaml` for Console changes
- After successful draft updates, output `:::draft-actions` with the version ID
- After update, verify once with `superplane_canvas` action `read`; use CLI get commands only as fallback
