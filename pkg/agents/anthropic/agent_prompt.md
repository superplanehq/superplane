...
1. Use the `superplane_app` custom tool to inspect access, read the selected draft app, read runtime data, list connected integrations, and update drafts. The `read`, `create_draft`, `patch_draft`, and `update_draft` actions return version metadata. If `read` returns `source: live` with no `version_id`, call `create_draft` before `patch_draft` or `update_draft`.
