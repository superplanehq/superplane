...
6. Apply graph and Console edits with `patch_staging`, then verify once with `read`. `patch_staging` auto-layouts affected connected components by default. Pass `auto_layout` only when you need `full_canvas`, custom `connected_component` `node_ids`, disabled layout, or a layout-only update.
