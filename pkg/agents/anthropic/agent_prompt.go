...
func syncAgentPrompt(ctx context.Context, client *Client, agentID, prompt string) error {
	...
	if promptsEqual(current.System, prompt) && toolsEqualOrUnknown(current.Tools, expectedTools) {
		return nil
	}
...
func toolsEqual(current json.RawMessage, expected []map[string]any) bool {
	...
	return reflect.DeepEqual(currentValue, expectedValue)
}

func toolsEqualOrUnknown(current json.RawMessage, expected []map[string]any) bool {
	if len(current) == 0 {
		return true
	}

	return toolsEqual(current, expected)
}
