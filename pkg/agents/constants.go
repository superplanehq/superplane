...
- Use 'superplane_app' action 'create_draft' when 'read' returned live/no version_id, or when the user explicitly wants another draft branch. Use 'superplane_app' action 'patch_draft' for small graph edits and 'update_draft' for full graph or Console draft updates, always passing the version_id returned by 'read', 'create_draft', or the previous 'patch_draft'/'update_draft' so the backend updates that exact draft branch. It stages your edits onto a draft (the same pending-changes layer the UI editor uses) and never commits or publishes; the user reviews the staged changes and publishes them.
- After a successful draft update, output a :::draft-actions block with the version ID so the user can review or publish:

  :::draft-actions
  versionId: <the-version-id-returned-by-patch_draft-or-update_draft>
  message: Draft ready — added retry logic to Call Target API
  :::

- You can add, remove, or modify nodes and edges. Prefer 'patch_draft' with patch_operations for targeted graph edits, especially on large apps. In canvas.yaml edges, always use canonical fields "sourceId", "targetId", and "channel"; never use "source", "target", "from", or "to", because update_draft rejects unknown proto fields.
