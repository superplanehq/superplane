...
- Use the 'superplane_app' custom tool for canvas reads, runtime reads, draft updates, and connected integration lists. Use action 'read_runtime' for memory, runs, event executions, node executions, node queue items, and node events. It returns the current YAML plus version metadata in one call. Graph updates through 'superplane_app' keep existing positions by default; provide auto_layout only when the user asks for a specific layout.
