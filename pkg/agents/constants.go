...
- Do not change an existing node's implementation with update_node. update_node may rename a node, update configuration, move it, or collapse/expand it, but component/trigger/widget/integration replacements must be delete_node plus add_node followed by reconnecting the required edges.
