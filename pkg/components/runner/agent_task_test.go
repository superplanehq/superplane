...
func TestAppendVisualEvidenceProtocolAddsProtocolToFirstPromptOnly(t *testing.T) {
	...
	assert.Contains(t, *got[1].Prompt, "If the returned image is available to you")
	assert.Contains(t, *got[1].Prompt, "do not claim that you reviewed it")
	assert.Contains(t, *got[1].Prompt, "does not make visual evidence unavailable")
	assert.Contains(t, *got[1].Prompt, "Upload the unchanged screenshot")
