...
func TestPollBrokerTaskIgnoresFinishedExecution(t *testing.T) {
	...
	assert.Empty(t, state.Channel)
	assert.Empty(t, state.Payloads)
	assert.Empty(t, requests.Action, "a finished execution must not reschedule the poll")
	assert.Empty(t, httpCtx.Requests, "a finished execution must not reach the broker")
}
