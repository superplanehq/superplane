...
var errRepositoryCommandFileRemoved = errors.New("repository command files are no longer supported")
...
// before the source field existed keep working. Any other value is returned
// verbatim so leftover "file" configs and padded values fail loudly instead
...
func (s Spec) usesRemovedRepositoryCommandFile() bool {
	return s.commandSourceOrDefault() == CommandSourceFile
}
...
- **Commands**: One or more commands to run, one per line (supports expressions). Each non-empty line becomes a command joined with &&. The output payload is based on the last command.
...
func (c *SSHCommand) Setup(ctx core.SetupContext) error {
	...
	if err := validateCommands(spec); err != nil {
		return err
	}
...
func (c *SSHCommand) Execute(ctx core.ExecutionContext) error {
	...
	// The (already template-resolved) commands are stored in metadata so
	// retries don't have to re-evaluate expressions.
...
func (c *SSHCommand) HandleHook(ctx core.ActionHookContext) error {
	...
	case "connectionRetry", "executionRetry":
		...
		if metadata.CommandSource == CommandSourceFile {
			return errRepositoryCommandFileRemoved
		}
...
func (c *SSHCommand) executeSSH(ctx ExecuteSSHContext) error {
	...
	command, stdin, err := c.buildExecutionCommand(ctx.execMetadata)
	if err != nil {
		return err
	}
...
//
// One-line commands stay on the command line. Multi-line scripts are streamed
// to `bash -s` so shell syntax that depends on real newlines (shebangs,
// comments, conditionals, here-docs) is not flattened into invalid `&&` chains.
func (c *SSHCommand) buildExecutionCommand(metadata ExecutionMetadata) (string, io.Reader, error) {
	if metadata.CommandSource == CommandSourceFile {
		return "", nil, errRepositoryCommandFileRemoved
	}
...
// validateCommands requires inline commands and rejects leftover repository
// file configs that the schema no longer accepts.
func validateCommands(spec Spec) error {
	if spec.usesRemovedRepositoryCommandFile() {
		return errRepositoryCommandFileRemoved
	}
	if spec.commandSourceOrDefault() != CommandSourceInline {
