...
func setupAuthHandler(t *testing.T, blockSignup bool) (*Handler, *support.ResourceRegistry) {
	...
	handler := NewHandler(signer, r.Encryptor, r.AuthService, "test", "/templates", blockSignup)
	return handler, r
}
...
func TestHandler_findOrCreateAccountForProvider(t *testing.T) {
	...
	t.Run("should handle email update failure gracefully", func(t *testing.T) {
		handler, _ := setupAuthHandler(t, false)
		...
		assert.Equal(t, account.ID, resultAccount.ID)
		assert.Equal(t, originalEmail, resultAccount.Email)
	})
}

func TestHandler_findOrCreateAccount_Legacy(t *testing.T) {
	t.Run("should find existing account by email", func(t *testing.T) {
		handler, _ := setupAuthHandler(t, false)

		email := "existing@example.com"
		account, err := models.CreateAccount("Existing User", email)
		require.NoError(t, err)

		resultAccount, err := handler.findOrCreateAccount("Existing User", email)
		require.NoError(t, err)
	...
	t.Run("should create new account when signup allowed", func(t *testing.T) {
		...
		resultAccount, err := handler.findOrCreateAccount(name, email)
		require.NoError(t, err)

		assert.NotNil(t, resultAccount)
		assert.Equal(t, email, resultAccount.Email)
		assert.Equal(t, name, resultAccount.Name)
	})

	t.Run("should return error when signup blocked", func(t *testing.T) {
		handler, _ := setupAuthHandler(t, true)
