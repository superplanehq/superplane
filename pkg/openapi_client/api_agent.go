...
type ApiAgentsDescribeAgentUsageRequest struct {
	ctx        context.Context
	ApiService *AgentAPIService
}

func (r ApiAgentsDescribeAgentUsageRequest) Execute() (*AgentsDescribeAgentUsageResponse, *http.Response, error) {
	return r.ApiService.AgentsDescribeAgentUsageExecute(r)
}

/*
AgentsDescribeAgentUsage Describe agent token usage for the organization

Returns aggregated token usage across all agent chats in the organization

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiAgentsDescribeAgentUsageRequest
*/
func (a *AgentAPIService) AgentsDescribeAgentUsage(ctx context.Context) ApiAgentsDescribeAgentUsageRequest {
	return ApiAgentsDescribeAgentUsageRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
// Execute executes the request
//
//	@return AgentsDescribeAgentUsageResponse
func (a *AgentAPIService) AgentsDescribeAgentUsageExecute(r ApiAgentsDescribeAgentUsageRequest) (*AgentsDescribeAgentUsageResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *AgentsDescribeAgentUsageResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "AgentAPIService.AgentsDescribeAgentUsage")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/agents/usage"
