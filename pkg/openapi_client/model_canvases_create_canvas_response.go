/*
Superplane Organizations API

API for managing organizations in the Superplane service

API version: 1.0
Contact: support@superplane.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_client

import (
	"encoding/json"
)

// checks if the CanvasesCreateCanvasResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CanvasesCreateCanvasResponse{}

// CanvasesCreateCanvasResponse struct for CanvasesCreateCanvasResponse
type CanvasesCreateCanvasResponse struct {
	Canvas *CanvasesCanvas `json:"canvas,omitempty"`
}

// NewCanvasesCreateCanvasResponse instantiates a new CanvasesCreateCanvasResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCanvasesCreateCanvasResponse() *CanvasesCreateCanvasResponse {
	this := CanvasesCreateCanvasResponse{}
	return &this
}

// NewCanvasesCreateCanvasResponseWithDefaults instantiates a new CanvasesCreateCanvasResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCanvasesCreateCanvasResponseWithDefaults() *CanvasesCreateCanvasResponse {
	this := CanvasesCreateCanvasResponse{}
	return &this
}

// GetCanvas returns the Canvas field value if set, zero value otherwise.
func (o *CanvasesCreateCanvasResponse) GetCanvas() CanvasesCanvas {
	if o == nil || IsNil(o.Canvas) {
		var ret CanvasesCanvas
		return ret
	}
	return *o.Canvas
}

// GetCanvasOk returns a tuple with the Canvas field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CanvasesCreateCanvasResponse) GetCanvasOk() (*CanvasesCanvas, bool) {
	if o == nil || IsNil(o.Canvas) {
		return nil, false
	}
	return o.Canvas, true
}

// HasCanvas returns a boolean if a field has been set.
func (o *CanvasesCreateCanvasResponse) HasCanvas() bool {
	if o != nil && !IsNil(o.Canvas) {
		return true
	}

	return false
}

// SetCanvas gets a reference to the given CanvasesCanvas and assigns it to the Canvas field.
func (o *CanvasesCreateCanvasResponse) SetCanvas(v CanvasesCanvas) {
	o.Canvas = &v
}

func (o CanvasesCreateCanvasResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CanvasesCreateCanvasResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Canvas) {
		toSerialize["canvas"] = o.Canvas
	}
	return toSerialize, nil
}

type NullableCanvasesCreateCanvasResponse struct {
	value *CanvasesCreateCanvasResponse
	isSet bool
}

func (v NullableCanvasesCreateCanvasResponse) Get() *CanvasesCreateCanvasResponse {
	return v.value
}

func (v *NullableCanvasesCreateCanvasResponse) Set(val *CanvasesCreateCanvasResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableCanvasesCreateCanvasResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableCanvasesCreateCanvasResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCanvasesCreateCanvasResponse(val *CanvasesCreateCanvasResponse) *NullableCanvasesCreateCanvasResponse {
	return &NullableCanvasesCreateCanvasResponse{value: val, isSet: true}
}

func (v NullableCanvasesCreateCanvasResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCanvasesCreateCanvasResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


