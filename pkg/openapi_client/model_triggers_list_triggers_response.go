/*
Superplane Organizations API

API for managing organizations in the Superplane service

API version: 1.0
Contact: support@superplane.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi_client

import (
	"encoding/json"
)

// checks if the TriggersListTriggersResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TriggersListTriggersResponse{}

// TriggersListTriggersResponse struct for TriggersListTriggersResponse
type TriggersListTriggersResponse struct {
	Triggers []SuperplaneTriggersTrigger `json:"triggers,omitempty"`
}

// NewTriggersListTriggersResponse instantiates a new TriggersListTriggersResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTriggersListTriggersResponse() *TriggersListTriggersResponse {
	this := TriggersListTriggersResponse{}
	return &this
}

// NewTriggersListTriggersResponseWithDefaults instantiates a new TriggersListTriggersResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTriggersListTriggersResponseWithDefaults() *TriggersListTriggersResponse {
	this := TriggersListTriggersResponse{}
	return &this
}

// GetTriggers returns the Triggers field value if set, zero value otherwise.
func (o *TriggersListTriggersResponse) GetTriggers() []SuperplaneTriggersTrigger {
	if o == nil || IsNil(o.Triggers) {
		var ret []SuperplaneTriggersTrigger
		return ret
	}
	return o.Triggers
}

// GetTriggersOk returns a tuple with the Triggers field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TriggersListTriggersResponse) GetTriggersOk() ([]SuperplaneTriggersTrigger, bool) {
	if o == nil || IsNil(o.Triggers) {
		return nil, false
	}
	return o.Triggers, true
}

// HasTriggers returns a boolean if a field has been set.
func (o *TriggersListTriggersResponse) HasTriggers() bool {
	if o != nil && !IsNil(o.Triggers) {
		return true
	}

	return false
}

// SetTriggers gets a reference to the given []SuperplaneTriggersTrigger and assigns it to the Triggers field.
func (o *TriggersListTriggersResponse) SetTriggers(v []SuperplaneTriggersTrigger) {
	o.Triggers = v
}

func (o TriggersListTriggersResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TriggersListTriggersResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Triggers) {
		toSerialize["triggers"] = o.Triggers
	}
	return toSerialize, nil
}

type NullableTriggersListTriggersResponse struct {
	value *TriggersListTriggersResponse
	isSet bool
}

func (v NullableTriggersListTriggersResponse) Get() *TriggersListTriggersResponse {
	return v.value
}

func (v *NullableTriggersListTriggersResponse) Set(val *TriggersListTriggersResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableTriggersListTriggersResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableTriggersListTriggersResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTriggersListTriggersResponse(val *TriggersListTriggersResponse) *NullableTriggersListTriggersResponse {
	return &NullableTriggersListTriggersResponse{value: val, isSet: true}
}

func (v NullableTriggersListTriggersResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTriggersListTriggersResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
