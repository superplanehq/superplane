...
func Test__BuildBacklogCanvas(t *testing.T) {
	...
	t.Run("the analysis runner authenticates with the workspace agent", func(t *testing.T) {
		...
		assert.Contains(t, prompt["prompt"], "Copy this markdown shape. Omit Diagram when a picture is not needed.")
		assert.Contains(t, prompt["prompt"], "### Goal")
		assert.Contains(t, prompt["prompt"], "### Done when")
		assert.Contains(t, prompt["prompt"], "### Out of scope")
		assert.Contains(t, prompt["prompt"], "### Key architecture decisions")
		assert.Contains(t, prompt["prompt"], "### Diagram")
		assert.NotContains(t, prompt["prompt"], "### Need")
		assert.NotContains(t, prompt["prompt"], "### Stay the same")
