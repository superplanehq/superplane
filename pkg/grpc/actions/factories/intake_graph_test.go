...
func Test__BuildBacklogCanvas(t *testing.T) {
	...
	t.Run("the analysis runner authenticates with the workspace agent", func(t *testing.T) {
		...
		assert.Contains(t, prompt["prompt"], "/tmp/spec.md")
		assert.Contains(t, prompt["prompt"], "propose_spec")
		assert.Contains(t, prompt["prompt"], "propose_confidence")
		...
		assert.Contains(t, prompt["prompt"], "If confidence is 2 through 5")
