...
func Test_ListGroups(t *testing.T) {
	...
	err := r.AuthService.CreateGroup(orgID, models.DomainTypeOrganization, "test-group-1", models.RoleOrgAdmin, "Test Group 1", "A test group")
	require.NoError(t, err)
	err = r.AuthService.CreateGroup(orgID, models.DomainTypeOrganization, "test-group-2", models.RoleOrgViewer, "Test Group 2", "Another test group")
	require.NoError(t, err)
	...
	t.Run("successful canvas groups list", func(t *testing.T) {
		err = r.AuthService.CreateGroup(r.Canvas.ID.String(), models.DomainTypeCanvas, "canvas-group-1", models.RoleCanvasAdmin, "Canvas Group 1", "A canvas group")
		require.NoError(t, err)
		err = r.AuthService.CreateGroup(r.Canvas.ID.String(), models.DomainTypeCanvas, "canvas-group-2", models.RoleCanvasViewer, "Canvas Group 2", "Another canvas group")
		require.NoError(t, err)
	...
	t.Run("groups with metadata have timestamps", func(t *testing.T) {
		err = r.AuthService.AddUserToGroup(orgID, "org", "test-user-1", "test-group-1")
		require.NoError(t, err)
