...
func Test_ListGroupUsers(t *testing.T) {
	...
	t.Run("successful get group users", func(t *testing.T) {
		resp, err := ListGroupUsers(ctx, orgID, models.DomainTypeOrganization, orgID, "test-group", r.AuthService)
		require.NoError(t, err)
	...
	t.Run("invalid request - missing group name", func(t *testing.T) {
		_, err := ListGroupUsers(ctx, orgID, models.DomainTypeOrganization, orgID, "", r.AuthService)
		assert.Error(t, err)
	...
	t.Run("successful canvas group get users", func(t *testing.T) {
		...
		resp, err := ListGroupUsers(ctx, orgID, models.DomainTypeCanvas, r.Canvas.ID.String(), "canvas-group", r.AuthService)
		require.NoError(t, err)
		assert.NotNil(t, resp)
		assert.Len(t, resp.Users, 1)
		assert.Equal(t, r.User.String(), resp.Users[0].Metadata.Id)
	...
	t.Run("empty group - no users", func(t *testing.T) {
		err = r.AuthService.CreateGroup(orgID, models.DomainTypeOrganization, "empty-group", models.RoleOrgViewer, "Empty Group", "Empty group description")
		require.NoError(t, err)

		resp, err := ListGroupUsers(ctx, orgID, models.DomainTypeOrganization, orgID, "empty-group", r.AuthService)
		require.NoError(t, err)
		assert.NotNil(t, resp)
