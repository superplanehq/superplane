...
func Test_RemoveRole(t *testing.T) {
	...
	t.Run("successful role removal with user ID", func(t *testing.T) {
		resp, err := RemoveRole(ctx, orgID, models.DomainTypeOrganization, orgID, models.RoleOrgAdmin, r.User.String(), "", r.AuthService)
		require.NoError(t, err)
		assert.NotNil(t, resp)
	})

	t.Run("user not found by email", func(t *testing.T) {
		_, err := RemoveRole(ctx, orgID, models.DomainTypeOrganization, orgID, models.RoleOrgAdmin, "", "nonexistent@example.com", r.AuthService)
		assert.Error(t, err)
