...
func Test__ListStageEvents(t *testing.T) {
	...
	t.Run("wrong canvas -> error", func(t *testing.T) {
		_, err := ListStageEvents(context.Background(), uuid.NewString(), r.Stage.ID.String(), states, []protos.StageEvent_StateReason{}, []protos.Execution_State{}, []protos.Execution_Result{}, 0, nil)
	...
	t.Run("stage does not exist -> error", func(t *testing.T) {
		_, err := ListStageEvents(context.Background(), r.Canvas.ID.String(), uuid.NewString(), states, []protos.StageEvent_StateReason{}, []protos.Execution_State{}, []protos.Execution_Result{}, 0, nil)
		s, ok := status.FromError(err)
		assert.True(t, ok)
		assert.Equal(t, codes.InvalidArgument, s.Code())
		assert.Equal(t, "stage not found", s.Message())
	...
	t.Run("stage with no stage events -> empty list", func(t *testing.T) {
		res, err := ListStageEvents(context.Background(), r.Canvas.ID.String(), r.Stage.ID.String(), states, []protos.StageEvent_StateReason{}, []protos.Execution_State{}, []protos.Execution_Result{}, 0, nil)
	...
	t.Run("pagination fields", func(t *testing.T) {
		...
		res, err := ListStageEvents(context.Background(), r.Canvas.ID.String(), r.Stage.ID.String(), states, []protos.StageEvent_StateReason{}, []protos.Execution_State{}, []protos.Execution_Result{}, 2, nil)
		require.NoError(t, err)
		require.NotNil(t, res)
		...
		res2, err := ListStageEvents(context.Background(), r.Canvas.ID.String(), r.Stage.ID.String(), states, []protos.StageEvent_StateReason{}, []protos.Execution_State{}, []protos.Execution_Result{}, 2, res.NextTimestamp)
		require.NoError(t, err)
