...
func ListAppDocs(ctx context.Context, organizationID, appID string) (*pb.ListAppDocsResponse, error) {
	...
	docs, err := models.FindAppDocsByAppID(appUUID)
	if err != nil {
		return nil, status.Error(codes.Internal, "failed to list app docs")
	}

	return &pb.ListAppDocsResponse{
		Docs: serializeAppDocs(docs),
	}, nil
}
