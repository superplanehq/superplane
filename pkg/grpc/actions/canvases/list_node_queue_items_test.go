...
func Test__SerializeNodeQueueItemsWithInputEvents__SkipsItemsWithMissingInput(t *testing.T) {
	now := time.Now()
	...
	assert.Equal(t, validQueueItemID.String(), result[0].Id)
	assert.Equal(t, "queued", result[0].Input.AsMap()["message"])
}
