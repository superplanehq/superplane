...
func canvasSpecFromVersionYAML(t *testing.T, ctx context.Context, orgID, canvasID, versionID string) *pb.Canvas_Spec {
	t.Helper()
...
func Test__UpdateCanvasVersion(t *testing.T) {
	...
	t.Run("invalid node field type -> serialized node carries error_message", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NotNil(t, version)
		spec := canvasSpecFromVersionYAML(t, ctx, r.Organization.ID.String(), canvas.ID.String(), version.ID.String())
