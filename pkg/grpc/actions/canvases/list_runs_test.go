...
func Test__ListRuns__ReturnsRunsWithRootEventsAndExecutionRefs(t *testing.T) {
	...
	execution := createRunExecution(t, run, "node-1", models.CanvasNodeExecutionResultPassed)
...
func createRunExecution(t *testing.T, run *models.CanvasRun, nodeID string, result string) *models.CanvasNodeExecution {
	now := time.Now()
