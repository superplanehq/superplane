...
func ListRuns(ctx context.Context, registry *registry.Registry, canvasID uuid.UUID, limit uint32, before *timestamppb.Timestamp) (*pb.ListRunsResponse, error) {
	...
	serialized, err := SerializeCanvasRuns(runs, executionsByRunID)
	if err != nil {
		return nil, err
	}
...
func SerializeCanvasRuns(runs []models.CanvasRun, executionsByRunID map[string][]models.CanvasNodeExecution) ([]*pb.CanvasRun, error) {
	result := make([]*pb.CanvasRun, 0, len(runs))

	for _, run := range runs {
		serializedRun, err := SerializeCanvasRun(run, executionsByRunID[run.ID.String()])
		if err != nil {
			return nil, err
		}
...
func SerializeCanvasRun(run models.CanvasRun, executions []models.CanvasNodeExecution) (*pb.CanvasRun, error) {
	if run.RootEvent == nil {
		rootEvent, err := models.FindCanvasEventForCanvas(run.WorkflowID, run.RootEventID)
		if err != nil {
			return nil, status.Errorf(codes.NotFound, "root event not found: %v", err)
		}

		run.RootEvent = rootEvent
	}

	rootEvent, err := SerializeCanvasEvent(*run.RootEvent)
	if err != nil {
		return nil, err
	}
