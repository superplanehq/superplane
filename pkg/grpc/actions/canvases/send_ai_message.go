...
func generateCanvasAIPlan(
	...
	prompt := strings.Join([]string{
		...
		"- For github.onPullRequest, set configuration.actions to match intent: use ['closed'] for close/destroy/cleanup flows, ['opened'] for open/create flows, and ['opened','closed'] only when both are explicitly requested.",
		"- Ensure github.onPullRequest node name/intent matches configured actions (never name a node 'Closed' while using action 'opened').",
		...
		"- For daily/nightly intent with explicit time (for example 'every night at 9:00'), prefer type 'days' with daysInterval: 1, hour/minute set from user time, and timezone set explicitly.",
		"- If timezone is not specified by the user for schedule requests, default to UTC by setting timezone to '0'.",
		"- Do not ask a clarifying question about timezone for schedule requests when user provided a runnable time; default to UTC ('0') and proceed.",
		"- The schedule timezone default-to-UTC rule overrides the general ambiguity/clarifying-question rule.",
		"- Use type 'cron' only when the user explicitly asks for cron syntax or a cadence that cannot be represented cleanly with built-in schedule types.",
		...
		"- If the immediately previous assistant turn asked for GitHub repository and the current user reply is short text (for example 'front'), treat it as the repository value and proceed without asking again.",
		"- Do not re-ask for repository once any valid repository value is present in either existing node configuration or the latest user reply.",
		...
		"- When removing persisted list mappings after cleanup, use clearData (not setData) with key/matchBy/matchValue.",
		"",
