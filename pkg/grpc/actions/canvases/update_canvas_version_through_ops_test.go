...
func Test__UpdateCanvasVersionThroughOps(t *testing.T) {
	r := support.Setup(t)

	t.Run("returns unauthenticated without user metadata", func(t *testing.T) {
		canvas, _ := support.CreateCanvas(t, r.Organization.ID, r.User, []models.CanvasNode{}, []models.Edge{})

		_, err := UpdateCanvasVersionThroughOps(
			context.Background(),
			r.Registry,
			r.Organization.ID,
			canvas.ID,
	...
	t.Run("applies operations and persists patched version", func(t *testing.T) {
		...
		response, err := UpdateCanvasVersionThroughOps(
			ctx,
			r.Registry,
			r.Organization.ID,
			canvas.ID,
