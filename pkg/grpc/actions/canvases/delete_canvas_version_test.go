...
func TestDeleteCanvasVersion(t *testing.T) {
	...
	t.Run("unauthenticated -> error", func(t *testing.T) {
		_, err := DeleteCanvasVersion(context.Background(), r.GitProvider, r.Organization.ID.String(), uuid.New().String(), uuid.New().String())
	...
	t.Run("invalid canvas id -> error", func(t *testing.T) {
		_, err := DeleteCanvasVersion(ctx, r.GitProvider, r.Organization.ID.String(), "invalid-id", uuid.New().String())
		s, ok := status.FromError(err)
		require.True(t, ok)
	...
	t.Run("invalid organization id -> error", func(t *testing.T) {
		_, err := DeleteCanvasVersion(ctx, r.GitProvider, "invalid-id", uuid.New().String(), uuid.New().String())
		s, ok := status.FromError(err)
		require.True(t, ok)
		assert.Equal(t, codes.InvalidArgument, s.Code())
	})
	...
	t.Run("invalid version id -> error", func(t *testing.T) {
		...
		_, err := DeleteCanvasVersion(ctx, r.GitProvider, r.Organization.ID.String(), canvasID, "invalid-id")
		s, ok := status.FromError(err)
		require.True(t, ok)
		assert.Equal(t, codes.InvalidArgument, s.Code())
	})
	...
	t.Run("canvas not found -> error", func(t *testing.T) {
		_, err := DeleteCanvasVersion(ctx, r.GitProvider, r.Organization.ID.String(), uuid.New().String(), uuid.New().String())
		s, ok := status.FromError(err)
		require.True(t, ok)
	...
	t.Run("missing version -> not found", func(t *testing.T) {
		...
		_, err := DeleteCanvasVersion(ctx, r.GitProvider, r.Organization.ID.String(), canvasID, uuid.New().String())
		s, ok := status.FromError(err)
		require.True(t, ok)
		assert.Equal(t, codes.NotFound, s.Code())
	})
	...
	t.Run("draft owned by another user -> error", func(t *testing.T) {
		...
		createResponse, err := CreateCanvasVersion(ctx, r.GitProvider, r.Registry, r.Organization.ID.String(), canvasID, "")
		...
		_, err = DeleteCanvasVersion(otherCtx, r.GitProvider, r.Organization.ID.String(), canvasID, versionID)
		s, ok := status.FromError(err)
		require.True(t, ok)
	...
	t.Run("deletes draft branch and version", func(t *testing.T) {
		...
		_, err = DeleteCanvasVersion(ctx, r.GitProvider, r.Organization.ID.String(), canvasID, versionID)
		require.NoError(t, err)
