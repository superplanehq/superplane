...
func Test__CancelExecution__CancelsExecutionSuccessfully(t *testing.T) {
	...
	response, err := CancelExecution(context.Background(), r.AuthService, r.Encryptor, "http://localhost:8080", r.Organization.ID.String(), r.Registry, canvas.ID, execution.ID)
...
func Test__CancelExecution__ReturnsNotFoundForNonExistentExecution(t *testing.T) {
	...
	_, err := CancelExecution(context.Background(), r.AuthService, r.Encryptor, "http://localhost:8080", r.Organization.ID.String(), r.Registry, canvas.ID, nonExistentID)
	require.Error(t, err)
}
...
func Test__CancelExecution__ReturnsErrorWhenCancellingChild(t *testing.T) {
	...
	_, err := CancelExecution(context.Background(), r.AuthService, r.Encryptor, "http://localhost:8080", r.Organization.ID.String(), r.Registry, canvas.ID, childExecution.ID)
	require.Error(t, err)
...
func Test__CancelExecution__CancelsAllChildrenWhenCancellingParent(t *testing.T) {
	...
	response, err := CancelExecution(context.Background(), r.AuthService, r.Encryptor, "http://localhost:8080", r.Organization.ID.String(), r.Registry, canvas.ID, parentExecution.ID)
	require.NoError(t, err)
	require.NotNil(t, response)
