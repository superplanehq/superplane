...
func TestCreateCanvasDuplicateName(t *testing.T) {
	...
	_, err := CreateCanvas(ctx, r.Registry, r.Encryptor, r.AuthService, baseURL, r.Organization.ID, "Duplicate Canvas", "", nil, nil, nil, nil)
...
func TestCreateCanvasNameUniquenessIsScopedToWorkspace(t *testing.T) {
	...
	createPlan := func(factoryID *uuid.UUID) error {
		_, err := CreateCanvas(ctx, r.Registry, r.Encryptor, r.AuthService, baseURL, r.Organization.ID, "Plan", "", factoryID, nil, nil, nil)
		return err
	}
...
func TestCreateCanvasRejectsWhitespaceOnlyName(t *testing.T) {
	...
	baseURL := "https://example.com"
	_, err := CreateCanvas(ctx, r.Registry, r.Encryptor, r.AuthService, baseURL, r.Organization.ID, "   ", "", nil, nil, nil, nil)
	require.Error(t, err)
...
func TestCreateCanvasOnFreshOrganization(t *testing.T) {
	...
	baseURL := "https://example.com"
	response, err := CreateCanvas(ctx, r.Registry, r.Encryptor, r.AuthService, baseURL, r.Organization.ID, "Health Check Monitor", "Quick start canvas on a fresh organization", nil, nil, nil, nil)
	require.NoError(t, err)
...
func TestCreateCanvasWithUsageRejectsLimitViolation(t *testing.T) {
	...
	baseURL := "https://example.com"
	_, err := CreateCanvas(ctx, r.Registry, r.Encryptor, r.AuthService, baseURL, r.Organization.ID, "Limited Canvas", "", nil, nil, nil, service)
	require.Error(t, err)
