...
func Test__InstallApplication(t *testing.T) {
	...
	t.Run("duplicate installation name -> error", func(t *testing.T) {
		...
		//
		response, err := InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "github", installationName, appConfig)
		require.NoError(t, err)
		...
		//
		_, err = InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "github", installationName, appConfig)
		require.Error(t, err)
	...
	t.Run("reuse installation name after deletion -> success", func(t *testing.T) {
		...
		//
		response2, err := InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "github", installationName, appConfig)
		require.NoError(t, err)
	...
	t.Run("different organizations can have installations with the same name -> success", func(t *testing.T) {
		...
		//
		response1, err := InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "github", installationName, appConfig)
		require.NoError(t, err)
		...
		//
		response2, err := InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, org2.ID.String(), "github", installationName, appConfig)
		require.NoError(t, err)
		require.NotNil(t, response2)
		assert.Equal(t, installationName, response2.Installation.Metadata.Name)
	...
	t.Run("application does not exist -> error", func(t *testing.T) {
		...
		//
		_, err = InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "nonexistent-app", installationName, appConfig)
		require.Error(t, err)
		s, ok := status.FromError(err)
		assert.True(t, ok)
	...
	t.Run("sync fails -> installation created in error state", func(t *testing.T) {
		...
		//
		response, err := InstallApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "dummy", installationName, appConfig)
		require.NoError(t, err)
		require.NotNil(t, response)
		require.NotNil(t, response.Installation)
