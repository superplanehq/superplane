	...
	t.Run("creates checkout and stores customer id", func(t *testing.T) {
		server := polarAPIServer(t, func(w http.ResponseWriter, req *http.Request) {
			...
			case req.Method == http.MethodPost && req.URL.Path == "/checkouts/":
				...
				assert.Equal(t, "http://localhost:8000/"+r.Organization.ID.String()+"/organization/llm-spend?credit=added", body["success_url"])
				require.NoError(t, json.NewEncoder(w).Encode(map[string]any{
...
func Test__HostedCreditCheckoutSuccessURL(t *testing.T) {
	...
	assert.Equal(
		t,
		"http://localhost:8000/11111111-1111-1111-1111-111111111111/organization/llm-spend?credit=added",
	...
	assert.Equal(
		t,
		"https://app.example/11111111-1111-1111-1111-111111111111/organization/llm-spend?credit=added",
