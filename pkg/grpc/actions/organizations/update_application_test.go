...
func Test__UpdateApplication(t *testing.T) {
	...
	t.Run("update configuration with successful sync -> installation updated", func(t *testing.T) {
		...
		//
		updateResponse, err := UpdateApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), installationID, map[string]any{"key": "value2", "new_key": "new_value"})
		require.NoError(t, err)
	...
	t.Run("update configuration with sync failure -> installation in error state", func(t *testing.T) {
		...
		//
		updateResponse, err := UpdateApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), installationID, map[string]any{"key": "value2"})
		require.NoError(t, err)
		require.NotNil(t, updateResponse)
	...
	t.Run("invalid installation ID -> error", func(t *testing.T) {
		...
		//
		_, err := UpdateApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), "invalid-uuid", map[string]any{"key": "value"})
		require.Error(t, err)
	...
	t.Run("non-existent installation -> error", func(t *testing.T) {
		...
		//
		_, err := UpdateApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), uuid.NewString(), map[string]any{"key": "value"})
		require.Error(t, err)
		s, ok := status.FromError(err)
		assert.True(t, ok)
	...
	t.Run("update preserves existing configuration keys -> success", func(t *testing.T) {
		...
		// Update only one key
		//
		_, err = UpdateApplication(ctx, r.Registry, nil, baseURL, baseURL, r.Organization.ID.String(), installationID, map[string]any{"key2": "updated_value2"})
		require.NoError(t, err)
