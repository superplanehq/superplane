...
func Test__UpdateIntegrationCapabilities(t *testing.T) {
	...
	t.Run("no capability changes -> invalid argument", func(t *testing.T) {
		...
		pbState := pb.Integration_CapabilityState_STATE_UNAVAILABLE
		switch current {
		case core.IntegrationCapabilityStateEnabled:
			pbState = pb.Integration_CapabilityState_STATE_ENABLED
		case core.IntegrationCapabilityStateDisabled:
			pbState = pb.Integration_CapabilityState_STATE_DISABLED
		case core.IntegrationCapabilityStateRequested:
			pbState = pb.Integration_CapabilityState_STATE_REQUESTED
		case core.IntegrationCapabilityStateUnavailable:
			pbState = pb.Integration_CapabilityState_STATE_UNAVAILABLE
		}

		_, err = UpdateIntegrationCapabilities(ctx, r.Registry, r.Organization.ID.String(), idStr, []*pb.Integration_CapabilityState{
