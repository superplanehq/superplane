...
func Test__UpdateIntegration(t *testing.T) {
	...
	t.Run("successful retry preserves a new integration error", func(t *testing.T) {
		syncCount := 0
		r.Registry.Integrations["dummy"] = impl.NewDummyIntegration(impl.DummyIntegrationOptions{
			OnSync: func(syncCtx core.SyncContext) error {
				syncCount++
				if syncCount == 1 {
					return errors.New("temporary sync failure")
				}
				syncCtx.Integration.Error("credential check failed")
				return nil
			},
		})
		...
		require.NoError(t, err)
		assert.Equal(t, models.IntegrationStateError, updateResponse.Integration.Status.State)
		assert.Equal(t, "credential check failed", updateResponse.Integration.Status.StateDescription)
	})
