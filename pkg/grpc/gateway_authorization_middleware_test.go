...
// caught by the gateway recovery middleware, and surface as a fatal Sentry
// event with sentryRecoveryHandler as the culprit. The middleware now reads
// the mux through a getter resolved at request time.
func TestGatewayAuthorizationMiddlewareAuthDeniedDoesNotPanic(t *testing.T) {
	...
	// /api/v1/canvases is in DefaultAuthorizationRules, so it requires auth.
	// With no x-user-id header AuthorizeHTTP fails and the middleware has to
	// render a gateway error response through the mux.
	r := httptest.NewRequest(http.MethodGet, "/api/v1/canvases", nil)
...
func TestGatewayAuthorizationMiddlewareWouldPanicWithNilMux(t *testing.T) {
	...
	assert.True(t, requiresAuth, "/api/v1/canvases must require auth for this test to be meaningful")
