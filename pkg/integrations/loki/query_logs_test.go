...
func Test__QueryLogs__Execute(t *testing.T) {
			...
			Responses: []*http.Response{
				{
					...
					Body: io.NopCloser(strings.NewReader(`{
						...
						"data": {
							...
							"result": [
								{
									"stream": {"job": "superplane", "env": "prod"},
									"values": [
										["1706400000000000000", "Deployment completed"],
										["1706399900000000000", "Starting deployment"]
									]
								}
							]
						}
	...
	t.Run("query with time range", func(t *testing.T) {
		...
		require.Len(t, httpContext.Requests, 1)
		req := httpContext.Requests[0]
		assert.Contains(t, req.URL.RawQuery, "start=")
		assert.Contains(t, req.URL.RawQuery, "end=")
		assert.Contains(t, req.URL.RawQuery, "limit=50")
		assert.Contains(t, req.URL.RawQuery, "direction=forward")
	})
			...
			Responses: []*http.Response{
				{
					...
					Body:       io.NopCloser(strings.NewReader(`{"error": "parse error"}`)),
				},
			},
		}
