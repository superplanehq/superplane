...
func (c *Client) GetDeployment(spaceID, deploymentID string) (Deployment, error) {
	path := fmt.Sprintf("/api/%s/deployments/%s", url.PathEscape(spaceID), url.PathEscape(deploymentID))
	_, body, err := c.execRequest(http.MethodGet, path, nil, nil)
	if err != nil {
		return Deployment{}, err
	}
