...
// GetIssue fetches a single issue by its key or numeric issue id.
func (c *Client) GetIssue(issueKey string) (*Issue, error) {
	issueURL := fmt.Sprintf("%s/rest/api/3/issue/%s", c.BaseURL, url.PathEscape(issueKey))
	responseBody, err := c.execRequest(http.MethodGet, issueURL, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) FetchCloudID() (string, error) {
	tenantURL := strings.TrimSuffix(c.BaseURL, "/") + "/_edge/tenant_info"
