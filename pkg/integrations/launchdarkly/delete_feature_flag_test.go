...
func Test__DeleteFeatureFlag__Setup(t *testing.T) {
	component := &DeleteFeatureFlag{}
...
func Test__DeleteFeatureFlag__Execute(t *testing.T) {
	component := &DeleteFeatureFlag{}

	t.Run("success deletes flag and emits output", func(t *testing.T) {
		...
		assert.Equal(t, "https://app.launchdarkly.com/api/v2/flags/default/old-feature", req.URL.String())
		assert.True(t, execStateCtx.Passed)
		require.Len(t, execStateCtx.Payloads, 1)
		payload := execStateCtx.Payloads[0].(map[string]any)
		assert.Equal(t, "launchdarkly.flag.deleted", payload["type"])
		data, ok := payload["data"].(map[string]any)
		require.True(t, ok)
		assert.Equal(t, "default", data["projectKey"])
