...
func (c *CreateIncident) Setup(ctx core.SetupContext) error {
	...
	if spec.AssignmentGroup != "" {
		...
		if err != nil {
			return fmt.Errorf("error verifying assignment group: %w", err)
		}
	...
	if spec.AssignedTo != "" {
		...
		if err != nil {
			return fmt.Errorf("error verifying assigned user: %w", err)
		}
	...
	if spec.Caller != "" {
		...
		if err != nil {
			return fmt.Errorf("error verifying caller: %w", err)
		}
