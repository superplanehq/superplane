...
func (w *WebhookID) UnmarshalJSON(data []byte) error {
	if len(data) == 0 || string(data) == "null" {
		*w = ""
		return nil
	}
	...
	if err := json.Unmarshal(data, &asString); err == nil {
		*w = WebhookID(asString)
		return nil
	}

	var asNumber json.Number
	if err := json.Unmarshal(data, &asNumber); err == nil {
		*w = WebhookID(asNumber.String())
		return nil
	}
...
func (c *Client) CreateWebhook(namespace, repository, name, hookURL string) (*Webhook, error) {
	namespace = strings.TrimSpace(namespace)
	repository = strings.TrimSpace(repository)
	if namespace == "" || repository == "" {
		return nil, fmt.Errorf("namespace and repository are required")
	...
	path := fmt.Sprintf("/v2/repositories/%s/%s/webhooks/", namespace, repository)
	_, responseBody, err := c.doRequest(http.MethodPost, path, bytes.NewReader(payload))
	if err != nil {
		return nil, err
	}
	...
	if webhook.ID == "" {
		return nil, fmt.Errorf("webhook response missing id")
	}

	return &webhook, nil
}

func (c *Client) DeleteWebhook(namespace, repository, webhookID string) error {
	namespace = strings.TrimSpace(namespace)
	repository = strings.TrimSpace(repository)
	webhookID = strings.TrimSpace(webhookID)
	if namespace == "" || repository == "" || webhookID == "" {
		return fmt.Errorf("namespace, repository, and webhook id are required")
	}

	path := fmt.Sprintf("/v2/repositories/%s/%s/webhooks/%s/", namespace, repository, webhookID)
