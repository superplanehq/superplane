...
//
// CloudWatch Logs is a separate AWS API from the alarms/metrics API above: JSON
// protocol instead of XML query protocol, and a different service endpoint
// (logs.*, not monitoring.*). LogsClient keeps that protocol boundary explicit
// instead of overloading Client with a second wire format.
...
// LogsClient talks to the CloudWatch Logs API.
