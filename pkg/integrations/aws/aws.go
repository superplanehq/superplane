...
- Add permissions for CloudWatch alarms if you use the CloudWatch or EC2 alarm components. The namespace, metric and dimension pickers require **cloudwatch:ListMetrics**, and the alarm picker requires **cloudwatch:DescribeAlarms**. Creating and updating alarms also needs **cloudwatch:PutMetricAlarm**, and deleting them needs **cloudwatch:DeleteAlarms**. Alarms that notify an SNS topic need **sns:ListTopics** for the topic picker, and alarms with EC2 automation actions (recover, reboot, stop, terminate) additionally need **iam:CreateServiceLinkedRole**.
