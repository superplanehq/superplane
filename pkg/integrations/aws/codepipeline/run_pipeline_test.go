...
func Test__RunPipeline__Metadata(t *testing.T) {
	component := &RunPipeline{}

	assert.Equal(t, "aws.codepipeline.runPipeline", component.Name())
	assert.Equal(t, "CodePipeline • Run Pipeline", component.Label())
	...
	assert.Equal(t, "region", config[0].Name)
	assert.Equal(t, "pipeline", config[1].Name)
