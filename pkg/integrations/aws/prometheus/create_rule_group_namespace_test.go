...
func Test__CreateRuleGroupNamespace__Execute(t *testing.T) {
	...
	t.Run("valid request -> emits namespace and base64-encodes the rules document", func(t *testing.T) {
		...
		assert.Equal(t, "CREATING", namespace.Status.StatusCode)
		...
		require.NoError(t, err)
		assert.Equal(t, "example-alerts", sentPayload["name"])
	...
	t.Run("non-existent workspace -> wrapped error includes AWS error details", func(t *testing.T) {
		httpContext := &contexts.HTTPContext{
			Responses: []*http.Response{
				{
					StatusCode: http.StatusNotFound,
					Body:       io.NopCloser(strings.NewReader(`{"__type":"ResourceNotFoundException","message":"workspace ws-missing not found"}`)),
				},
			},
		}
		...
		require.ErrorContains(t, err, "failed to create Prometheus rule group namespace")
		require.ErrorContains(t, err, "workspace ws-missing not found")
	})
}
