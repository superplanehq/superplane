...
func (c *CreateRuleGroupNamespace) Documentation() string {
	return `The Create Rule Group Namespace component creates a new rule group namespace in an Amazon Managed Service for Prometheus workspace, uploading a YAML document with one or more Prometheus alerting or recording rule groups.
...
func (c *CreateRuleGroupNamespace) Configuration() []configuration.Field {
	return []configuration.Field{
		regionField(),
		workspaceField("Workspace", "Workspace to create the rule group namespace in"),
		ruleGroupsNamespaceNameField("Name for the new rule group namespace"),
		ruleGroupsNamespaceDataField("YAML document containing one or more Prometheus alerting or recording rule groups"),
		clientTokenField(),
		tagsField("Tags to associate with the rule group namespace"),
	}
}
...
func (c *CreateRuleGroupNamespace) Execute(ctx core.ExecutionContext) error {
	...
	response, err := client.CreateRuleGroupsNamespace(CreateRuleGroupsNamespaceInput{
		WorkspaceID: config.WorkspaceID,
		Name:        config.Name,
		Data:        config.Data,
...
func (c *CreateRuleGroupNamespace) decodeConfiguration(rawConfiguration any) (CreateRuleGroupNamespaceConfiguration, error) {
	...
	if err != nil {
		return CreateRuleGroupNamespaceConfiguration{}, err
	}
	config.Data = data
