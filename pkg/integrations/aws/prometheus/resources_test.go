...
func Test__ListRuleGroupsNamespaces(t *testing.T) {
	t.Run("missing region -> error", func(t *testing.T) {
		_, err := ListRuleGroupsNamespaces(core.ListResourcesContext{
			Integration: validIntegrationContext(),
			Parameters:  map[string]string{"workspace": "ws-abc123"},
	...
	t.Run("valid request -> returns rule group namespace resources", func(t *testing.T) {
		httpContext := &contexts.HTTPContext{
			Responses: []*http.Response{
				{
					StatusCode: http.StatusOK,
					Body: io.NopCloser(strings.NewReader(`{
						"ruleGroupsNamespaces": [
							{"name": "example-alerts", "arn": "arn:aws:aps:us-east-1:123456789012:rulegroupsnamespace/ws-abc123/example-alerts", "status": {"statusCode": "ACTIVE"}, "createdAt": 1700000000, "modifiedAt": 1700000000},
							{"name": "recording-rules", "arn": "arn:aws:aps:us-east-1:123456789012:rulegroupsnamespace/ws-abc123/recording-rules", "status": {"statusCode": "CREATING"}, "createdAt": 1700000000, "modifiedAt": 1700000000}
						]
					}`)),
				},
			},
		...
		require.NoError(t, err)
		assert.Equal(t, []core.IntegrationResource{
			{Type: ruleGroupsNamespaceResourceType, Name: "example-alerts", ID: "example-alerts"},
			{Type: ruleGroupsNamespaceResourceType, Name: "recording-rules", ID: "recording-rules"},
		}, resources)
