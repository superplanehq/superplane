...
func Test__GetBuildStatus__Setup(t *testing.T) {
	component := &GetBuildStatus{}
...
func Test__GetBuildStatus__Execute(t *testing.T) {
	component := &GetBuildStatus{}
	...
	t.Run("valid request -> emits build details", func(t *testing.T) {
	...
	t.Run("build not found -> error", func(t *testing.T) {
		httpContext := &contexts.HTTPContext{
			Responses: []*http.Response{
				{
					StatusCode: http.StatusOK,
					Body: io.NopCloser(strings.NewReader(`{
						"builds": [],
						"buildsNotFound": ["my-project:abc123"]
					}`)),
				},
			},
		}
		...
		require.ErrorContains(t, err, "build not found")
	})
}
