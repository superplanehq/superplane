...
func Test__DeleteAlarm__Execute(t *testing.T) {
	...
	t.Run("deletes alarm and emits confirmation", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Contains(t, string(body), "Action=DeleteAlarms")
		assert.Contains(t, string(body), "AlarmNames.member.1=HighCPU")
	})
}
