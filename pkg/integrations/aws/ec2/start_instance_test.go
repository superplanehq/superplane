...
func Test__StartInstance__Setup(t *testing.T) {
...
func Test__StartInstance__Execute(t *testing.T) {
	component := &StartInstance{}
	...
	t.Run("valid request -> emits instance state change", func(t *testing.T) {
		...
		require.True(t, ok)
		stateChange, ok := output["instanceStateChange"].(*StartInstanceOutput)
		require.True(t, ok)
		assert.Equal(t, "i-1234567890abcdef0", stateChange.InstanceID)
		assert.Equal(t, "pending", stateChange.CurrentState)
		assert.Equal(t, "stopped", stateChange.PreviousState)
		...
		assert.Contains(t, requestBody, "Action=StartInstances")
