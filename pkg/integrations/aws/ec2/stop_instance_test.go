...
func Test__StopInstance__Setup(t *testing.T) {
	component := &StopInstance{}
...
func Test__StopInstance__Execute(t *testing.T) {
	component := &StopInstance{}
	...
	t.Run("valid request -> emits instance state change", func(t *testing.T) {
		...
		require.True(t, ok)
		stateChange, ok := output["instanceStateChange"].(*StopInstanceOutput)
		require.True(t, ok)
		assert.Equal(t, "i-1234567890abcdef0", stateChange.InstanceID)
		assert.Equal(t, "stopping", stateChange.CurrentState)
		assert.Equal(t, "running", stateChange.PreviousState)
		assert.Equal(t, "us-east-1", stateChange.Region)
		...
		assert.Contains(t, requestBody, "Action=StopInstances")
