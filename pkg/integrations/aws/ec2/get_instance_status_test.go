...
func Test__GetInstanceStatus__Setup(t *testing.T) {
	component := &GetInstanceStatus{}
...
func Test__GetInstanceStatus__Execute(t *testing.T) {
	component := &GetInstanceStatus{}
	...
	t.Run("valid request -> emits instance status", func(t *testing.T) {
		...
		require.True(t, ok)
		status, ok := output["instanceStatus"].(*InstanceStatusCheck)
		require.True(t, ok)
		assert.Equal(t, "i-1234567890abcdef0", status.InstanceID)
		assert.Equal(t, "running", status.InstanceState)
		assert.Equal(t, "ok", status.SystemStatus)
		assert.Equal(t, "ok", status.InstanceCheck)
		...
		assert.Contains(t, requestBody, "Action=DescribeInstanceStatus")
