...
- **Cleanup workflows**: Remove temporary buckets after processing
- **Environment teardown**: Delete buckets during environment decommissioning
...
func (c *DeleteBucket) Execute(ctx core.ExecutionContext) error {
	...
	if err := client.DeleteBucket(config.Bucket); err != nil {
		return fmt.Errorf("failed to delete bucket %q: %w", config.Bucket, err)
	}
