...
func Test__EmptyBucket__Execute(t *testing.T) {
	...
	t.Run("valid request with objects -> emits emptied result", func(t *testing.T) {
	...
	t.Run("empty bucket -> emits emptied result with zero count", func(t *testing.T) {
		httpContext := &contexts.HTTPContext{
			Responses: []*http.Response{
				{
					StatusCode: http.StatusOK,
		...
		data, ok := payloadWrapper["data"].(map[string]any)
		require.True(t, ok)
		deletedCount, ok := data["deletedCount"].(int)
		require.True(t, ok)
