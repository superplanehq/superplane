...
- **Change detection**: Retrieve LastModified and ETag to detect if a file has been updated since the last workflow run
- **Config reading**: Fetch a JSON or YAML config file and use its contents in downstream workflow steps
- **Pre-flight checks**: Verify a file exists and check its size or type before processing
- **Backup verification**: Confirm a backup file was created today by checking LastModified
- **Conditional processing**: Route workflow based on whether an object exists or not
- **Tag-based routing**: Use object tags to route workflow logic (e.g. status=ready)

## Configuration

- **Region**: The Spaces region where the bucket is located (required)
- **Bucket**: The Spaces bucket to read from (required)
- **File Path**: The path to the object within the bucket, e.g. reports/daily.csv (required)
- **Include Body**: Download the object content. Only available for text-based content types (JSON, YAML, plain text, etc.).

> Spaces Access Key and Secret Key are configured in the DigitalOcean integration settings.
...
- **body**: Object content as a string (only present for readable content types when Include Body is enabled)`
}
