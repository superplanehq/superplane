...
const (
	...
	onObjectFinalizedEvent = "gcp.cloudstorage.objectFinalized"
...
func (t *OnObjectFinalized) Description() string {
	return "Trigger a workflow when a new object is created or overwritten in a Cloud Storage bucket"
}

func (t *OnObjectFinalized) Documentation() string {
	return `The On Object Finalized trigger starts a workflow execution when a new object is created or overwritten in a Google Cloud Storage bucket.
...
- **Data pipeline automation**: Process files as soon as they arrive in a bucket
- **ETL workflows**: Trigger data transformations when new data lands in storage
- **Backup verification**: Run checks when new backups are uploaded
- **Media processing**: Start transcoding or analysis when media files are uploaded

## Setup

**Required GCP setup:**
1. Enable the **Pub/Sub** API and **Cloud Logging** API in your project.
2. Enable **Data Access audit logs** for Cloud Storage in the Google Cloud Console under IAM & Admin → Audit Logs → Cloud Storage. Make sure "Admin Read" and "Data Write" are checked.
3. The integration's service account must have ` + "`roles/logging.configWriter`" + ` and ` + "`roles/pubsub.admin`" + ` permissions.

SuperPlane automatically creates a Cloud Logging sink to capture Cloud Storage object creation events.

## Configuration

- **Bucket**: Optionally filter events to a specific bucket. Leave empty to listen for object creation across all buckets in the project.

## Event Data

Each event includes the audit log entry with resourceName, serviceName (storage.googleapis.com), methodName (storage.objects.create), and the full log entry data.`
}
	...
	return []configuration.Field{
		{
			...
			Required:    false,
			Description: "Optionally filter events to a specific Cloud Storage bucket. Leave empty to listen for all buckets.",
...
func (t *OnObjectFinalized) OnIntegrationMessage(ctx core.IntegrationMessageContext) error {
	...
	if config.Bucket != "" && !resourceMatchesBucket(event.ResourceName, config.Bucket) {
		return nil
	}

	return ctx.Events.Emit(onObjectFinalizedEvent, ctx.Message)
}
...
func resourceMatchesBucket(resourceName, bucket string) bool {
