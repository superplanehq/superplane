...
func errStatusCode(err error) int {
	...
	msg := err.Error()
	// Internal secret retrieval (e.g. DB/decryption) -> 500 so Linear does not treat as auth rejection
	if strings.Contains(msg, "get webhook secret") {
		return http.StatusInternalServerError
	}
	// Auth/signature validation failure -> 403
	if strings.Contains(msg, "secret") || strings.Contains(msg, "signature") || strings.Contains(msg, "Linear-Signature") {
