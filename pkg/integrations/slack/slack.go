	...
	manifest := map[string]any{
		...
		"settings": map[string]any{
			"event_subscriptions": map[string]any{
				"request_url": fmt.Sprintf("%s/api/v1alpha/integrations/%s/events", appURL, ctx.Integration.ID().String()),
				"bot_events": []string{
					"app_mention",
					"reaction_added",
					"reaction_removed",
			...
			"interactivity": map[string]any{
				"is_enabled":  true,
				"request_url": fmt.Sprintf("%s/api/v1alpha/integrations/%s/interactions", appURL, ctx.Integration.ID().String()),
			},
