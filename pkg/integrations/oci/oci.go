...
func (o *OCI) Sync(ctx core.SyncContext) error {
	...
	privateKeyRaw, err := ctx.Integration.GetConfig("privateKey")
	if err != nil {
		return fmt.Errorf("failed to retrieve unencrypted private key from secure storage: %w", err)
	}
	if len(privateKeyRaw) == 0 {
		return fmt.Errorf("private key not found in secure storage")
	}
	...
	if err != nil {
		return fmt.Errorf("failed to initialize OCI client for validation: %w", err)
	}

	limit := 1
	req := ocicore.ListInstancesRequest{
	...
	if _, err := client.ListInstances(ctxTimeout, req); err != nil {
		return fmt.Errorf("failed to validate OCI credentials: %w", err)
	}
