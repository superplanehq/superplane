...
func (o *OCI) Sync(ctx core.SyncContext) error {
	...
	// Security: Do NOT store sensitive fields in metadata.
	// Metadata is for non-sensitive, frequently accessed info.
	...
	// Framework handles storing the full configuration (including sensitive fields)
	// securely in its own encrypted storage, accessible via GetConfig.
...
func (h *OCIWebhookHandler) HandleRequest(ctx core.HTTPRequestContext) {
	// Retrieve sensitive webhook token from secure configuration
	...
	ctx.Integration.Subscribe(nil) // Framework dispatches based on event data
