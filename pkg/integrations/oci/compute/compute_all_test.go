...
func (m *MockExecutionState) Emit(channel, payloadType string, payloads []any) error { return m.Called(channel, payloadType, payloads).Error(0) }
func (m *MockExecutionState) Pass() error { return m.Called().Error(0) }
func (m *MockExecutionState) Fail(reason, message string) error { return m.Called(reason, message).Error(0) }
...
func TestCreateInstance(t *testing.T) {
	...
	_ = action.Execute(core.ExecutionContext{
		Context: context.Background(),
		Configuration: map[string]interface{}{"compartmentId": "c1", "displayName": "test", "shape": "VM", "imageId": "i1", "subnetId": "s1"},
