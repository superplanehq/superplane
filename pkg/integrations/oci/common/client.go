...
func NewConfigurationProvider(ctx core.IntegrationContext) (ocicommon.ConfigurationProvider, error) {
	...
	// Security: Retrieve private key from secure configuration storage
	privateKeyRaw, err := ctx.GetConfig("privateKey")
	if err != nil || len(privateKeyRaw) == 0 {
		return nil, fmt.Errorf("failed to retrieve private key from secure storage")
	}
