...
- **OpenCost Base URL**: URL of your OpenCost instance (e.g., ` + "`http://opencost.example.com:9003`" + `)
- **API Auth**: ` + "`none`" + `, ` + "`basic`" + `, or ` + "`bearer`" + ` for API requests

### OpenCost Setup

OpenCost exposes a REST API on port 9003 by default. Ensure your OpenCost instance is accessible from SuperPlane.`
}
...
func (o *OpenCost) Sync(ctx core.SyncContext) error {
	...
	if _, err := client.GetAllocation("1h", "namespace"); err != nil {
		return fmt.Errorf("error validating connection: %w", err)
	}
