...
func (t *OnDeploy) Setup(ctx core.TriggerContext) error {
	...
	return ctx.Integration.RequestWebhook(
		webhookConfigurationForResource(ctx.Integration, webhookResourceTypeDeploy, requestedEventTypes),
	)
}
...
func (t *OnDeploy) HandleWebhook(ctx core.WebhookRequestContext) (int, *core.WebhookResponseBody, error) {
	config, err := decodeOnResourceEventConfiguration(ctx.Configuration)
	if err != nil {
