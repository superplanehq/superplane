...
func Test__GetFlakyTests__Execute(t *testing.T) {
	...
	t.Run("fetches flaky tests -> emits to default channel", func(t *testing.T) {
		httpContext := &contexts.HTTPContext{
			Responses: []*http.Response{
				{
					StatusCode: http.StatusOK,
		...
		assert.Equal(t, "circleci.insights.flaky-tests", executionState.Type)
		require.Len(t, httpContext.Requests, 1)
		assert.Contains(t, httpContext.Requests[0].URL.String(), "/insights/gh/org/my-repo/flaky-tests")
	})
}
