...
func Test__NewClient(t *testing.T) {
	...
	t.Run("success with default base URL", func(t *testing.T) {
		integrationCtx := &contexts.IntegrationContext{
			Configuration: map[string]any{"apiKey": "apikey-123"},
		}

		client, err := NewClient(httpCtx, integrationCtx)

		require.NoError(t, err)
		assert.Equal(t, "apikey-123", client.APIKey)
...
func Test__extractResponseText(t *testing.T) {
	...
	t.Run("prefers output_text", func(t *testing.T) {
		resp := &Response{OutputText: "top-level text"}
		assert.Equal(t, "top-level text", extractResponseText(resp))
	})

	t.Run("falls back to output content", func(t *testing.T) {
		resp := &Response{
		...
		assert.Equal(t, "first\nsecond", extractResponseText(resp))
	})
}
