...
func (w *OnWorkflowRun) DefaultRunTitle() string {
	return `{{ root().data.workflow_run.display_title != "" ? root().data.workflow_run.display_title : (root().data.workflow_run.name != "" ? root().data.workflow_run.name : (root().data.workflow.name != "" ? root().data.workflow.name : "Workflow")) }}`
}
