...
func Test__CreateIssueComment__Configuration(t *testing.T) {
	...
	t.Run("has required fields", func(t *testing.T) {
		...
		// Check issue number field
		issueField := config[1]
		require.Equal(t, "issueNumber", issueField.Name)
		require.True(t, issueField.Required)

		// Check body field
		bodyField := config[2]
		require.Equal(t, "body", bodyField.Name)
		require.True(t, bodyField.Required)
	})
}
