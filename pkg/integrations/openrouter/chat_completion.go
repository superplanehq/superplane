...
func providerRoutingSchema() []configuration.Field {
	return []configuration.Field{
		...
		},
		{
			...
			Required:    false,
			Default:     false,
			Description: "Only route to providers that honour every parameter sent. Otherwise unsupported parameters are accepted and ignored. Forced on whenever Structured Output is configured.",
		},
		{
			Name:        "dataCollection",
...
//
// It overrides whatever the routing block carries rather than only filling an
// unset value. The field defaults to false and the UI persists that default as
// soon as Provider Routing is switched on, so an unset value is
// indistinguishable from a deliberate opt-out; honouring it would silently drop
// the pin exactly when routing is in use.
