...
func Test__OnMessage__Setup(t *testing.T) {
	trigger := &OnMessage{}
	...
	t.Run("empty channel -> subscribes without channel metadata", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, []string{"message"}, subConfig.EventTypes)

		stored, ok := metadata.Metadata.(OnMessageMetadata)
		require.True(t, ok)
	...
	t.Run("same channel -> no new subscription", func(t *testing.T) {
		...
		metadata := &contexts.MetadataContext{Metadata: OnMessageMetadata{
...
func Test__OnMessage__OnIntegrationMessage(t *testing.T) {
	trigger := &OnMessage{}
	...
	t.Run("channel match -> emit", func(t *testing.T) {
		...
		assert.Equal(t, "teams.channel.message", events.Payloads[0].Type)
