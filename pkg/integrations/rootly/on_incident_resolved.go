...
func (t *OnIncidentResolved) Documentation() string {
	return `The On Incident Resolved trigger starts a workflow execution when a Rootly incident is marked as resolved.

## Use Cases

- **Ticket closure**: Close linked Jira / ServiceNow items when the incident is resolved
- **Status updates**: Post to a status page or Slack when resolution is recorded
- **Post-incident automation**: Trigger retrospectives, action-item sync, and cleanup steps
...
Each resolved incident event includes:
- **event**: ` + "`incident.resolved`" + `
- **incident**: Incident data. At minimum includes: id, sequential_id, title, slug, status, resolution_message, resolved_at, resolved_by, url
...
func (t *OnIncidentResolved) HandleWebhook(ctx core.WebhookRequestContext) (int, error) {
	...
	if !allowedByFilters(webhook.Data, config) {
		return http.StatusOK, nil
	}

	err = ctx.Events.Emit(
		fmt.Sprintf("rootly.%s", eventType),
		buildResolvedIncidentPayload(webhook),
	)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("error emitting event: %v", err)
	}
...
func allowedByFilters(incident map[string]any, config OnIncidentResolvedConfiguration) bool {
	if len(config.SeverityFilter) > 0 && !matchesSeverityFilter(incident, config.SeverityFilter) {
		return false
	}
	if len(config.ServiceFilter) > 0 && !matchesListFilter(incident, "services", config.ServiceFilter) && !matchesListFilter(incident, "service", config.ServiceFilter) {
		return false
	}
	if len(config.TeamFilter) > 0 && !matchesListFilter(incident, "teams", config.TeamFilter) && !matchesListFilter(incident, "team", config.TeamFilter) {
		return false
	}
	return true
}

func matchesSeverityFilter(incident map[string]any, allowed []string) bool {
	if incident == nil {
		return false
	}

	// Common shapes we try to support:
	// - {"severity":"sev2"} (string)
	// - {"severity":{"id":"...","slug":"...","name":"..."}} (object)
	// - {"attributes":{"severity":"sev2"}} or {"attributes":{"severity":{"id":...}}}
	if matchesStringOrObject(incident, "severity", allowed) {
		return true
	}
	if attrs, ok := incident["attributes"].(map[string]any); ok {
		if matchesStringOrObject(attrs, "severity", allowed) {
			return true
		}
	}
...
func matchesListFilter(incident map[string]any, field string, allowed []string) bool {
	if incident == nil {
		return false
	}

	// Common shapes we try to support:
	// - {"services":[{"id":"..."},{"slug":"..."},{"name":"..."}]}
	// - {"services":["svc-1","svc-2"]}
	// - {"attributes":{"services":[...]}}
	if matchesStringArrayOrObjectArray(incident, field, allowed) {
		return true
	}
	if attrs, ok := incident["attributes"].(map[string]any); ok {
		if matchesStringArrayOrObjectArray(attrs, field, allowed) {
			return true
		}
	}
...
func matchesStringOrObject(m map[string]any, field string, allowed []string) bool {
	val, ok := m[field]
	if !ok || val == nil {
		return false
	}
	...
	for _, key := range []string{"id", "slug", "name"} {
		if s, ok := obj[key].(string); ok && slices.Contains(allowed, s) {
			return true
		}
	}
...
func matchesStringArrayOrObjectArray(m map[string]any, field string, allowed []string) bool {
	val, ok := m[field]
	if !ok || val == nil {
		return false
	}
...
func buildResolvedIncidentPayload(webhook WebhookPayload) map[string]any {
	payload := map[string]any{
		"event":     webhook.Event.Type,
		"event_id":  webhook.Event.ID,
		"issued_at": webhook.Event.IssuedAt,
	...
	// Ensure these commonly-used fields exist at the top-level of incident payload,
	// even if Rootly sends them nested under "attributes".
	ensureIncidentField(incident, webhook.Data, "id")
	ensureIncidentField(incident, webhook.Data, "sequential_id")
	ensureIncidentField(incident, webhook.Data, "title")
	ensureIncidentField(incident, webhook.Data, "slug")
	ensureIncidentField(incident, webhook.Data, "status")
	ensureIncidentField(incident, webhook.Data, "resolution_message")
	ensureIncidentField(incident, webhook.Data, "resolved_at")
	ensureIncidentField(incident, webhook.Data, "resolved_by")
	ensureIncidentField(incident, webhook.Data, "url")

	payload["incident"] = incident
	return payload
}

func ensureIncidentField(dst map[string]any, src map[string]any, key string) {
	if dst == nil || src == nil {
		return
	}
	if _, ok := dst[key]; ok {
