...
type OrganizationOktaSettings struct {
...
func (s *Server) getOrganizationOktaConfig(w http.ResponseWriter, r *http.Request) {
	account, ok := middleware.GetAccountFromContext(r.Context())
	if !ok {
		http.Error(w, "", http.StatusUnauthorized)
		return
	...
	roles, err := s.authService.GetUserRolesForOrg(user.ID.String(), org.ID.String())
	if err != nil {
	...
	for _, role := range roles {
		if role.Name == models.RoleOrgOwner || role.Name == models.RoleOrgAdmin {
			isAdmin = true
			break
		}
	}
	...
	if err != nil {
		// No config yet: return empty settings
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(OrganizationOktaSettings{
			SamlIssuer:      "",
			SamlCertificate: "",
			EnforceSSO:      false,
			HasSCIMToken:    false,
	...
	settings := OrganizationOktaSettings{
...
func (s *Server) updateOrganizationOktaConfig(w http.ResponseWriter, r *http.Request) {
	account, ok := middleware.GetAccountFromContext(r.Context())
	if !ok {
		http.Error(w, "", http.StatusUnauthorized)
		return
	...
	var payload OrganizationOktaSettings
	...
	config.SamlIssuer = payload.SamlIssuer
	config.SamlCertificate = payload.SamlCertificate
	config.EnforceSSO = payload.EnforceSSO

	if err := database.Conn().Save(config).Error; err != nil {
...
func (s *Server) rotateOrganizationOktaSCIMToken(w http.ResponseWriter, r *http.Request) {
	account, ok := middleware.GetAccountFromContext(r.Context())
	if !ok {
		http.Error(w, "", http.StatusUnauthorized)
		return
	...
	config, err := models.FindOrganizationOktaConfig(org.ID)
	if err != nil {
		http.Error(w, "Okta not configured for organization", http.StatusBadRequest)
		return
	}

	plainToken, err := crypto.Base64String(32)
	if err != nil {
		http.Error(w, "failed to generate token", http.StatusInternalServerError)
		return
	}

	config.ScimTokenHash = hashSCIMToken(plainToken)
