...
func (s *Server) RegisterGRPCGateway(grpcServerAddr string) error {
	...
	err = pbExtensions.RegisterExtensionsHandlerFromEndpoint(ctx, grpcGatewayMux, grpcServerAddr, opts)
	if err != nil {
		return err
	}
	...
	s.Router.PathPrefix("/api/v1/extensions").Handler(protectedGRPCHandler)
