...
func Test__NodeRequestWorker_InvokeTriggerAction(t *testing.T) {
	r := support.Setup(t)
	defer r.Close()
	worker := NewNodeRequestWorker(r.Encryptor, r.Registry, r.BaseURL)
...
func Test__NodeRequestWorker_PreventsConcurrentProcessing(t *testing.T) {
	...
	//
	go func() {
		worker1 := NewNodeRequestWorker(r.Encryptor, r.Registry, r.BaseURL)
		results <- worker1.LockAndProcessRequest(request)
	}()

	go func() {
		worker2 := NewNodeRequestWorker(r.Encryptor, r.Registry, r.BaseURL)
