...
func (b *NodeConfigurationBuilder) resolve(configuration map[string]any) (map[string]any, error) {
	...
	for k, v := range configuration {
		//
		// reportTemplate is resolved later against the node's outputs, not
		// its inputs, so the raw template string is carried through here.
		//
		if k == "reportTemplate" {
			result[k] = v
			continue
...
// toMapViaJSON converts a value (potentially a typed struct) to a plain
// map/slice/scalar via JSON round-trip so expression evaluation sees json
