...
// at that base URL are considered. Multiple matches on different base URLs
// without urlFilter returns an error that lists installations and suggests --url.
func SwitchContextByOrganization(orgOrID, urlFilter string) (*ConfigContext, error) {
	...
	if len(urls) == 1 {
		return nil, fmt.Errorf(
			"multiple saved contexts match %q for %s; remove duplicate entries from your config",
			orgOrID,
			urls[0],
		)
	}
