...
func CountEventsByCanvasID(canvasID uuid.UUID, sourceType string, sourceIDStr string, states []string) (int64, error) {
	...
	if sourceIDStr != "" {
		sourceID, err := uuid.Parse(sourceIDStr)
		if err != nil {
			return 0, fmt.Errorf("invalid source ID: %v", err)
		}
		query = query.Where("source_id = ?", sourceID)
	}
...
func BulkListEventsByCanvasIDAndMultipleSources(canvasID uuid.UUID, sourceFilters []SourceFilter, limitPerSource int, before *time.Time, states []string) (map[string][]Event, error) {
