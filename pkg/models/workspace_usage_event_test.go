...
func Test__ListWorkOrderRunUsage__GroupsModelAndComputeForOneRun(t *testing.T) {
	...
	assert.Contains(t, rows[0].BYOKModels, "claude-sonnet-4-6")
...
func Test__ListWorkOrderRunUsage__HostedModelDoesNotMarkYourKeys(t *testing.T) {
	...
	require.Equal(t, int64(1), total)
	require.Len(t, rows, 1)
	assert.Contains(t, rows[0].Models, "claude-sonnet-4-6")
...
func Test__ListWorkOrderRunUsage__SplitsMixedFundingModels(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, rows, 1)
	assert.Equal(t, []string{"claude-sonnet-4-6"}, rows[0].Models)
	assert.Equal(t, []string{"claude-opus-4-1"}, rows[0].BYOKModels)
}
