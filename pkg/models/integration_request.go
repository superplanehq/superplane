...
// CompletePendingSyncRequestsInTransaction completes every pending sync request for
// the installation. ScheduleResync uses it to keep at most one pending sync, scoped
// to the sync type so a resync never collapses an unrelated pending action request
// (e.g. a token-refresh loop) on an installation that uses both.
func CompletePendingSyncRequestsInTransaction(tx *gorm.DB, installationID uuid.UUID) error {
	return tx.Model(&IntegrationRequest{}).
		Where("app_installation_id = ? AND state = ? AND type = ?",
			installationID, IntegrationRequestStatePending, IntegrationRequestTypeSync).
		Updates(map[string]any{
			"state":      IntegrationRequestStateCompleted,
			"updated_at": time.Now(),
		}).
