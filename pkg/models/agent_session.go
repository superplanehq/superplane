...
func CalculateAgentSessionTokenUsageDelta(sessionID uuid.UUID, usage AgentSessionTokenUsage) (AgentSessionTokenUsage, error) {
	session, err := FindAgentSession(sessionID)
	if err != nil {
		return AgentSessionTokenUsage{}, err
	}

	return agentSessionTokenUsageDelta(session, usage), nil
}
