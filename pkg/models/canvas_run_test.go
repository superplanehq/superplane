...
func Test__CanvasRun__QueueItemKeepsRunStarted(t *testing.T) {
	...
	queueItem := support.CreateQueueItem(t, run.WorkflowID, "node-1", run.RootEventID, run.RootEventID)
...
func Test__CanvasRun__ResultPrecedence(t *testing.T) {
	...
	failedExecution := createExecutionForRun(t, run, "node-2")
...
func setupRunWithExecution(t *testing.T) (*models.CanvasRun, *models.CanvasNodeExecution) {
	...
	execution := createExecutionForRun(t, run, "node-1")
...
func createExecutionForRun(t *testing.T, run *models.CanvasRun, nodeID string) *models.CanvasNodeExecution {
	now := time.Now()
