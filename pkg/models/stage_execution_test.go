...
func Test__StageExecution(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, canvas.CreateStage("stg-1", user.String(), []StageCondition{}, ExecutorSpec{}, []StageConnection{}, []InputDefinition{}, []InputMapping{}, []OutputDefinition{}))
	...
	require.NoError(t, err)
	stageEvent, err := CreateStageEvent(stage.ID, event, StageEventStatePending, "", map[string]any{})
	require.NoError(t, err)
