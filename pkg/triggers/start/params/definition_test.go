...
func TestNewDefinition_success(t *testing.T) {
	t.Run("string", func(t *testing.T) {
		def, err := NewDefinition("body.name", ParamTypeString, "Name", "machine-1", false, nil)
	...
	t.Run("number", func(t *testing.T) {
		def, err := NewDefinition("count", ParamTypeNumber, "", float64(42), false, nil)
		require.NoError(t, err)
	...
	t.Run("boolean", func(t *testing.T) {
		def, err := NewDefinition("enabled", ParamTypeBoolean, "", true, true, nil)
		require.NoError(t, err)
	...
	t.Run("select", func(t *testing.T) {
		def, err := NewDefinition("body.size", ParamTypeSelect, "Size", "2 vCPU", true, []string{"2 vCPU", "4 vCPU"})
		require.NoError(t, err)
	...
	t.Run("select without default", func(t *testing.T) {
		def, err := NewDefinition("body.size", ParamTypeSelect, "Size", nil, true, []string{"a", "b"})
		require.NoError(t, err)
