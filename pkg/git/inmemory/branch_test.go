...
func TestProviderBranchOperations(t *testing.T) {
	...
	mainHead, err := p.Head(ctx, repoID, models.CanvasGitBranchMain)
	require.NoError(t, err)

	draftBranch := "drafts/" + uuid.New().String()
	require.NoError(t, p.CreateBranch(ctx, repoID, draftBranch, models.CanvasGitBranchMain))
	...
	mergedSHA, err := p.MergeBranch(ctx, repoID, draftBranch, models.CanvasGitBranchMain, "merge draft", provider.CommitAuthor{
		Name:  "Test",
	...
	files, err := p.ListFiles(ctx, repoID, models.CanvasGitBranchMain)
	require.NoError(t, err)
