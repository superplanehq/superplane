...
func Test__Get(t *testing.T) {
	...
	t.Run("fanOut feature is registered", func(t *testing.T) {
		f, ok := Get(FeatureFanOut)
		assert.True(t, ok)
		assert.Equal(t, FeatureFanOut, f.ID)
		assert.Equal(t, "Fan Out", f.Label)
	})
...
func Test__Exists(t *testing.T) {
	...
	assert.True(t, Exists(FeatureFanOut))
