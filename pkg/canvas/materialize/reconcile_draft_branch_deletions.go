...
// ReconcileDraftBranchDeletionsFromGit removes canvas_draft_branches metadata for DB rows
// whose git refs no longer exist. Safe to call repeatedly.
func ReconcileDraftBranchDeletionsFromGit(
	...
	dbBranches, err := models.ListDraftBranchesForCanvasInTransaction(tx, canvasID)
	if err != nil {
		return nil, err
	}
	...
	for i := range dbBranches {
		...
		if deleteErr := models.DeleteDraftBranchInTransaction(tx, canvasID, branchName); deleteErr != nil {
			return nil, deleteErr
		}
		if deleteErr := models.DeleteRepositoryMaterializationStateInTransaction(tx, canvasID, branchName); deleteErr != nil {
			return nil, deleteErr
		}
...
func PublishDraftBranchDeletionEvents(canvasID string, removed []string) {
	for _, branch := range removed {
		if publishErr := messages.NewRepositoryBranchUpdatedMessage(
			...
			branch,
			"",
			models.MaterializationStatusDeleted,
			"",
			"",
