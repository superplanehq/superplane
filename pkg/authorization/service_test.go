...
func Test__AuthService_AccessibleResources(t *testing.T) {
	...
	userID := r.User.String()
	org1 := r.Organization.ID.String()

	org, err := models.CreateOrganization("org2", "org2", "")
	require.NoError(t, err)
	require.NoError(t, r.AuthService.SetupOrganizationRoles(org.ID.String()))
	org2 := org.ID.String()

	canvas1 := r.Canvas.ID.String()
	canvas, err := models.CreateCanvas(r.User, r.Organization.ID, "", "")
	require.NoError(t, err)
	require.NoError(t, r.AuthService.SetupCanvasRoles(canvas.ID.String()))
	canvas2 := canvas.ID.String()

	t.Run("get accessible organizations", func(t *testing.T) {
		// Assign user to organizations
		err := r.AuthService.AssignRole(userID, models.RoleOrgViewer, org1, models.DomainTypeOrganization)
		require.NoError(t, err)
		err = r.AuthService.AssignRole(userID, models.RoleOrgAdmin, org2, models.DomainTypeOrganization)
		require.NoError(t, err)

		// Get accessible orgs
		orgs, err := r.AuthService.GetAccessibleOrgsForUser(userID)
		require.NoError(t, err)
	...
	t.Run("get accessible canvases", func(t *testing.T) {
		...
		err := r.AuthService.AssignRole(userID, models.RoleCanvasViewer, canvas1, models.DomainTypeCanvas)
		require.NoError(t, err)
		err = r.AuthService.AssignRole(userID, models.RoleCanvasOwner, canvas2, models.DomainTypeCanvas)
		require.NoError(t, err)

		// Get accessible canvases
		canvases, err := r.AuthService.GetAccessibleCanvasesForUser(userID)
		require.NoError(t, err)
...
func Test__AuthService_RoleHierarchy(t *testing.T) {
	...
	t.Run("org owner inherits admin and viewer permissions", func(t *testing.T) {
		...
		err := r.AuthService.AssignRole(ownerID, models.RoleOrgOwner, orgID, models.DomainTypeOrganization)
		require.NoError(t, err)

		roles, err := r.AuthService.GetUserRolesForOrg(ownerID, orgID)
		require.NoError(t, err)
...
func Test__AuthService_PermissionBoundaries(t *testing.T) {
	...
	t.Run("member remove is owner-only permission", func(t *testing.T) {
		...
		err = r.AuthService.AssignRole(adminID, models.RoleCanvasAdmin, canvasID, models.DomainTypeCanvas)
		require.NoError(t, err)
		err = r.AuthService.AssignRole(ownerID, models.RoleCanvasOwner, canvasID, models.DomainTypeCanvas)
		require.NoError(t, err)
		...
		allowed, err = r.AuthService.CheckCanvasPermission(ownerID, canvasID, "member", "remove")
		require.NoError(t, err)
		assert.True(t, allowed)
	})
	...
	t.Run("org update and delete are owner-only permissions", func(t *testing.T) {
		...
		allowed, err = r.AuthService.CheckOrganizationPermission(ownerID, orgID, models.DomainTypeOrganization, "update")
		require.NoError(t, err)
		...
		allowed, err = r.AuthService.CheckOrganizationPermission(ownerID, orgID, models.DomainTypeOrganization, "delete")
		require.NoError(t, err)
...
func Test__AuthService_GetAllRoleDefinitions(t *testing.T) {
	...
	t.Run("domain isolation", func(t *testing.T) {
		...
		err := r.AuthService.SetupOrganizationRoles(anotherOrgID)
		require.NoError(t, err)
		...
		roles2, err := r.AuthService.GetAllRoleDefinitions(models.DomainTypeOrganization, anotherOrgID)
		require.NoError(t, err)
...
func Test__AuthService_DetectMissingPermissions(t *testing.T) {
	...
	t.Run("detect missing permissions after partial setup", func(t *testing.T) {
		...
		missingOrgs, _, err := r.AuthService.DetectMissingPermissions()
		require.NoError(t, err)
...
func Test__AuthService_CheckAndSyncMissingPermissions(t *testing.T) {
	...
	t.Run("check and sync in one operation", func(t *testing.T) {
		...
		err := r.AuthService.CheckAndSyncMissingPermissions()
		require.NoError(t, err)
		...
		allowed, err := r.AuthService.CheckOrganizationPermission(userID, orgID, "canvas", "create")
		require.NoError(t, err)
		assert.True(t, allowed)
	})
...
func Test__AuthService_SyncOrganizationRoles(t *testing.T) {
	...
	t.Run("sync organization roles creates expected policies", func(t *testing.T) {
		// Sync org roles
		err := r.AuthService.SyncOrganizationRoles(orgID)
		require.NoError(t, err)
		...
		for _, role := range expectedRoles {
			roleDef, err := r.AuthService.GetRoleDefinition(role, models.DomainTypeOrganization, orgID)
			require.NoError(t, err)
...
func Test__AuthService_SyncCanvasRoles(t *testing.T) {
	...
	t.Run("sync canvas roles creates expected policies", func(t *testing.T) {
		// Sync canvas roles
		err := r.AuthService.SyncCanvasRoles(canvasID)
		require.NoError(t, err)

		// Test that all expected roles exist
		expectedRoles := []string{models.RoleCanvasViewer, models.RoleCanvasAdmin, models.RoleCanvasOwner}
		for _, role := range expectedRoles {
			roleDef, err := r.AuthService.GetRoleDefinition(role, models.DomainTypeCanvas, canvasID)
			require.NoError(t, err)
			assert.Equal(t, role, roleDef.Name)
			assert.NotEmpty(t, roleDef.Permissions)
		}
...
func Test__AuthService_PermissionSync_Integration(t *testing.T) {
	...
	t.Run("manual sync sets up permissions correctly", func(t *testing.T) {
		...
		orgRoles, err := r.AuthService.GetAllRoleDefinitions(models.DomainTypeOrganization, orgID)
		require.NoError(t, err)
	...
	t.Run("permissions work end-to-end after startup sync", func(t *testing.T) {
		...
		err = r.AuthService.AssignRole(userID, models.RoleCanvasViewer, canvasID, models.DomainTypeCanvas)
		require.NoError(t, err)
		...
		allowed, err = r.AuthService.CheckCanvasPermission(userID, canvasID, "stage", "create")
		require.NoError(t, err)
		assert.False(t, allowed)
	})
}
...
func Test__AuthService_MissingPermissions(t *testing.T) {
	...
	t.Run("only syncs entities with missing permissions", func(t *testing.T) {
		...
		orgsMissingBefore, err := r.AuthService.GetOrganizationsWithMissingPermissions()
		require.NoError(t, err)
		canvasesMissingBefore, err := r.AuthService.GetCanvasesWithMissingPermissions()
		require.NoError(t, err)
		...
		orgsMissingAfter, err := r.AuthService.GetOrganizationsWithMissingPermissions()
		require.NoError(t, err)
		canvasesMissingAfter, err := r.AuthService.GetCanvasesWithMissingPermissions()
		require.NoError(t, err)
