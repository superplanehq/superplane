...
func (a *AuthService) GetCanvasUsersForRole(role string, canvasID string) ([]string, error) {
	return a.GetCanvasUsersForRoleWithOrgContext(role, canvasID, "")
}

func (a *AuthService) GetCanvasUsersForRoleWithOrgContext(role string, canvasID string, orgID string) ([]string, error) {
	prefixedRole := prefixRoleName(role)

	var canvasDomain string
	if orgID == "" {
		canvasDomain = prefixDomain(models.DomainTypeCanvas, canvasID)
	} else {
		canvasDomain = prefixDomainWithOrgContext(models.DomainTypeCanvas, canvasID, orgID)
	}
...
func (a *AuthService) GetAccessibleCanvasesForUser(userID string) ([]string, error) {
	...
	for _, canvas := range canvases {
		if strings.HasPrefix(canvas[2], "canvas:") && !strings.HasPrefix(canvas[2], "canvas:*") {
...
func (a *AuthService) GetUserRolesForOrg(userID string, orgID string) ([]*RoleDefinition, error) {
	...
	return a.getUserRolesForRawDomain(userID, models.DomainTypeOrganization, orgDomain, "")
}

func (a *AuthService) GetUserRolesForCanvasWithOrgContext(userID, canvasID, orgID string) ([]*RoleDefinition, error) {
	domain := prefixDomainWithOrgContext(models.DomainTypeCanvas, canvasID, orgID)
	return a.getUserRolesForRawDomain(userID, models.DomainTypeCanvas, domain, orgID)
}

func (a *AuthService) GetUserRolesForCanvas(userID string, canvasID string) ([]*RoleDefinition, error) {
	canvasDomain := prefixDomain(models.DomainTypeCanvas, canvasID)
	return a.getUserRolesForRawDomain(userID, models.DomainTypeCanvas, canvasDomain, "")
}

func (a *AuthService) getUserRolesForRawDomain(userID string, domainType string, domain string, orgID string) ([]*RoleDefinition, error) {
	prefixedUserID := prefixUserID(userID)
	roleNames, err := a.enforcer.GetImplicitRolesForUser(prefixedUserID, domain)
	if err != nil {
		return nil, err
	}
	...
	for _, roleName := range unprefixedRoleNames {
		roleDef, err := a.getRoleDefinitionForDomain(roleName, domainType, domain, orgID)
		if err != nil {
			continue
		}
...
func (a *AuthService) SetupGlobalCanvasRoles(orgID string) error {
	...
	err := a.enforcer.GetAdapter().(*gormadapter.Adapter).Transaction(a.enforcer, func(e casbin.IEnforcer) error {
		return database.Conn().Transaction(func(tx *gorm.DB) error {
			for _, policy := range a.canvasPolicyTemplates {
				switch policy[0] {
				case "g":
					// g,lower_role,higher_role,canvas:*|org:{ORG_ID}
					_, err := e.AddGroupingPolicy(policy[1], policy[2], domain)
					if err != nil {
						return fmt.Errorf("failed to add grouping policy: %w", err)
					}
				case "p":
					// p,role,canvas:*|org:{ORG_ID},resource,action
					_, err := e.AddPolicy(policy[1], domain, policy[3], policy[4])
					if err != nil {
						return fmt.Errorf("failed to add policy: %w", err)
					}
			...
			if err := a.setupDefaultGlobalCanvasRoleMetadataInTransaction(tx, orgID); err != nil {
				log.Errorf("Error setting up default global canvas role metadata: %v", err)
				return err
			}
	...
	if err != nil {
		return fmt.Errorf("failed to setup global canvas roles: %w", err)
	}
...
func (a *AuthService) DestroyGlobalCanvasRoles(orgID string) error {
	...
	ok, err := a.enforcer.RemoveFilteredGroupingPolicy(2, domain)
	if err != nil {
		return fmt.Errorf("failed to remove global canvas roles: %w", err)
	}
	if !ok {
		return fmt.Errorf("global canvas roles not found for org %s", orgID)
	}

	ok, err = a.enforcer.RemoveFilteredPolicy(1, domain)
	if err != nil {
		return fmt.Errorf("failed to remove global canvas policies: %w", err)
	}
	if !ok {
		return fmt.Errorf("global canvas policies not found for org %s", orgID)
	}
...
func (a *AuthService) GetGlobalCanvasRoleDefinition(roleName string, orgID string) (*RoleDefinition, error) {
	domain := prefixDomainWithOrgContext(models.DomainTypeCanvas, "*", orgID)
	return a.getRoleDefinitionForDomain(roleName, "canvas", domain, orgID)
}

func (a *AuthService) getRoleDefinitionForDomain(roleName string, domainType string, domain string, orgID string) (*RoleDefinition, error) {
	...
	if !a.IsDefaultRole(roleName, domainType) {
		var originalDomainID string
		globalDomain := fmt.Sprintf("%s:*", domainType)
		if orgID != "" && strings.HasPrefix(domain, globalDomain) {
			originalDomainID = "*"
		} else {
			originalDomainID = strings.TrimPrefix(domain, fmt.Sprintf("%s:", domainType))
		}
		...
		if orgID != "" {
			metadata, err = models.FindRoleMetadataWithOrgContext(roleName, domainType, originalDomainID, orgID)
		} else {
			metadata, err = models.FindRoleMetadata(roleName, domainType, originalDomainID)
		}
		if err == nil {
	...
	roleDefinition := &RoleDefinition{
		...
		Readonly:    a.IsDefaultRole(roleName, domainType),
	}
...
func (a *AuthService) GetRoleDefinition(roleName string, domainType string, domainID string) (*RoleDefinition, error) {
	...
	domain := prefixDomain(domainType, domainID)
	return a.getRoleDefinitionForDomain(roleName, domainType, domain, "")
}
...
func (a *AuthService) GetAllRoleDefinitionsWithOrgContext(domainType string, domainID string, orgID string) ([]*RoleDefinition, error) {
	domain := prefixDomainWithOrgContext(domainType, domainID, orgID)
	return a.getAllRoleDefinitionsWithRawDomain(domainType, domain, orgID)
}
...
func (a *AuthService) getAllRoleDefinitionsWithRawDomain(domainType string, domain string, orgID string) ([]*RoleDefinition, error) {
...
func (a *AuthService) SyncDefaultRoles(orgIDs []string, canvasIDs []string) error {
	...
	if err := a.syncGlobalCanvasDefaultRoles(orgIDs); err != nil {
...
func (a *AuthService) getOrganizationsWithMissingPermissions() ([]string, error) {
	orgIDs, err := models.GetActiveOrganizationIDs()
	if err != nil {
	...
	for _, orgID := range orgIDs {
		...
		if err != nil {
			log.Warnf("Error checking global canvas permissions for org %s: %v", orgID, err)
			continue
		}
		if hasMissing {
			missingOrgIDs = append(missingOrgIDs, orgID)
...
func (a *AuthService) syncOrganizationDefaultRoles(orgIDs []string) error {
	if len(orgIDs) == 0 {
...
func (a *AuthService) syncGlobalCanvasDefaultRoles(orgIDs []string) error {
	if len(orgIDs) == 0 {
		log.Debug("No organizations with missing permissions found")
		return nil
	}
	...
	for _, orgID := range orgIDs {
		if err := a.SyncGlobalCanvasRoles(orgID); err != nil {
			log.Errorf("Failed to sync default global canvas roles for org %s: %v", orgID, err)
			continue
		}
		log.Infof("Synced default global canvas roles for org %s", orgID)
	}
...
func (a *AuthService) CreateCustomRole(domainID string, roleDefinition *RoleDefinition) error {
	return a.CreateCustomRoleWithOrgContext(domainID, "", roleDefinition)
}

func (a *AuthService) CreateCustomRoleWithOrgContext(domainID string, orgID string, roleDefinition *RoleDefinition) error {
	...
	err := a.createCustomRoleWithNestedTransaction(domainID, orgID, roleDefinition)
...
func (a *AuthService) createCustomRoleWithNestedTransaction(domainID string, orgID string, roleDefinition *RoleDefinition) error {
	...
	return a.enforcer.GetAdapter().(*gormadapter.Adapter).Transaction(a.enforcer, func(e casbin.IEnforcer) error {
		return database.Conn().Transaction(func(tx *gorm.DB) error {
			var err error
			if orgID != "" {
				err = models.UpsertRoleMetadataWithOrgContextInTransaction(tx, roleDefinition.Name, roleDefinition.DomainType, domainID, roleDefinition.DisplayName, roleDefinition.Description, orgID)
			} else {
...
func (a *AuthService) UpdateCustomRole(domainID string, roleDefinition *RoleDefinition) error {
	return a.UpdateCustomRoleWithOrgContext(domainID, "", roleDefinition)
}

func (a *AuthService) UpdateCustomRoleWithOrgContext(domainID string, orgID string, roleDefinition *RoleDefinition) error {
	if a.IsDefaultRole(roleDefinition.Name, roleDefinition.DomainType) {
		return fmt.Errorf("cannot update default role: %s", roleDefinition.Name)
	}

	err := a.updateCustomRoleWithNestedTransaction(domainID, orgID, roleDefinition)
...
func (a *AuthService) DeleteCustomRole(domainID string, domainType string, roleName string) error {
	return a.DeleteCustomRoleWithOrgContext(domainID, "", domainType, roleName)
}

func (a *AuthService) DeleteCustomRoleWithOrgContext(domainID string, orgID string, domainType string, roleName string) error {
	if a.IsDefaultRole(roleName, domainType) {
		return fmt.Errorf("cannot delete default role: %s", roleName)
	}

	err := a.deleteCustomRoleWithNestedTransaction(domainID, orgID, domainType, roleName)
...
func (a *AuthService) deleteCustomRoleWithNestedTransaction(domainID string, orgID string, domainType string, roleName string) error {
	...
	return a.enforcer.GetAdapter().(*gormadapter.Adapter).Transaction(a.enforcer, func(e casbin.IEnforcer) error {
		return database.Conn().Transaction(func(tx *gorm.DB) error {
			var err error
			if orgID != "" {
				err = models.DeleteRoleMetadataWithOrgContextInTransaction(tx, roleName, domainType, domainID, orgID)
			} else {
...
func (a *AuthService) setupDefaultGlobalCanvasRoleMetadataInTransaction(tx *gorm.DB, orgID string) error {
	defaultRoles := []struct {
		name        string
		displayName string
		description string
	...
	for _, role := range defaultRoles {
		if err := models.UpsertRoleMetadataWithOrgContextInTransaction(tx, role.name, models.DomainTypeCanvas, "*", role.displayName, role.description, orgID); err != nil {
			return fmt.Errorf("failed to upsert global canvas role metadata for %s: %w", role.name, err)
		}
	}
