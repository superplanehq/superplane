...
func TestValidateExpression_Valid(t *testing.T) {
	runExprCases(t, "valid", []exprCase{
		...
		{name: "app call", raw: `app().url`},
		{name: "app name", raw: `app().name`},
		{name: "app description", raw: `app().description`},
		{name: "order call", raw: `order()`},
		{name: "order id", raw: `order().id`},
		{name: "order artifacts", raw: `order().artifacts`},
		{name: "order none pr", raw: `none(order().artifacts, {#.type == "pr"})`},
		{name: "task call", raw: `task()`},
		{name: "task id", raw: `task().id`},
		{name: "task artifacts", raw: `task().artifacts`},
...
func TestValidateExpression_BadArity(t *testing.T) {
	runExprCases(t, "bad_arity", []exprCase{
		...
		{name: "app with int", raw: `app(1)`, wantErr: "app() takes no arguments"},
		{name: "app with string", raw: `app('x')`, wantErr: "app() takes no arguments"},
		{name: "order with int", raw: `order(1)`, wantErr: "order() takes no arguments"},
		{name: "order with string", raw: `order('x')`, wantErr: "order() takes no arguments"},
		{name: "task with int", raw: `task(1)`, wantErr: "task() takes no arguments"},
		{name: "task with string", raw: `task('x')`, wantErr: "task() takes no arguments"},
