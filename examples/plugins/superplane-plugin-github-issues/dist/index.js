...
async function getInstallationToken(http, appId, installationId, pem) {
    const jwt = createJWT(appId, pem);
    const resp = await http.request("POST", `https://api.github.com/app/installations/${installationId}/access_tokens`, {
        headers: {
    ...
    if (resp.status !== 201) {
        throw new Error(`Failed to get installation token: ${resp.status} ${resp.body?.message}`);
    }
    return resp.body.token;
}
function findSecret(secrets, name) {
...
function activate(context) {
    ...
    context.integrations.register("github-issues", {
        async sync(ctx) {
            const metadata = (await ctx.integration.getMetadata()) || {};
        ...
        webhookHandler: {
            async setup(ctx) {
                ...
                const token = await getInstallationToken(ctx.http, metadata.githubApp.id, metadata.installationId, pem);
                const config = ctx.configuration;
                const resp = await ctx.http.request("POST", `https://api.github.com/repos/${metadata.owner}/${config.repository}/hooks`, {
                    headers: {
                ...
                if (resp.status !== 201) {
                    throw new Error(`Failed to create webhook: ${resp.status} ${resp.body?.message}`);
                }
            ...
            async cleanup(ctx) {
                ...
                const token = await getInstallationToken(ctx.http, metadata.githubApp.id, metadata.installationId, pem);
                const config = ctx.configuration;
                const webhookMeta = ctx.webhookMetadata;
                if (!webhookMeta?.id)
                    return;
                await ctx.http.request("DELETE", `https://api.github.com/repos/${metadata.owner}/${config.repository}/hooks/${webhookMeta.id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        Accept: "application/vnd.github+json",
                        "X-GitHub-Api-Version": "2022-11-28",
    ...
    context.components.register("github-issues.create-issue", {
        ...
        async execute(ctx) {
            ...
            const token = await getInstallationToken(ctx.http, metadata.githubApp.id, metadata.installationId, pem);
            const config = ctx.configuration;
            const resp = await ctx.http.request("POST", `https://api.github.com/repos/${metadata.owner}/${config.repository}/issues`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/vnd.github+json",
                    "Content-Type": "application/json",
            ...
            if (resp.status !== 201) {
                ctx.fail("api_error", `GitHub API returned ${resp.status}: ${resp.body?.message}`);
                return;
            }
...
async function handleAfterAppCreation(ctx, metadata) {
    ...
    const resp = await ctx.http.request("POST", `https://api.github.com/app-manifests/${code}/conversions`, {
        headers: {
            Accept: "application/vnd.github+json",
            "X-GitHub-Api-Version": "2022-11-28",
        },
...
async function handleAfterAppInstallation(ctx, metadata) {
    const integrationId = await ctx.integration.id();
    if (metadata.installationId) {
        return {
            action: "redirect",
    ...
    const secrets = await ctx.integration.getSecrets();
    const pem = findSecret(secrets, PEM_SECRET);
    const token = await getInstallationToken(ctx.http, metadata.githubApp.id, installationId, pem);
    if (!metadata.owner) {
        const appResp = await ctx.http.request("GET", `https://api.github.com/apps/${metadata.githubApp.slug}`, {
            headers: {
    ...
    const reposResp = await ctx.http.request("GET", "https://api.github.com/installation/repositories", {
        headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/vnd.github+json",
            "X-GitHub-Api-Version": "2022-11-28",
...
async function handleIntegrationWebhook(ctx, metadata) {
    const secrets = await ctx.integration.getSecrets();
    let webhookSecret;
    try {
