 ...
 */
export const StageCard: React.FC<NodeProps> = ({
  data,
  selected,
  id,
}) => {
  const nodeData = data as WorkflowData;
  ...
  const nodeStyle = useMemo(
    () => ({
      width: nodeData.style?.width || 320,
  ...
  const statusConfig = getStatusConfig(nodeData.status || 'pending');
  ...
  return (
    <div 
      ...
      style={nodeStyle}
      role="article"
      aria-label={`Deployment stage: ${nodeData.label}`}
    >
  ...
  - name: "${nodeData.label}"
    task:
      ...
      <div className="p-4 flex justify-between items-center border-b border-gray-200">
        ...
        {nodeData.hasHealthCheck && (
          <div className="flex items-center">
            <Tippy 
              content={`Health check: ${nodeData.healthCheckStatus}. Last check run 2 hours ago`}
              placement="top"
            >
              <div
                className={`w-3 h-3 rounded-full border-2 ${
                  nodeData.healthCheckStatus === 'healthy'
                    ? 'bg-success-500 border-success-200'
                    : nodeData.healthCheckStatus === 'unhealthy'
                    ? 'bg-error-500 border-error-200'
                    : 'bg-warning-500 border-warning-200'
                }`}
                role="status"
                aria-label={`Health status: ${nodeData.healthCheckStatus}`}
              />
            </Tippy>
          </div>
        )}
