...
import {
  ...
  nameSpaceClassNames,
  withReset
} from "./chunk-ZMBA3GRA.js";
import {
  ActionBar
} from "./chunk-GG44W4IB.js";
import {
  ThemeProvider,
  convert,
  ensure,
  ...
  themes,
  useTheme
} from "./chunk-EVA35BWT.js";
import {
  index_modern_default
} from "./chunk-L2RXNQK2.js";
import {
  SNIPPET_RENDERED,
  SourceType,
  str
} from "./chunk-WUT6YCCQ.js";
import {
  P
...
import {
  require_pickBy
} from "./chunk-MDBXHKYF.js";
import {
  require_uniq
} from "./chunk-P4T442HV.js";
import {
  require_mapValues
} from "./chunk-F6HY5NS3.js";
import {
  require_cloneDeep
} from "./chunk-CFGZHOQC.js";
import {
  renderElement,
  unmountElement
} from "./chunk-L536MYPK.js";
import {
...
var Preview = ({ isLoading, isColumn, columns, children, withSource, withToolbar = false, isExpanded = false, additionalActions, className, layout = "padded", ...props }) => {
  let [expanded, setExpanded] = (0, import_react.useState)(isExpanded), { source, actionItem } = getSource(withSource, expanded, setExpanded), [scale, setScale] = (0, import_react.useState)(1), previewClasses = [className].concat(["sbdocs", "sbdocs-preview", "sb-unstyled"]), defaultActionItems = withSource ? [actionItem] : [], [additionalActionItems, setAdditionalActionItems] = (0, import_react.useState)(additionalActions ? [...additionalActions] : []), actionItems = [...defaultActionItems, ...additionalActionItems], { window: globalWindow4 } = import_global.global, copyToClipboard = (0, import_react.useCallback)(async (text) => {
    let { createCopyToClipboardFunction } = await import("./dist-EXF4N4H6.js");
...
var CheckboxControl = ({ name, options, value: value2, onChange, isInline }) => {
  ...
  return import_react.default.createElement(Wrapper3, { isInline }, Object.keys(options).map((key, index) => {
    ...
    return import_react.default.createElement(Label2, { key: id, htmlFor: id }, import_react.default.createElement("input", { type: "checkbox", id, name: id, value: key, onChange: handleChange, checked: selected?.includes(key) }), import_react.default.createElement(Text2, null, key));
  }));
};
...
var TextControl = ({ name, value: value2, onChange, onFocus, onBlur, maxLength }) => {
  ...
  return import_react.default.createElement(Wrapper6, null, import_react.default.createElement(Form.Textarea, { id: getControlId(name), maxLength, onChange: handleChange, size: "flex", placeholder: "Edit string...", autoFocus: forceVisible, valid: isValid ? null : "error", name, value: isValid ? value2 : "", onFocus, onBlur }), maxLength && import_react.default.createElement(MaxLength, { isMaxed: value2?.length === maxLength }, value2?.length ?? 0, " / ", maxLength));
};
...
var LazyColorControl = (0, import_react.lazy)(() => import("./Color-6VNJS4EI-DIYZIN5B.js"));
...
var ArgRow = (props) => {
  let [isHovered, setIsHovered] = (0, import_react.useState)(false), { row, updateArgs, compact, expandable, initialExpandedArgs } = props, { name, description } = row, table = row.table || {}, type = table.type || row.type, defaultValue = table.defaultValue || row.defaultValue, required = row.type?.required, hasDescription = description != null && description !== "";
...
var SectionRow = ({ level = "section", label, children, initialExpanded = true, colSpan = 3 }) => {
  let [expanded, setExpanded] = (0, import_react.useState)(initialExpanded), Level = level === "subsection" ? Subsection : Section, itemCount = children?.length || 0, caption = level === "subsection" ? `${itemCount} item${itemCount !== 1 ? "s" : ""}` : "", icon = expanded ? "arrowdown" : "arrowright", helperText = `${expanded ? "Hide" : "Show"} ${level === "subsection" ? itemCount : label} item${itemCount !== 1 ? "s" : ""}`;
...
var sortFns = { alpha: (a, b) => a.name.localeCompare(b.name), requiredFirst: (a, b) => +!!b.type?.required - +!!a.type?.required || a.name.localeCompare(b.name), none: void 0 };
...
var ArgTypes = (props) => {
  ...
  let resolved = useOf(of || "meta"), { argTypes, parameters } = getArgTypesFromResolved(resolved), argTypesParameters = parameters.docs?.argTypes || {}, include = props.include ?? argTypesParameters.include, exclude = props.exclude ?? argTypesParameters.exclude, sort = props.sort ?? argTypesParameters.sort, filteredArgTypes = (0, import_preview_api.filterArgTypes)(argTypes, include, exclude);
...
var StoryTable = (props) => {
  ...
  try {
    ...
    let argTypes = (0, import_preview_api.filterArgTypes)(story.argTypes, include, exclude), mainLabel = getComponentName(component) || "Story", tabs = { [mainLabel]: { rows: argTypes, args, globals, updateArgs, resetArgs } }, storyHasArgsWithControls = argTypes && Object.values(argTypes).find((v) => !!v?.control);
...
var ArgsTable2 = (props) => {
  ...
  let { include, exclude, components: components22, sort: sortProp } = props, { story: storyName } = props, sort = sortProp || parameters.controls?.sort, main = getComponent(props, component);
...
var getSnippet = ({ snippet, storyContext, typeFromProps, transformFromProps }) => {
  let { __isArgsStory: isArgsStory } = storyContext.parameters, sourceParameters = storyContext.parameters.docs?.source || {}, type = typeFromProps || sourceParameters.type || SourceType.AUTO;
  ...
  `), storyContext.parameters.docs?.transformSource && (0, import_client_logger.deprecate)(esm_default`The \`transformSource\` parameter at \`parameters.docs.transformSource\` is deprecated, please use \`parameters.docs.source.transform\` instead. 
    
    Please refer to the migration guide: https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#source-block
  `), storyContext.parameters.jsx?.transformSource && (0, import_client_logger.deprecate)(esm_default`The \`transformSource\` parameter at \`parameters.jsx.transformSource\` is deprecated, please use \`parameters.docs.source.transform\` instead. 
...
var useSourceProps = (props, docsContext, sourceContext) => {
  ...
  let sourceParameters = stories[0]?.parameters?.docs?.source || {}, { code } = props, format2 = props.format ?? sourceParameters.format, language = props.language ?? sourceParameters.language ?? "jsx", dark = props.dark ?? sourceParameters.dark ?? false;
  code || (code = stories.map((story, index) => {
    ...
    return index === 0 && (format2 = source.format ?? story.parameters.docs?.source?.format ?? false), getSnippet({ snippet: source.code, storyContext: { ...storyContext, args: argsForSource }, typeFromProps: props.type, transformFromProps: props.transform });
  }).join(`
...
var Canvas = (props) => {
  ...
  let layout = props.layout ?? story.parameters.layout ?? story.parameters.docs?.canvas?.layout ?? "padded", withToolbar = props.withToolbar ?? story.parameters.docs?.canvas?.withToolbar ?? false, additionalActions = props.additionalActions ?? story.parameters.docs?.canvas?.additionalActions, sourceState = props.sourceState ?? story.parameters.docs?.canvas?.sourceState ?? "hidden", className = props.className ?? story.parameters.docs?.canvas?.className;
...
var getDescriptionFromDeprecatedProps = ({ type, markdown, children }, { storyById }) => {
  ...
  let { extractComponentDescription = noDescription, description } = docs || {}, componentDescriptionParameter = description?.component;
...
var Subtitle2 = ({ children }) => {
  let docsContext = (0, import_react.useContext)(DocsContext), content = children || docsContext.storyById().parameters?.componentSubtitle;
...
var Stories = ({ title = "Stories", includePrimary = true }) => {
  let { componentStories } = (0, import_react.useContext)(DocsContext), stories = componentStories().filter((story) => !story.parameters?.docs?.disable);
...
//# sourceMappingURL=chunk-3MHBWAMM.js.map
