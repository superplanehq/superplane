...
function applyJsDocResult(propDef, jsDocParsingResult) {
  if (jsDocParsingResult.includesJsDoc) {
    ...
    let value = { ...extractedTags, params: extractedTags?.params?.map((x) => ({ name: x.getPrettyName(), description: x.description })) };
...
var extractComponentSectionArray = (docgenSection) => {
  ...
  return docgenSection.map((item) => {
    ...
    return item.type?.elements && (sanitizedItem = { ...item, type: { ...item.type, value: item.type.elements } }), extractProp(sanitizedItem.name, sanitizedItem, typeSystem, createPropDef);
  });
};
...
//# sourceMappingURL=chunk-WUT6YCCQ.js.map
