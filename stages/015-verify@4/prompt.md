...
- **verify**: succeeded
  - Model: claude-sonnet-4-6, 13.3k tokens in / 1.6k out
- **fix**: succeeded
...
- **wait_checks**: failed
  ...
  - Output:
    ...
    ```
- **fix_ci**: succeeded
  - Model: claude-sonnet-4-6, 10.6k tokens in / 2.5k out
- **verify**: succeeded
  - Model: claude-sonnet-4-6, 13.3k tokens in / 1.6k out
- **ensure_pr**: failed
  - Script: `@scripts/ensure-pr.sh`
...
- verify.summary: format.js passed (all files unchanged); vitest ran canvasAppPreferencePresentation.spec.ts (1 test) and index.spec.tsx (23 tests) — all 24 tests passed
