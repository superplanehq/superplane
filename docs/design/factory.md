...
| `updatePullRequestActivity` | optional `description`, `access` | Updates the current activity. Can request exclusive access. Emits `limitReached` and records a run error when the check-handler attempt limit applies. |
