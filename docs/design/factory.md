...
- **Artifacts** are first-class rows in `factory_work_order_artifacts`. They can be created by the `addWorkOrderArtifact` canvas component (automation authorship), by `POST …/artifacts` (interactive user authorship), or by the CLI (`superplane factory artifacts add`). Each artifact has a required `type` (`markdown`, `branch`, or `link`) plus a JSONB `data` map that carries everything else — the on-wire shape is `{ id, type, data, createdBy, createdAt }`, and `url`, `title`, `body`, `name`, and any free-form extras all live inside `data`. `markdown` requires `data.body`; `branch` requires `data.name`; `link` requires `data.url` (with an optional `data.title` used as the chip's label, e.g. a preview-environment URL). `data.url` is optional on any other type, but whenever it is present — regardless of type — the model enforces that it is an absolute `http(s)` URL with a host and rejects `javascript:`, `data:`, `file:`, `mailto:`, and protocol-relative URLs, so no caller can smuggle a dangerous scheme into a link teammates will click. The client mirrors this check with `lib/safeExternalUrl` before rendering `href`s. Creation is transactional with an `order.artifact.added` event that includes the artifact `data` so the timeline can render markdown inline without a second fetch. The Work Order detail sidebar lists artifacts read-only (no attach composer in the UI yet).
- Artifacts optionally carry a `key` (`VARCHAR(512)`, nullable, unique per factory via a partial index that excludes `NULL`) so a work order can be looked up from an external identifier without already knowing the order id. `addWorkOrderArtifact`'s `artifactKey` field sets it; `findWorkOrder` (`by: artifactKey`) reads it. Setting it is currently only possible from the canvas component — the artifact-key field is not yet exposed on the REST API or CLI, so artifacts created that way can't be tagged with a key (known gap).
- **Pull requests** are first-class rows in `factory_pull_requests`, owned by a factory and a work order. Create them with `addPullRequest` or `POST …/prs`. Update state with `updatePullRequest`. Look them up with `findPullRequest`. Link the current canvas run with `addPullRequestActivity` (idempotent). List them with `GET …/prs?order=<work-order-number>`. Timeline events are `order.pull_request.added` and `order.pull_request.updated`. Chips render GitHub-style icons and colors from `state` (`open` green, `draft` gray, `closed` red, `merged` purple) and stay live through the `work_order_updated` websocket notification.
- **Revisions** record each observed pull request head SHA. A newer head updates the current revision. It does not cancel a run that is still bound to an older SHA. Discussion activities omit a revision and stay pull-request-scoped.
- **Activity access** is exclusive or concurrent. Only one activity can hold exclusive access and change a pull request. Concurrent activities can wait for checks without a lock. Waiting activities do not consume automatic-fix attempts.
- **Check handlers** wait for GitHub Checks and Commit Statuses on one revision, then start one agent run when selected checks fail. Additional runner integrations stay in the generated canvas `environmentFrom` list. They do not copy secrets into handler settings.
...
- Pull requests: list (`GET …/prs?order=<work-order-number>`), describe, create, update.
...
| `addWorkOrderArtifact` | required `orderId` (defaults to `{{ order().id }}`), `artifactType` (`markdown`/`branch`/`link`); for `markdown`: required `body`; for `branch`: required `name`, optional `url`; for `link`: required `url`; optional `title` on `markdown`/`link`; optional `artifactKey`; free-form `data` (`{name, value}` list, merged into the artifact's `data` map — typed fields win on name collisions) | Creates the artifact row + `order.artifact.added` event. |
| `addPullRequest` | required `orderId` (defaults to `{{ order().id }}`), `repository`, `number`, `url`; optional `provider` (defaults to `github`), `title`, `state` (`open`/`draft`/`closed`/`merged`), `merged`, `draft`, `mergedAt`, `closedAt` | Creates the pull request row + `order.pull_request.added` event. |
| `updatePullRequest` | required `pullRequestId`; optional `repository`, `url`, `title`, `state`, `merged`, `draft`, `mergedAt`, `closedAt` | Updates the pull request row + `order.pull_request.updated` event. |
| `findPullRequest` | identity by URL, or provider + external id, or provider + repository + number | Emits `pullRequest.found` or `pullRequest.notFound`. Does not create a row or link a run. |
| `addPullRequestActivity` | required `pullRequestId`, optional `description`, `revision`, `access` | Creates one activity for the current run. A revision binds the activity to one head SHA. Exclusive access is required before a run changes the pull request. |
| `updatePullRequestActivity` | optional `description`, `access` | Updates the current activity. Can request exclusive access. An exclusive request that hits the attempt limit emits `limitReached` and records a run error. A description-only update after the limit emits `default`. |

`updateWorkOrderStatus` / `addWorkOrderComment` / `addWorkOrderArtifact` / `addPullRequest` always target a work order explicitly via `orderId` — there is no implicit fallback. The field defaults to `{{ order().id }}`, which resolves the work order driving the current canvas run (via the `factory_work_order_executions` row created when the run was dispatched from a factory line) and only works in that context. Runs not dispatched from a line, e.g. a flow triggered by `github.onPullRequest`, must replace the default with an id resolved another way — typically `{{ previous().data.workOrder.id }}` after a `findWorkOrder` or `findPullRequest` step. Canvas invocations attribute events to the caller line: the `automation` payload snapshots `{ nodeId, nodeName, appId, appName, lineId, lineName, stepIndex, stepName }` at write time (line/step are omitted when the run isn't attached to one), and status updates additionally carry the current `run` + `app` refs so the timeline can link straight back to the originating run. No acting user is attributed on canvas-driven events.

### Closing a work order from an external merge event

`findPullRequest` plus the work-order id on its output let a flow that starts on `github.onPullRequest` (not a factory line) close the work order a merged pull request belongs to:

```yaml
github.onPullRequest (merged)
  -> if ($.pull_request.merged == true)
  -> findPullRequest (url: {{ $.pull_request.html_url }})
       [found]    -> updateWorkOrderStatus (orderId: {{ previous().data.workOrder.id }}, status: closed, result: completed)
       [notFound] -> (no-op; the merged PR is not tied to a tracked order)
```

This assumes an earlier `addPullRequest` step (e.g. when the PR was opened) so `findPullRequest` has a row to match. Work-order lookup by artifact key remains available for generic artifacts.

### Keeping a pull request's state live

`findPullRequest` plus `updatePullRequest` keep the chip's icon/color in sync with GitHub as the pull request moves through its lifecycle:

```yaml
github.onPullRequest (opened | ready_for_review | converted_to_draft | closed)
  -> findPullRequest (url: {{ $.pull_request.html_url }})
       [found]    -> updatePullRequest (
                       pullRequestId: {{ previous().data.pullRequest.id }},
                       state: <derived from the action, see below>,
                     )
       [notFound] -> (no-op; the PR is not tracked)
```
...
- **Work order detail** — status-aware overflow menu (`Draft`: Reject; `Open`: Complete / Reject; `Closed`: Reopen). Dispatch stays on the sidebar. Inline **comment composer**, activity timeline (comments, status transitions, artifacts, pull requests, dispatches) with line-centric automation attribution, owners panel, and a read-only **Artifacts** sidebar. Pull request chips (sidebar and the timeline's "attached" line) render GitHub-style icons/colors by `state` (open green, draft gray, closed red, merged purple) and stay live: both surfaces re-query on the `work_order_updated` websocket notification, so a chip updates in place when `updatePullRequest` changes the underlying state.
...
superplane factory artifacts add --order-id <uuid> --type <markdown|branch|link> [flags]
```
...
`--type` is `markdown`, `branch`, or `link`. Typed flags build the API `data` map:
