...
| `updateWorkOrderStatus` | optional `orderId`, `status` (`draft`/`open`/`closed`), conditional `result` (`completed`/`rejected`/`failed`, required for `closed`; only `rejected` is valid when closing from `draft`) | Runs the FSM and records an enriched `order.status.updated`. |
| `addWorkOrderComment` | optional `orderId`, `body` | Appends an `order.comment.added` event. Authorship is derived from the executing canvas node (`kind = automation`, `automation = { nodeName, appName, lineName, stepName }`). |
| `addWorkOrderArtifact` | optional `orderId`, `artifactType` (`pr`/`markdown`/`branch`); for `pr`: required `url`, optional `number`; for `markdown`: required `body`; for `branch`: required `name`; optional `title` on `pr`/`markdown`; optional `artifactKey`; free-form `data` (`{name, value}` list, merged into the artifact's `data` map — typed fields win on name collisions) | Creates the artifact row + `order.artifact.added` event. |

By default, `updateWorkOrderStatus` / `addWorkOrderComment` / `addWorkOrderArtifact` target the work order that owns the current canvas run — the link is the `factory_work_order_executions` row created when the run was dispatched from a factory line. Set their optional `orderId` field to target a work order explicitly instead; this is required for runs not dispatched from a line, e.g. a flow triggered by `github.onPullRequest`. Canvas invocations attribute events to the caller line: the `automation` payload snapshots `{ nodeId, nodeName, appId, appName, lineId, lineName, stepIndex, stepName }` at write time (line/step are omitted when the run isn't attached to one), and status updates additionally carry the current `run` + `app` refs so the timeline can link straight back to the originating run. No acting user is attributed on canvas-driven events.
