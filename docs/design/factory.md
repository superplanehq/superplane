...
| `updatePullRequestActivity` | optional `description`, `access` | Updates the current activity. Can request exclusive access. Emits `limitReached` when the check-handler attempt limit applies. |
