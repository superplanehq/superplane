...
- Edit paths:
  - **Web UI and agent tools** stage canvas/console/files edits into `workflow_staged_files`, then require **Commit** with a message to go live. Agent reads serve the effective staged content and agent `patch_draft` writes go through the same staging layer
  - **CLI** can stage via `PUT /staging` and go live via `POST /staging/commit`, or commit repository files directly via `CommitCanvasRepositoryFiles`
