...
1. Edit a draft version through the UI **stage → Commit** loop (canvas/console/files edits autosave into `workflow_staging`; **Commit** materializes them onto the draft version row) or commit directly via the CLI or agent tools.
