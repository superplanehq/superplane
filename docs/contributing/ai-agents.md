...
When adding backend custom tools for managed agents, see [Agent Tools](agent-tools.md). App-related capabilities should usually be added as `superplane_app` actions rather than new top-level tools.

## Validate AI-assisted changes

AI assistance does not replace code ownership. Before opening a pull request:

- Read the complete diff and remove unrelated changes.
- Run the smallest relevant tests and checks locally.
- Confirm that logs, credentials, generated files, and customer data are not included.
- Explain the behavior change, test coverage, and known limitations in the pull request.

For changes that affect workflows, components, or agent tools, verify the success path and the failure or recovery path. Keep pull requests focused so reviewers can evaluate them quickly.
