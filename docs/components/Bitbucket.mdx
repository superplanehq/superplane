...
To connect Bitbucket with SuperPlane, you'll need to create an access token. Choose one of the following methods::
...
- **CI/CD automation**: Trigger builds and deployments on every push
...
This trigger automatically sets up a Bitbucket webhook when configured. The webhook is managed by SuperPlane and will be automatically cleaned up when the trigger is removed.
