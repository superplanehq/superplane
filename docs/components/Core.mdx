...
Enable **Input is a list** to write one memory row per element of a list expression.

- **List Source**: an expression that evaluates to a list, for example `$["Runner"].data.result.services`.
- **Item Variable**: the variable name used to reference each element in the value expressions (defaults to `item`).

In list mode, each field value in the **Values** list is evaluated once per element with the iteration variable in scope. A single `memory.added` event is emitted with all written rows and a `count` field.

#### Example Configuration

- List Source: `$["Runner"].data.result.services`
- Item Variable: `service`
- Values:
  - `name` → `service.name`
  - `id` → `service.id`
...
The `matchList` is evaluated once and applied identically to every iteration. Each `valueList` field value is evaluated per element with the iteration variable in scope. All resulting updates are reported in a single `memory.updated` event; the channel is `found` when at least one row was updated across all iterations.
...
The `matchList` is evaluated once and applied identically to every iteration. Each `valueList` field value is evaluated per element with the iteration variable in scope. All upserts are reported in a single `memory.upserted` event with a per-item `items` array containing the operation result (`updated` or `created`) and the affected records.

When a non-empty `matchList` is configured in list mode, each iteration attempts to update the same matched rows. List mode is most useful for inserting multiple rows (empty `matchList`) or when each item's values are meant to overwrite the matched rows.
