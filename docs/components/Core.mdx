...
<CardGrid>
  ...
  <LinkCard title="On Task" href="#on-task" description="Start when a task is created in this factory" />
...
<CardGrid>
  ...
  <LinkCard title="Add Pull Request" href="#add-pull-request" description="Attach a pull request to a task" />
  ...
  <LinkCard title="Add Task Artifact" href="#add-task-artifact" description="Attach a typed artifact (markdown note, branch, or link) to a task" />
  <LinkCard title="Add Task Comment" href="#add-task-comment" description="Append a comment to a task timeline" />
  ...
  <LinkCard title="Create Task" href="#create-task" description="Create a new task" />
  ...
  <LinkCard title="Find Task" href="#find-task" description="Look up a task by id or by an artifact's key" />
  ...
  <LinkCard title="Report Task Check" href="#report-task-check" description="Report a scored or pass/fail check (risk, coverage, CI) on a task" />
  ...
  <LinkCard title="Run SuperPlane Agent" href="#run-super-plane-agent" description="Runs a SuperPlane-hosted coding agent on a fleet runner." />
  <LinkCard title="Set Task Status Note" href="#set-task-status-note" description="Announce what a waiting task is blocked on and what resolves it" />
  ...
  <LinkCard title="Update Pull Request" href="#update-pull-request" description="Update a factory pull request that is already attached to a task" />
  ...
  <LinkCard title="Update Task Status" href="#update-task-status" description="Transition a task between lifecycle states" />
  ...
  <LinkCard title="Wait for Endpoint" href="#wait-for-endpoint" description="Poll an HTTP endpoint until it is ready or the deadline expires" />
</CardGrid>
...
The On Task trigger starts a workflow when a task is created in the factory that owns this app.
...
The Add Pull Request component records a pull request on a task.
...
<a id="add-task-artifact"></a>

## Add Task Artifact

**Component key:** `addWorkOrderArtifact`

The Add Task Artifact component stores a typed artifact against a task.
...
<a id="add-task-comment"></a>

## Add Task Comment

**Component key:** `addWorkOrderComment`

The Add Task Comment component appends a comment from this automation to the task's activity timeline. Authorship is captured automatically — the timeline shows which canvas node and app wrote the comment, without any configurable label.
...
The Create Task component creates a new task in the factory. This component can only be used in factory-owned apps.
...
The Find Task component resolves a task without requiring the current run to be attached to one — the missing piece for flows that don't start on a factory line, e.g. a `github.onPullRequest` merged event that needs to close the task a pull request belongs to.
...
<a id="report-task-check"></a>

## Report Task Check

**Component key:** `reportWorkOrderCheck`

The Report Task Check component stores a scored check against a task. Checks appear as scorecards on the task page, next to the description.
...
<a id="run-super-plane-agent"></a>

## Run SuperPlane Agent

**Component key:** `runnerSuperPlane`

Runs a SuperPlane-hosted coding agent on a fleet runner.
...
```json
{
  ...
  "timestamp": "2026-01-16T17:56:16.680755501Z",
  "type": "runnerSuperPlane.finished"
}
```

<a id="set-task-status-note"></a>

## Set Task Status Note

**Component key:** `setWorkOrderStatusNote`

The Set Task Status Note component announces what a waiting task is blocked on and what resolves it. The note shows as a "next step" panel on the task page — for example, a PR watcher can announce "Review the pull request: when it merges, this task completes automatically."
...
<a id="wait-for-endpoint"></a>

## Wait for Endpoint

**Component key:** `waitForEndpoint`

Wait for Endpoint checks a service after deployment and continues the workflow only when it is ready.
...
```json
{
  "data": {
    ...
    "elapsedMs": 20000,
    "finishedAt": "2026-08-01T06:00:20Z",
    "resolvedUrl": "https://service.example.com/ready",
    "startedAt": "2026-08-01T06:00:00Z",
    "status": 200,
    "url": "https://service.example.com/ready"
  },
  "timestamp": "2026-08-01T06:00:20Z",
  "type": "endpoint.ready"
}
```
