...
- **secrets(name)**: Access an organization secret's keys as a map (resolved at execution time, never persisted to stored configuration). Example: `secrets("api").token`.
