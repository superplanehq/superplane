...
<CardGrid>
  ...
  <LinkCard title="SSH Command" href="#ssh-command" description="Run a command on a remote host via SSH using Secrets for authentication" />
...
The SSH Command component runs a single command on a remote host via SSH. Authentication uses organization Secrets: choose **SSH key** (private key, optional passphrase) or **Password**, and select the Secret and key name that holds the credential.

### Configuration

- **Host**, **Port** (default 22), **Username**: Connection details.
- **Authentication**: SSH key or Password. For SSH key, specify the Secret and key name for the private key (and optionally for passphrase). For Password, specify the Secret and key name for the password.
- **Command**: The command to run (supports expressions).
- **Working directory**: Optional; runs `cd <dir> && <command>`.
- **Timeout (seconds)**: Required. Limits how long the command may run (default 60 seconds).
