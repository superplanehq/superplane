...
- Supported keys: `type` (`string`, `number`, `boolean`, `select`), `title`, `default`, `required`, `values` (select only), `order` (optional).
- `order` is an optional non-negative integer (unquoted, for example `order:1`). It controls field order in autogenerated run forms only; run-time merge ignores it. Omitted `order` is treated as `0`. Sort fields by `order` ascending, then by path for ties.
