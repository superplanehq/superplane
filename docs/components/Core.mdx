...
<CardGrid>
  ...
  <LinkCard title="Add Pull Request Activity" href="#add-pull-request-activity" description="Link the current run to a factory pull request" />
  ...
  <LinkCard title="Update Pull Request" href="#update-pull-request" description="Update a factory pull request already attached to a work order" />
...
The Add Pull Request Activity component links the current canvas run to a factory pull request. An existing relation is treated as success.
...
- **Artifact Key** (`by: artifactKey`): resolves the work order that owns the artifact tagged with `artifactKey` (set via `addWorkOrderArtifact`'s `artifactKey` field).
...
Typical wiring to stay in sync with GitHub: a `github.onPullRequest` trigger → `findPullRequest` (`url: {{ event.data.pull_request.html_url }}`) → this component.
