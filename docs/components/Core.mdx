...
The Update Work Order Status component transitions a work order across the lifecycle draft → open → closed (with reopen from closed back to open). When closing, a result (completed, rejected, or failed) must be provided. This component can only be used in factory-owned apps.
