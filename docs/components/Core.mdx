...
<CardGrid>
  ...
  <LinkCard title="Get Canvas Data" href="#get-canvas-data" description="Read a value from the canvas data store, optionally a previous version" />
  ...
  <LinkCard title="Set Canvas Data" href="#set-canvas-data" description="Store a value under a key on the canvas with history" />
...
<a id="get-canvas-data"></a>

## Get Canvas Data

The Get Canvas Data component reads a value for a key from the canvas data store. You can read the current (latest) value or a previous version.

### Use Cases

- **Rollback**: Get the previous version of a key (e.g. last deployed version) to roll back.
- **Compare**: Read current and previous in separate nodes to detect changes.
- **Ephemeral teardown**: Read a list of machine IDs stored by an earlier step.

### Configuration

- **Key**: Canvas-scoped key to read (e.g. `app/my-service/last_version`).
- **Version**: Current (latest), Previous, or N steps back.
...
{
  "data": {
    "key": "app/backend/last_version",
    "found": true,
    "value": "v1.2.3",
    "created_at": "2026-02-06T12:00:00.000Z"
  },
  "type": "canvasdata.get.finished"
}
```

<a id="set-canvas-data"></a>

## Set Canvas Data

The Set Canvas Data component writes a key-value pair to the canvas data store. Every write is versioned so you can later read the current or previous values.
...
- **Value**: The value to store (expression supported; stored as string or JSON).
