...
1. **Create an Azure App Registration**:
   ...
   - Note the **Application (client) ID** — this is the **App ID**
   - Note the **Directory (tenant) ID** — this is the **Tenant ID**
   - Go to **Certificates & secrets** → **New client secret** → copy the **Value** — this is the **App Password**
...
3. **Enter credentials in SuperPlane** and click Connect. SuperPlane will verify your credentials and show your webhook URL.

4. **Create an Azure Bot**:
   - Go to **Azure Portal** → **Create a resource** → search **Azure Bot**
   - Link it to the App Registration from step 1
   - Set the **Messaging endpoint** to the webhook URL shown by SuperPlane
   - Under **Channels**, click **Microsoft Teams** to enable it

5. **Install the Teams App**: Download the manifest ZIP from SuperPlane and upload it to **Teams Admin Center** → **Manage apps** → **Upload new app**, or sideload to specific teams
...
- **Content Filter**: Optional regex pattern to filter mentions by content (e.g., `/deploy` to only trigger on mentions containing "/deploy")
