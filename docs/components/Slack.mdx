...
<CardGrid>
  ...
  <LinkCard title="Send Message and Wait" href="#send-message-and-wait" description="Send a message with buttons and wait for a response" />
</CardGrid>
...
<a id="send-message-and-wait"></a>

## Send Message and Wait

Send a message to a Slack channel or DM and wait for the user to click one of the configured buttons.
...
- **Received**: Emitted when the user clicks a button; payload includes the selected button's value.
- **Timeout**: Emitted when no button click is received within the configured timeout.

### Notes

- The component automatically sets up Slack interactivity handling.
- Buttons are displayed as a Block Kit actions block.
- Each button click is unique to the workflow execution.
