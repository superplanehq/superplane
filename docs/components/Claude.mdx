...
The Create Batch Message component uses [Anthropic's Message Batches API](https://platform.claude.com/docs/en/build-with-claude/batch-processing) to process one or many prompts asynchronously in one request, at a lower cost than issuing them individually.
...
- A batch can contain up to 1000 requests.
