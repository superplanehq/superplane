...
<CardGrid>
  <LinkCard title="On Pipeline Completed" href="#on-pipeline-completed" description="Listen to CircleCI workflow completion events within a pipeline" />
</CardGrid>
...
Triggers when a CircleCI workflow completes within a pipeline. Note that a single pipeline can contain multiple workflows.
