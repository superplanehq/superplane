...
<CardGrid>
  ...
  <LinkCard title="Get Workflow Usage" href="#get-workflow-usage" description="Get GitHub Actions billable usage for an organization" />
...
The Get Workflow Usage component retrieves billable GitHub Actions usage for the installation organization.

### Prerequisites

- GitHub App must include **Organization Administration: Read** permission.
- Existing installations may need to approve this permission update before usage requests succeed.

### Use Cases

- **Billing visibility**: Track Actions billable minutes over time
- **Cost monitoring**: Break usage down by runner OS/SKU
- **Repository slicing**: Limit usage report to selected repositories

### Configuration

- **Repositories**: Optional multi-select. Leave empty to include all organization repositories.
- **Year / Month / Day**: Optional period filters.
- **Product**: Billing product (defaults to Actions).
- **Runner OS / SKU**: Optional SKU filter.

### Output

Returns usage data including:
- **minutes_used**: Total billable minutes
- **minutes_used_breakdown**: Billable minutes grouped by OS/SKU
- **net_amount**: Aggregated cost (when available)
- **usage_items**: Raw usage rows from GitHub billing report

Note: The API supports breakdown by repository and OS/SKU, not by workflow.
...
```json
{
  ...
  "timestamp": "2026-02-16T10:12:00.000000000Z",
