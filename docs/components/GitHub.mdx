...
<CardGrid>
  ...
  <LinkCard title="Wait For Pull Request Checks" href="#wait-for-pull-request-checks" description="Wait until GitHub Checks and Commit Statuses on a commit become complete" />
</CardGrid>
...
The pull request trigger and this wait action create separate managed webhooks. The wait action listens to `check_run`, `check_suite`, and `status` events. It uses a five-minute poll only as a fallback.
