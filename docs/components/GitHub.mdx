...
- **Paths** *(optional)*: GitHub Actions–style glob patterns against added, modified, and removed files. Use `!` as a prefix to exclude a path pattern (for example include `billing/**` and exclude `!billing/**/*.md`). If every pattern is an exclude, an implicit include of `**` is used so negation-only lists still work. Older workflows may still store paths as predicate objects: `equals` entries are interpreted as globs; `matches` (regex) entries must be recreated as globs (they are skipped until updated). If the push payload has no usable per-commit file paths, a path filter cannot match and the trigger will not fire. Leave empty to fire on all pushes regardless of paths.
