...
- **Snapshot**: Base environment snapshot (optional, defaults to "default")
- **Target**: Target region for the sandbox (optional)
- **Auto Stop Interval**: Time in minutes before the sandbox auto-stops
- **Environment Variables**: Environment variables for the sandbox
- **Repository**: Repository URL to clone
- **Bootstrap**: Object with one of the following forms:
  - `{ "from": "inline", "script": "npm ci && npm test" }`
  - `{ "from": "file", "path": "scripts/bootstrap.sh" }`
  - `{ "from": "url", "url": "https://example.com/bootstrap.sh" }`
- **Command Timeout**: Timeout in seconds for clone and bootstrap commands

### Output

Returns:
- **sandbox**: The started sandbox information
- **repository**: Repository URL used for cloning
- **directory**: Full clone directory path
- **clone**: Clone command result
- **bootstrap**: Bootstrap command result
