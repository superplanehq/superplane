...
- **Sandbox**: The sandbox ID to run commands in (from **Create Sandbox** or **Create Repository Sandbox** output). Supports expressions, e.g. `{{ previous().data.id }}` or `{{ $["Create Sandbox"].data.id }}`
