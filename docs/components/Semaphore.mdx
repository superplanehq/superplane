...
<CardGrid>
  <LinkCard title="Get Pipeline" href="#get-pipeline" description="Retrieve a pipeline from Semaphore by ID" />
...
The Get Pipeline component retrieves a single pipeline from Semaphore by its ID, returning detailed information about the pipeline status, jobs, and results.

### Use Cases

- **Pipeline status checks**: Fetch current pipeline details to drive workflow decisions
- **Result verification**: Check if a specific pipeline passed or failed before proceeding
- **Job-level analysis**: Inspect individual job results within a pipeline
- **Cross-system reporting**: Pull pipeline data for external dashboards or notifications

### Configuration

- **Pipeline ID**: The Semaphore pipeline UUID to retrieve (required, supports expressions)
...
- **id**: Pipeline ID
- **name**: Pipeline name
- **state**: Pipeline state (e.g. done, running, queuing)
- **result**: Pipeline result (passed, failed, stopped)
- **created_at**: Pipeline creation timestamp
- **done_at**: Pipeline completion timestamp
- **blocks**: Array of blocks with their jobs and results
- **workflow**: Associated workflow information
...
```json
{
  ...
  "timestamp": "2026-01-22T15:32:56Z",
