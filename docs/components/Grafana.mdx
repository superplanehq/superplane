...
SuperPlane-managed alert rules use a three-step data pipeline: `A` is the source query, `B` is the reduce expression, and `C` is the threshold condition Grafana evaluates.
...
The returned structure depends on how the rule was created. SuperPlane-managed rules use the `A`/`B`/`C` pipeline shown below.
