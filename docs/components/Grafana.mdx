...
- **Job**: Display name for the check
- **Target**: HTTP or HTTPS URL to monitor
- **Enabled**: Whether the check should begin running immediately
- **Frequency / Timeout**: Probe cadence in seconds and request timeout in milliseconds
- **Probes**: Probe IDs selected from the connected Grafana synthetics tenant
- **Method / Headers / Body**: Core HTTP request configuration
- **Redirect / SSL / TLS / Status / Body / Header Matchers**: Request validation behavior
- **Per-Check Alerts**: Optional Grafana synthetic monitoring alerts configured after check creation
- **Labels / Basic Auth / Bearer Token**: Optional metadata and authentication
...
- **Healthy**: The most recent run outcome is "Healthy"
- **Degraded**: The most recent run outcome is "Degraded"
- **Critical**: The most recent run outcome is "Critical"
...
- **Synthetic Check**: The synthetic check to update
- **Job**, **Labels**, **Request**, **Schedule**, **Response validation**, and **Per-Check Alerts** are togglable: enable a section only when you want to change it; disabled sections keep the values stored in Grafana
