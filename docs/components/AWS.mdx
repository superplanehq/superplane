...
<CardGrid>
  ...
  <LinkCard title="SNS • On Topic Message" href="#sns-•-on-topic-message" description="Listen to AWS SNS topic notification events" />
</CardGrid>
...
<CardGrid>
  ...
  <LinkCard title="SNS • Subscribe" href="#sns-•-subscribe" description="Create an AWS SNS subscription" />
  <LinkCard title="SNS • Unsubscribe" href="#sns-•-unsubscribe" description="Delete an AWS SNS subscription" />
</CardGrid>
...
```json
{
  "data": {
    ...
    "id": "5f0bc5d7-9ad1-4d2c-b68b-41f26032e41e",
    "region": "us-east-1",
    "resources": [
      "arn:aws:sns:us-east-1:123456789012:orders-events"
    ],
    "source": "aws.sns",
...
### Required IAM Permissions for SNS

The following IAM permissions are required for SNS components:

| Component | Required Permissions |
|-----------|---------------------|
| On Topic Message | `events:PutRule`, `events:PutTargets`, `events:DeleteRule`, `events:RemoveTargets` |
| Get Topic | `sns:GetTopicAttributes` |
| Get Subscription | `sns:GetSubscriptionAttributes` |
| Create Topic | `sns:CreateTopic`, `sns:GetTopicAttributes` |
| Delete Topic | `sns:DeleteTopic` |
| Publish Message | `sns:Publish` |
| Subscribe | `sns:Subscribe`, `sns:GetSubscriptionAttributes` |
| Unsubscribe | `sns:Unsubscribe` |

For resource listing in the UI:
- `sns:ListTopics` - Required for topic selection dropdowns
- `sns:ListSubscriptions` - Required for subscription selection dropdowns
...
<a id="sns-•-subscribe"></a>

## SNS • Subscribe

The Subscribe component creates a new AWS SNS subscription for a topic.
...
<a id="sns-•-unsubscribe"></a>

## SNS • Unsubscribe

The Unsubscribe component removes an AWS SNS subscription.
...
```json
{
  ...
  "timestamp": "2026-01-10T10:00:02.000000000Z",
  "type": "aws.sns.subscription.unsubscribed"
}
```
