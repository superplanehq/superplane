...
<CardGrid>
  ...
  <LinkCard title="SQS • Create Queue" href="#sqs-•-create-queue" description="Create an SQS queue" />
  <LinkCard title="SQS • Delete Queue" href="#sqs-•-delete-queue" description="Delete an SQS queue" />
  <LinkCard title="SQS • Get Queue" href="#sqs-•-get-queue" description="Retrieve an existing SQS queue" />
...
The Create Queue component creates a new SQS queue.

### Use Cases

- **New workflows**: Create a dedicated queue for a new workflow or service
- **Ephemeral queues**: Spin up short-lived queues for temporary processing
- **Environment isolation**: Create separate queues per environment or tenant

### Configuration

- **Region**: AWS region where the queue will be created
- **Queue Name**: Name of the SQS queue
- **Queue Type**: Standard or FIFO
- **Attributes**: Optional SQS attributes (visibility timeout, message retention, etc.)
...
The Get Queue component retrieves metadata and configuration for an existing SQS queue.

### Use Cases

- **Health checks**: Verify that a queue exists before sending messages
- **Configuration audits**: Inspect attributes like visibility timeout or retention period
- **Dynamic routing**: Resolve queue URLs from names at runtime

### Configuration

- **Region**: AWS region of the queue
- **Queue URL or Name**: URL or name of the SQS queue
...
The Send Message component sends a message to an SQS queue.

### Use Cases

- **Event fan-out**: Push events into queues for downstream workers
- **Asynchronous processing**: Offload long-running work from synchronous flows
- **Retries and buffering**: Use SQS as a durable buffer between services

### Configuration

- **Region**: AWS region of the queue
- **Queue URL or Name**: Target SQS queue
- **Message Body**: JSON payload to send
- **Message Attributes**: Optional typed attributes for routing and filtering
- **Delay Seconds**: Optional delay before the message becomes visible
...
{
  "data": {
    "messageId": "7c8b9d10-1112-1314-1516-171819202122",
    "md5OfMessageBody": "5eb63bbbe01eeed093cb22bb8f5acdc3"
  },
  "timestamp": "2026-02-18T12:00:00Z",
  "type": "aws.sqs.message.sent"
}
```
...
The Delete Queue component deletes an SQS queue.

### Use Cases

- **Cleanup**: Remove queues created for temporary environments or tests
- **Decommissioning**: Delete queues that are no longer used
- **Cost control**: Reduce clutter and avoid accidental usage

### Configuration

- **Region**: AWS region of the queue
- **Queue URL or Name**: SQS queue to delete
...
The Purge Queue component removes all messages from an SQS queue.

### Use Cases

- **Reset state**: Clear stuck or invalid messages before reprocessing
- **Test environments**: Clean up test data between runs
- **Operational recovery**: Quickly clear a backlog after a bad deployment

### Configuration

- **Region**: AWS region of the queue
- **Queue URL or Name**: SQS queue to purge
