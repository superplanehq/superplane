...
<CardGrid>
  ...
  <LinkCard title="S3 • Get Object Attributes" href="#s3-•-get-object-attributes" description="Retrieve attributes of an S3 object" />
  <LinkCard title="S3 • Head Bucket" href="#s3-•-head-bucket" description="Check if an S3 bucket exists and retrieve its region" />
  <LinkCard title="S3 • Head Object" href="#s3-•-head-object" description="Retrieve metadata of an S3 object without downloading its content" />
...
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region for the S3 bucket |
| Bucket Name | String | Yes | Name of the bucket to create |
...
The Copy Object component copies an object from one S3 location to another within the same region.

### Configuration

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region of the S3 buckets |
| Source Bucket | Integration Resource | Yes | Bucket containing the source object |
| Source Key | String | Yes | Key (path) of the source object |
| Destination Bucket | Integration Resource | Yes | Bucket to copy the object to |
| Destination Key | String | Yes | Key (path) for the copied object |
...
```json
{
  ...
  "data": {
    ...
    "destinationKey": "path/to/dest.txt",
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region of the S3 bucket |
| Bucket | Integration Resource | Yes | Target S3 bucket |
| Object Key | String | Yes | Key (path) of the object to delete |
...
The Head Object component retrieves metadata of an object in an AWS S3 bucket without downloading the object itself.
...
| Region | Select | Yes | AWS region of the S3 bucket |
| Bucket | Integration Resource | Yes | Target S3 bucket |
| Object Key | String | Yes | Key (path) of the object |

### Example Output

```json
{
  "type": "aws.s3.object.head",
  "timestamp": "2026-02-24T12:00:00Z",
  "data": {
    "bucket": "my-bucket",
    "key": "path/to/object.txt",
    ...
    "etag": "\"d41d8cd98f00b204e9800998ecf8427e\"",
    "lastModified": "Mon, 24 Feb 2026 12:00:00 GMT"
  }
}
```
...
The Head Bucket component checks if an AWS S3 bucket exists and retrieves its region.

### Configuration

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region to use for the request |
| Bucket | Integration Resource | Yes | Target S3 bucket to check |

### Example Output

```json
{
  "type": "aws.s3.bucket.head",
  "timestamp": "2026-02-24T12:00:00Z",
  "data": {
    "bucket": "my-bucket",
    "exists": true,
...
The Get Object Attributes component retrieves attributes of an object in an AWS S3 bucket.
...
| Region | Select | Yes | AWS region of the S3 bucket |
| Bucket | Integration Resource | Yes | Target S3 bucket |
| Object Key | String | Yes | Key (path) for the object |
| Content Type | String | No | MIME type of the content (default: application/octet-stream) |
| Body | Text | Yes | Content to upload |
...
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region of the S3 bucket |
| Bucket | Integration Resource | Yes | Target S3 bucket to delete |
...
|-------|------|----------|-------------|
| Region | Select | Yes | AWS region of the S3 bucket |
| Bucket | Integration Resource | Yes | Target S3 bucket to empty |
