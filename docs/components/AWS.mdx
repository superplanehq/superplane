...
<CardGrid>
  ...
  <LinkCard title="CodeArtifact • Dispose Package Versions" href="#code-artifact-•-dispose-package-versions" description="Delete assets and set package version status to Disposed" />
  ...
  <LinkCard title="CodeArtifact • Update Package Versions Status" href="#code-artifact-•-update-package-versions-status" description="Set package version status to Archived, Published, or Unlisted" />
...
If you use **CodeArtifact** components (Create Repository, Delete Repository, Get Package Version, etc.), the IAM role must allow CodeArtifact API calls. Otherwise the Domain/Repository dropdowns will show "Failed to load resources" and workflow runs will fail with 403.

**Minimal policy for listing domains and repositories** (for the configuration dropdowns):
...
**For Create/Delete Repository**: `codeartifact:CreateRepository`, `codeartifact:DeleteRepository`, `codeartifact:DescribeRepository`. **For Update Package Versions Status**: `codeartifact:UpdatePackageVersionsStatus`. **For Copy Package Versions**: `codeartifact:CopyPackageVersions`. **For Delete Package Versions**: `codeartifact:DeletePackageVersions`. **For Dispose Package Versions**: `codeartifact:DisposePackageVersions`.

Attach this (or a combined policy) to the same IAM role you use for the SuperPlane AWS integration. You can use the AWS managed policy **AWSCodeArtifactFullAccess** for full CodeArtifact access, or create a custom policy with only the actions you need.
...
- **Region**: AWS region for CodeArtifact (e.g. us-east-1)
- **Domain**: CodeArtifact domain (from integration resource)
- **Repository name**: Name of the new repository (2–100 characters)
- **Description**: Optional description for the repository

### Example Output

The component emits the created repository details, including ARN, name, domain, and creation time.
...
- **Domain**: CodeArtifact domain (from integration resource)
- **Repository**: Repository to delete (from integration resource)

### Example Output

The component emits the deleted repository details (same shape as create), including ARN, name, and domain.

<a id="code-artifact-•-update-package-versions-status"></a>

## CodeArtifact • Update Package Versions Status

The Update Package Versions Status component sets the status of one or more package versions to **Archived**, **Published**, or **Unlisted**.
...
- **Region**, **Domain**, **Repository**: CodeArtifact target (from integration resources)
- **Format**, **Package**: Package format (e.g. npm, pypi) and package name
- **Versions**: Comma- or newline-separated list of version strings
- **Target status**: Archived | Published | Unlisted
- **Expected status** (optional): Update only if current status matches
- **Namespace** (optional): For Maven/npm/Swift/generic

### Example Output

The component emits `successfulVersions` and `failedVersions` maps (version → revision/status or error).
...
- **Region**, **Domain**: CodeArtifact domain (from integration resources)
- **Source repository**, **Destination repository**: From integration resources
- **Format**, **Package**: Package format and package name
- **Versions**: Comma- or newline-separated list of version strings
- **Allow overwrite**: Overwrite existing versions in the destination
- **Include from upstream**: Include versions from upstream repositories
- **Namespace** (optional): For Maven/npm/Swift/generic
...
The Delete Package Versions component permanently removes one or more package versions and their assets. Deleted versions cannot be restored. To hide versions but keep the option to restore, use **Update Package Versions Status** to set status to **Archived**.
...
- **Format**, **Package**: Package format and package name
- **Versions**: Comma- or newline-separated list of version strings
- **Expected status** (optional): Delete only if current status matches
- **Namespace** (optional): For Maven/npm/Swift/generic

### Example Output

Emits `successfulVersions` and `failedVersions` (status will be `Deleted` for successful versions).

<a id="code-artifact-•-dispose-package-versions"></a>

## CodeArtifact • Dispose Package Versions

The Dispose Package Versions component deletes the assets of package versions and sets their status to **Disposed**. The version record remains (visible in ListPackageVersions with status Disposed); assets cannot be restored.
...
- **Format**, **Package**: Package format and package name
- **Versions**: Comma- or newline-separated list of version strings
- **Expected status** (optional): Dispose only if current status matches
- **Namespace** (optional): For Maven/npm/Swift/generic

### Example Output

Emits `successfulVersions` and `failedVersions` (status will be `Disposed` for successful versions).

### Testing CodeArtifact components

- **Integration per node:** Each action node (Create Repository, Copy Package Versions, etc.) must have the **AWS integration** selected in the node’s Integration dropdown; otherwise runs fail with "AWS integration context is missing". Save the workflow after selecting it.
- **Manual testing:** Use the **Manual Run** trigger and one action per workflow (e.g. Manual Run → Create Repository). Downstream nodes do not run automatically; test each component in its own workflow.
