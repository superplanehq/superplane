...
- Additional roles depending on which components you use (e.g. `roles/compute.admin` for VM management, `roles/cloudfunctions.invoker` for Cloud Functions)
...
The integration's service account must have `roles/cloudfunctions.invoker` on the target function.
