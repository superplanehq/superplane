...
<CardGrid>
  <LinkCard title="Create Build" href="#create-build" description="Create and run a Cloud Build build" />
  <LinkCard title="Get Build" href="#get-build" description="Retrieve a Cloud Build build by ID" />
...
Creates and starts a Google Cloud Build build, then waits for it to finish.
...
- **Source**: Optional JSON object for the build source. Use this for advanced sources like `gitSource`, `repoSource`, `connectedRepository`, or `storageSource`. Example: `{"gitSource":{"url":"https://github.com/org/repo.git","revision":"main"}}`
- **Use Connected Repository**: Enable this to pick a Cloud Build 2nd-gen region, connection, repository, and branch/tag/commit directly from GCP. SuperPlane sends `source.connectedRepository` and creates the build in that region.
- **Cloud Build Location / Connection / Connected Repository**: Select the regional Cloud Build connection and repository to build from.
- **Connected Repository Revision**: Choose whether the connected repository uses a branch, tag, or commit SHA.
- **Connected Branch / Tag / Commit SHA**: Choose exactly one connected repository revision based on the selected revision type.
- **Repository URL / Cloud Source Repo**: Manual shortcut source field. If the value is a Git URL such as `https://github.com/org/repo.git` or `git@github.com:org/repo.git`, SuperPlane creates `source.gitSource`. If it is a plain repository name, SuperPlane treats it as a Cloud Source Repository and creates `source.repoSource`.
- **Branch Name / Tag Name / Commit SHA**: Choose exactly one revision field when using **Repository URL / Cloud Source Repo**.
...
- **Project ID Override**: Override the connected GCP project when needed.

### How To Use

- **Custom build**: Provide **Steps** only, or **Steps** plus **Source** if you need an advanced source definition.
- **Connected repository build**: Enable **Use Connected Repository**, select the **Cloud Build Location**, **Connection**, **Connected Repository**, then choose **Branch**, **Tag**, or **Commit SHA** from the connected repository selectors.
- **Repository build**: Set **Repository URL / Cloud Source Repo** to a Git URL, then set exactly one of **Branch Name**, **Tag Name**, or **Commit SHA**. Cloud Build checks out the repository and runs your steps against that workspace.
- **Cloud Source Repository build**: Either set **Repository URL / Cloud Source Repo** to the repository name and one revision field, or use **Source** with a `repoSource` object if you need full control.
...
```json
{
  ...
  "branchName": "main",
  "steps": "[{\"name\":\"golang:1.22\",\"entrypoint\":\"bash\",\"args\":[\"-lc\",\"go test ./...\"]}]"
}
```
