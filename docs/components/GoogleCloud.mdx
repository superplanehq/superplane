...
- Additional roles depending on which components you use (e.g. `roles/compute.admin` for VM management, `roles/cloudfunctions.developer` for Cloud Functions)
...
- **Serverless automation**: Trigger custom business logic or data processing from a workflow step
- **Third-party integrations**: Call a function that wraps an external API or webhook
- **Data transformation**: Pass data through a Cloud Function and use the result in downstream steps
...
| Function | Yes | The Cloud Function to invoke (select from deployed functions in the chosen location) |
| Payload | No | JSON object sent as input data to the function |
| Project ID Override | No | Override the GCP project from the integration |

### Required IAM roles

The integration's service account must have one of:

- `roles/cloudfunctions.developer` — covers listing locations/functions and invoking (recommended)
- `roles/cloudfunctions.viewer` + `roles/cloudfunctions.invoker` — minimum set for listing and invoking

### Output

Emits a payload with the invocation result:

- `functionName`: Full resource name of the invoked function
- `executionId`: Unique ID assigned to this invocation by Cloud Functions
- `result`: The function's response, parsed as a JSON object when possible
