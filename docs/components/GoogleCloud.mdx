...
<CardGrid>
  <LinkCard title="Create Build" href="#create-build" description="Create a Cloud Build build and wait for it to finish" />
  <LinkCard title="Run Trigger" href="#run-trigger" description="Run a Cloud Build trigger and wait for the build to finish" />
...
- **Reuse existing trigger config**: Fire a preconfigured trigger from a workflow without duplicating build step definitions
- **Branch or tag overrides**: Run a trigger against a specific branch, tag, or commit SHA that differs from the trigger's default
- **Pipeline chaining**: Kick off a downstream Cloud Build trigger after another workflow step completes

### Configuration

| Field | Required | Description |
|-------|----------|-------------|
| Trigger | Yes | The Cloud Build trigger to run (select from the project's triggers) |
| Branch Name | No | Override the branch to build from. Mutually exclusive with Tag Name and Commit SHA |
| Tag Name | No | Override the tag to build from |
| Commit SHA | No | Override the commit SHA to build from |
| Project ID Override | No | Override the GCP project from the integration |

### Output Channels

- **Passed**: Emitted when the build finishes with `SUCCESS`
- **Failed**: Emitted for any other terminal status: `FAILURE`, `INTERNAL_ERROR`, `TIMEOUT`, `CANCELLED`, `EXPIRED`

### Notes

- If no revision override is set, Cloud Build uses the trigger's configured default branch/tag.
- SuperPlane listens for Pub/Sub notifications and falls back to polling every 5 minutes.
- Cancelling the execution from the UI sends a Cloud Build cancel request for the active build.
