...
<CardGrid>
  ...
  <LinkCard title="Compute • Update Firewall Rule" href="#compute-•-update-firewall-rule" description="Update a VPC firewall rule: its protocols and ports, ranges, priority, target tags, description, or enabled state" />
...
The Update Firewall Rule component changes an existing VPC firewall rule. Toggle on only the fields you want to change; everything else is left untouched.
