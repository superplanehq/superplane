...
The Update Incident component updates an existing incident in Rootly with new details.

### Use Cases

- **Status transitions**: Update incident status as investigation progresses (started → mitigated → resolved)
- **Severity adjustments**: Escalate or de-escalate incident severity based on impact assessment
- **Incident enrichment**: Add or update title and summary with investigation findings
- **Automated workflows**: Automatically update incidents based on monitoring data or workflow outcomes
...
- **Severity**: Updated severity level (optional, uses integration resource selector)
- **Status**: Updated incident status, e.g. mitigated, resolved (optional, supports expressions)

All fields except Incident ID are optional, allowing partial updates.
...
- **mitigated_at**: Mitigation timestamp (if mitigated)
- **url**: Link to the incident in Rootly
...
```json
{
  "data": {
    ...
    "started_at": "2026-01-19T12:00:00Z",
    "status": "mitigated",
    "summary": "Database connection pool exhausted due to connection leak in payment service.",
    "title": "Database connection issues - payment service",
    "url": "https://app.rootly.com/incidents/abc123-def456"
  },
