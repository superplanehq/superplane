...
- **Service Filter** (optional): Only trigger for incidents attached to specific Rootly service(s)
- **Team Filter** (optional): Only trigger for incidents attached to specific Rootly team(s)

### Event Data

Each resolved incident event includes an `incident` object with fields like `id`, `sequential_id`, `title`, `slug`, `status`, `resolution_message`, `resolved_at`, `resolved_by`, and `url`.
