...
- **Provisioning**: Add a record when a new service or environment is provisioned
- **Verification**: Create TXT or CNAME records for certificates or domain ownership
- **Releases**: Add or update DNS records during canary rollouts or migrations

### Configuration

- **Zone**: Zone ID or domain name (e.g., `example.com`)
...
- **Priority**: MX or SRV priority (optional)

### Output

**Success channel**: Emits the created DNS record (id, type, name, content, proxied, ttl)  
**Failed channel**: Emits when the zone is not found, the record is invalid, or a duplicate exists
