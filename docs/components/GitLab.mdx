...
<CardGrid>
  ...
  <LinkCard title="Get Pipeline" href="#get-pipeline" description="Get a GitLab pipeline by ID" />
  <LinkCard title="Get Latest Pipeline" href="#get-latest-pipeline" description="Get the latest GitLab pipeline for a project" />
  <LinkCard title="Get Test Report Summary" href="#get-test-report-summary" description="Get test report summary for a GitLab pipeline" />
</CardGrid>
...
The Run Pipeline component triggers a GitLab pipeline and waits for completion.

### Configuration

- **Project** (required): GitLab project where the pipeline will run
- **Ref** (required): Branch or tag ref to run the pipeline on
- **Inputs** (optional): Pipeline inputs (`name`/`value`) sent in the create pipeline request
- **Variables** (optional): Pipeline variables sent to the create pipeline API

### Output Channels

- **Passed**: Emitted when pipeline completes with `success`
- **Failed**: Emitted when pipeline completes with `failed`, `canceled`, `skipped`, `manual`, or `blocked`

### Notes

- Uses webhook updates to detect completion quickly
- Falls back to polling if webhook updates are delayed

<a id="get-pipeline"></a>

## Get Pipeline

The Get Pipeline component retrieves a specific pipeline from a GitLab project by pipeline ID.

### Configuration

- **Project** (required): GitLab project containing the pipeline
- **Pipeline** (required): Select a pipeline from the selected project

### Output

Returns the pipeline object, including status, ref, SHA, and URL.

<a id="get-latest-pipeline"></a>

## Get Latest Pipeline

The Get Latest Pipeline component retrieves the most recent pipeline for a GitLab project.

### Configuration

- **Project** (required): GitLab project to query
- **Ref** (optional): Branch or tag to scope the latest pipeline lookup

### Output

Returns the latest matching pipeline object.

<a id="get-test-report-summary"></a>

## Get Test Report Summary

The Get Test Report Summary component fetches test report summary data for a specific pipeline.
...
Returns total test statistics and per-suite summary information.
...
- **Default channel**: Emits pipeline payload data including pipeline status, ref, SHA, and URL
