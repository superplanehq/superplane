...
This trigger uses issue webhooks from the SuperPlane Sentry app, or from a Sentry internal integration when Connect is not available. SuperPlane verifies each webhook signature before routing the event to matching triggers.
