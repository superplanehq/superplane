...
The On Build trigger emits build-related  events for a selected Render service.
...
- **Service**:The Render service you want to monitor or act on.
...
- **Event Types**: Deploy event states to listen for (e.g started, succeeded, failed). Defaults to `deploy.finished`.
