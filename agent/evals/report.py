...
class ReportBuilder:
    ...
    def _ordered_report_rows(self) -> list[ReportCase[Any, Any, Any] | ReportCaseFailure[Any, Any, Any]]:
        """Match dataset order; pydantic-evals splits successes vs task exceptions."""
        by_name: dict[str, ReportCase[Any, Any, Any] | ReportCaseFailure[Any, Any, Any]] = {}
        for row in self.report.cases:
            if row.name in by_name:
                raise RuntimeError(f"duplicate successful report case name {row.name!r}")
            by_name[row.name] = row
        for row in self.report.failures:
            if row.name in by_name:
                raise RuntimeError(f"eval report case name collision: {row.name!r}")
            by_name[row.name] = row
    ...
    def render(self) -> None:
        ...
        for i, case_result in enumerate(ordered_rows):
            ...
            print("  assertions:")
            assertion_lines = (
                [] if isinstance(case_result, ReportCaseFailure) else self._format_assertion_lines(case_result)
            )
