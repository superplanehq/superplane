...
from ai.agent import load_system_prompt
...
def test_load_system_prompt_includes_index(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(skills_mod, "_skills_root", lambda: tmp_path)
    (tmp_path / "x.md").write_text("---\ndescription: d\n---\n\nHi\n", encoding="utf-8")
    prompt = load_system_prompt()
    assert "## Patterns vs skills" in prompt
