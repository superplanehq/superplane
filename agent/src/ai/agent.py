...
CatalogCacheValue = list[str] | list[dict[str, Any]]


def _clone_catalog_cache_rows(rows: CatalogCacheValue) -> CatalogCacheValue:
    """Detach cached catalog results so callers cannot mutate the in-session cache."""
    if not rows:
        return []
    if isinstance(rows[0], str):
        return list(cast(list[str], rows))
...
class AgentDeps:
    ...
    catalog_list_cache: dict[tuple[str, str, str], CatalogCacheValue] = field(
...
def _put_cached_catalog_list(
    ...
    provider: str | None,
    query: str | None,
    rows: CatalogCacheValue,
) -> None:
    key = _catalog_list_cache_key(kind, provider, query)
    deps.catalog_list_cache[key] = _clone_catalog_cache_rows(rows)
    ...
    *,
    ...
    def list_components(
        ...
        try:
            ...
            return cast(list[str], _clone_catalog_cache_rows(rows))
        except Exception as error:
            _tool_debug(f"list_components failed: {error}")
            return cast(list[str], [_tool_error_entry("list_components", error)])
    ...
    def list_triggers(
        ...
        try:
            ...
            return cast(list[dict[str, Any]], _clone_catalog_cache_rows(rows))
        except Exception as error:
