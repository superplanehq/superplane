...
describe("ElkLayoutEngine", () => {
  ...
  it("lays out factory apps top-to-bottom when direction is vertical", async () => {
    const workflow: CanvasesCanvas = {
      metadata: {
        id: "canvas-vertical",
    ...
    expect(byId.get("process")!.position!.y!).toBeLessThan(byId.get("check")!.position!.y!);
  });

  it("packs disconnected components side-by-side for vertical direction", async () => {
    const workflow: CanvasesCanvas = {
      metadata: {
        id: "canvas-vertical-pack",
        name: "vertical-pack",
      },
      spec: {
        nodes: [
  ...
  it("reverses SOUTH port indexes for vertical flow so left-to-right handles match ELK", () => {
    const workflow: CanvasesCanvas = {
      metadata: { id: "canvas-if", name: "if-vertical" },
      spec: {
        nodes: [
          { id: "if", name: "if", type: "TYPE_ACTION", component: "control.if", position: { x: 0, y: 0 } },
          { id: "when-true", name: "true", type: "TYPE_ACTION", component: "noop", position: { x: 0, y: 300 } },
          { id: "when-false", name: "false", type: "TYPE_ACTION", component: "noop", position: { x: 400, y: 300 } },
        ],
        edges: [
          { sourceId: "if", targetId: "when-true", channel: "true" },
          { sourceId: "if", targetId: "when-false", channel: "false" },
        ],
      },
    };
    ...
    const graph = (autoLayout as unknown as ElkLayoutEngineInternals).buildElkGraph(
      workflow,
      workflow.spec!.nodes!,
      new Map([["if", ["true", "false"]]]),
      undefined,
      "vertical",
    );
    const ifNode = graph.children?.find((child) => child.id === "if");
    const ports = ifNode?.ports ?? [];
    const truePort = ports.find((port) => port.id === "if__true");
    const falsePort = ports.find((port) => port.id === "if__false");

    // Clockwise SOUTH: index 0 = right, so visual-left "true" must get the higher index.
    expect(truePort?.properties?.["elk.port.index"]).toBe("1");
    expect(falsePort?.properties?.["elk.port.index"]).toBe("0");
  });

  it("places true-branch left of false-branch for vertical if layout", async () => {
    const workflow: CanvasesCanvas = {
      metadata: { id: "canvas-if-layout", name: "if-vertical-layout", factoryId: "factory-1" },
      spec: {
        nodes: [
          { id: "if", name: "if", type: "TYPE_ACTION", component: "control.if", position: { x: 0, y: 0 } },
          { id: "when-true", name: "true", type: "TYPE_ACTION", component: "noop", position: { x: 400, y: 300 } },
          { id: "when-false", name: "false", type: "TYPE_ACTION", component: "noop", position: { x: 0, y: 300 } },
        ],
        edges: [
          { sourceId: "if", targetId: "when-true", channel: "true" },
          { sourceId: "if", targetId: "when-false", channel: "false" },
