...
function draft(branchName: string, overrides: Partial<CanvasesCanvasDraftBranch> = {}): CanvasesCanvasDraftBranch {
  return {
    branchName,
...
describe("pickDefaultDraftBranch", () => {
  ...
  it("prefers the branch stored in localStorage when it still exists", () => {
    ...
    expect(pickDefaultDraftBranch(branches, canvasId)?.branchName).toBe("drafts/stored");
  });

  it("falls back to the current user's most recently updated draft", () => {
    const branches = [
      draft("drafts/old", { owner: { id: "user-1", name: "Ada" }, updatedAt: "2026-06-01T12:00:00.000Z" }),
      draft("drafts/new", { owner: { id: "user-1", name: "Ada" }, updatedAt: "2026-06-03T12:00:00.000Z" }),
    ...
    expect(pickDefaultDraftBranch(branches, canvasId, "user-1")?.branchName).toBe("drafts/new");
  });

  it("falls back to the most recently updated org draft", () => {
    const branches = [
      draft("drafts/old", { updatedAt: "2026-06-01T12:00:00.000Z" }),
      draft("drafts/new", { updatedAt: "2026-06-03T12:00:00.000Z" }),
    ];

    expect(pickDefaultDraftBranch(branches, canvasId)?.branchName).toBe("drafts/new");
  });
});
