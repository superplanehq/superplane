...
export function useDispatchWorkOrder(organizationId: string, factoryId: string) {
  ...
  return useMutation({
    ...
    // Do not move the card to the first phase before the server replies.
    // A queued start must stay in Backlog; an optimistic phase placement
    // would flash the card onto Implement and then jump it back. Cancel
    // in-flight list queries so a stale ListWorkOrders cannot overwrite
    // the order that onSuccess writes.
    onMutate: async () => {
      await queryClient.cancelQueries({ queryKey: workOrdersKey(organizationId, factoryId) });
    },
