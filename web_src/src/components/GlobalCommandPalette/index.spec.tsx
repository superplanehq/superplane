...
const { createCanvasMock, navigateMock } = vi.hoisted(() => ({
...
describe("GlobalCommandPalette", () => {
  ...
  it("searches canvas components through the command palette", async () => {
    const user = userEvent.setup();
    ...
    fireEvent.keyDown(document, { key: "k", metaKey: true });
    await user.click(await screen.findByText("Search Components"));
    await user.type(screen.getByPlaceholderText("Search components"), "worker");
