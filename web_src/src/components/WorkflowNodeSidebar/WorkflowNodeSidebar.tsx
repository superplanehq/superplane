...
import { WorkflowNodeEventsTab } from './WorkflowNodeEventsTab'

type Tab = 'queue' | 'executions' | 'events'
    ...
    <div className="bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 flex flex-col z-50 h-full">
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as Tab)} className="h-full flex flex-col">
          {isTriggerNode ? (
            <TabsContent value="events" className="flex-1 overflow-hidden data-[state=inactive]:hidden">
              <WorkflowNodeEventsTab workflowId={workflowId} nodeId={nodeId} />
            </TabsContent>
          ) : (
            <>
