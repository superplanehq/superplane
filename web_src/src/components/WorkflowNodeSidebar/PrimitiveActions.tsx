...
import { primitivesListPrimitiveActions, workflowsInvokeNodeExecutionAction } from '../../api-client/sdk.gen'
...
interface PrimitiveActionsProps {
  executionId: string
  primitiveName: string
  executionState: string
}

export const PrimitiveActions = ({ executionId, primitiveName, executionState }: PrimitiveActionsProps) => {
  const [selectedAction, setSelectedAction] = useState<any | null>(null)
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [actionParameters, setActionParameters] = useState<Record<string, any>>({})
  const queryClient = useQueryClient()

  // Fetch available actions for this primitive
  const { data: actionsData, isLoading } = useQuery({
    queryKey: ['primitive-actions', primitiveName],
    queryFn: async () => {
      const response = await primitivesListPrimitiveActions(
