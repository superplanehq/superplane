...
export function BannerWidget({ variant, content }: BannerWidgetProps) {
  ...
  return (
    <div
      className={cn(
        "my-2 flex items-start gap-2 rounded-lg px-3 py-2 text-sm",
        isSuccess ? "bg-green-50 border border-green-200 text-green-900" : "bg-red-50 border border-red-200 text-red-900",
      )}
    >
      {isSuccess ? (
