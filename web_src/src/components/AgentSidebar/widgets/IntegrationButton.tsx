...
export function IntegrationButton({ integrationRef, label }: IntegrationButtonProps) {
  ...
  return (
    ...
    >
      ...
      {isVendorOnly && <span className="shrink-0 text-[10px] font-bold leading-none text-content-muted">+</span>}
    </button>
  );
}
...
function toneClassName(tone: "connected" | "pending" | "error" | "default"): string {
  switch (tone) {
    ...
    default:
      return "border-edge-default text-content-secondary hover:border-edge-strong hover:bg-action-neutral-hover";
  }
}
