...
function XYChartWidget({ config }: { config: ChartConfig }) {
  ...
  if (!x || !series?.length) {
    return <div className="my-2 text-xs text-content-muted">Chart: missing data</div>;
  }
  ...
  return (
    <div className="my-4 w-full min-w-0 rounded-lg border border-edge-default bg-surface-raised p-3">
      {title && <p className="mb-2 text-xs font-medium text-content-secondary">{title}</p>}
...
function PieChartWidget({ config }: { config: ChartConfig }) {
  ...
  if (!data?.length) {
    return <div className="my-2 text-xs text-content-muted">Pie chart: missing data</div>;
  }
