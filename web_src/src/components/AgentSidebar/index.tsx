...
import { useAgentChatMessages, useCanvasAgentChat, useInterruptAgentChat, useSendAgentChatMessage } from "@/hooks/useAgentChats";
...
import { isSystemNotification, formatSystemNotification, createSystemMessage } from "./systemMessages";
      ...
      fetch(`/api/v1/canvases/${canvasId}/versions/${versionId}`, {
        ...
        .then(r => {
          if (!r.ok) {
            ...
            sendMutation.mutateAsync({
              chatId,
              content: createSystemMessage(`User discarded draft version ${versionId}. Changes were NOT applied. The canvas is unchanged from the last published version.`),
              mode: agentMode,
            }).catch(() => {});
            return null;
          }
        ...
        .then(data => {
          ...
          if (state === "STATE_PUBLISHED") {
            ...
            sendMutation.mutateAsync({
              chatId,
              content: createSystemMessage(`User published draft version ${versionId}. Changes are now live. Re-read the canvas to see the current state.`),
              mode: agentMode,
            }).catch(() => {});
...
function MessageRow({
  ...
  if (message.role === "user" && isSystemNotification(message.content)) {
    const text = formatSystemNotification(message.content);
    return (
