...
export function ModeToggle({
  ...
  // Sync when parent mode changes (e.g. external restore).
  useEffect(() => {
    setDisplayMode(mode);
  }, [mode]);
  ...
  const updateIndicator = useCallback(() => {
    ...
    const button = buttonRefs.current[displayMode];
  ...
  const handleSwitch = useCallback(
    (next: AgentMode) => {
      if (disabled || streaming || next === displayMode) return;
      setDisplayMode(next);
      onSwitch(next);
    },
    [disabled, streaming, displayMode, onSwitch],
  );
