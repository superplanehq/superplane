...
function ToolGroupRow({ messages }: { messages: AgentMessage[] }) {
  ...
  const isEditing = messages.some(isCanvasEditMessage);
  const [expanded, setExpanded] = useState(hasRunning);
  const count = messages.length;
  let label: string;
  if (isEditing) {
    label = hasRunning ? "Editing canvas…" : "Edited canvas";
  } else if (hasRunning) {
    label = `Running command${count > 1 ? ` (${count})` : ""}...`;
  } else {
    label = `Ran ${count} command${count !== 1 ? "s" : ""}`;
  }
