...
export function useRunFilters({ runs, workflowNodes, componentIconMap, onStatusFiltersChange }: UseRunFiltersParams) {
  ...
  const filteredRuns = useMemo(() => {
    ...
    return decoratedRuns.filter(({ run, status, title, triggerName }) => {
      ...
      if (normalizedQuery && !`${title} ${triggerName}`.toLowerCase().includes(normalizedQuery)) return false;

      return true;
    });
  }, [decoratedRuns, selectedStatuses, selectedTriggerIds, searchQuery]);
  ...
  const hasAnyFilter = selectedTriggerIds.size > 0 || selectedStatuses.size > 0 || searchQuery.trim().length > 0;
