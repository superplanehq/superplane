...
export const createAlertMapper: ComponentBaseMapper = {
  props(context: ComponentBaseContext): ComponentBaseProps {
    ...
    const componentName = context.componentDefinition.name ?? "jira.createAlert";
...
function metadataList(node: NodeInfo): MetadataItem[] {
  const configuration = node.configuration as CreateAlertConfiguration;
  const items: MetadataItem[] = [];
  if (configuration?.message) {
    items.push({ icon: "alert-circle", label: configuration.message });
  }
  if (configuration?.priority && configuration.priority !== "__none__") {
