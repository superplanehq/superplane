...
describe("getCommitStatusMapper", () => {
  it("summarizes the retrieved commit statuses", () => {
    const details = getCommitStatusMapper.getExecutionDetails(
      buildDetailsContext({
        outputs: {
          default: [
            {
              type: "gitlab.commitStatuses",
  ...
  it("falls back to the payload timestamp when created_at is missing", () => {
          ...
          default: [
            {
              type: "gitlab.commitStatuses",
              timestamp: "2026-02-13T18:04:12.000Z",
              data: [{ id: 93, sha: "18f3e63d05582537", status: "success" }],
            },
          ],
        },
      }),
    ...
    expect(details["Retrieved At"]).not.toBe("-");
