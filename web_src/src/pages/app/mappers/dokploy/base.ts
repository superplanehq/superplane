...
function metadataList(node: NodeInfo): MetadataItem[] {
  ...
  const configuration = (node.configuration as DokployConfiguration | undefined) ?? {};

  const application = getConfigValue(configuration.application);
  const title = getConfigValue(configuration.title);

  if (application) {
    metadata.push({ icon: "box", label: `Application: ${application}` });
  }
  if (title) {
    metadata.push({ icon: "tag", label: `Title: ${title}` });
  }
...
// Currently unused, but kept for future per-component expansion.
export type DokployExecution = ExecutionInfo;

export const dokployBaseMapper: ComponentBaseMapper = {
  ...noopMapper,
  props,
  getExecutionDetails,
  subtitle,
