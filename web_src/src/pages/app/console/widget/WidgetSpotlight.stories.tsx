...
/**
 * Prototype `spotlight` panel renderer — a single record blown up into a hero
 * banner. The slots are generic (who, what + link, when + duration, a labeled
 * secondary person, and a strip of status "checks"), so the same panel serves
 * very different SuperPlane apps. These stories are grounded in two real apps:
 * the "SuperPlane SaaS" delivery pipeline (deploy stages become the checks) and
 * the "Docs Reviewer" PR agent (its review steps become the checks). This is a
 * mockup: it takes already-resolved props and is not yet wired into
 * `panelTypes.ts`, the panel router, or the backend.
 */
const meta = {
  title: "Console/Spotlight (prototype)",
