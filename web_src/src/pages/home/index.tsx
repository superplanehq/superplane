...
const createUserDisplayNames = (orgUsers: UserData[]) => {
  ...
  orgUsers.forEach((user) => {
    if (user.metadata?.id) {
      const name =
        user.spec?.displayName || user.metadata?.email || user.metadata.id;
      const initials = name
        .split(" ")
        .map((n) => n[0])
        .join("")
...
const HomePage = () => {
  ...
  const [showCreateBlueprintModal, setShowCreateBlueprintModal] =
    useState(false);
  ...
  const { data: orgUsers = [], isLoading: usersLoading } = useOrganizationUsers(
    organizationId || ""
  );
  const {
    data: blueprintsData = [],
  ...
  const canvasError = canvasApiError
    ? "Failed to fetch canvases. Please try again later."
    : null;
  const blueprintError = blueprintApiError
    ? "Failed to fetch custom components. Please try again later."
    : null;
  ...
  const canvases: CanvasCardData[] = canvasesData.map(
  ...
  const blueprints: BlueprintCardData[] = (blueprintsData || []).map(
    (blueprint: any) => ({
      id: blueprint.id!,
      name: blueprint.name!,
  ...
  const workflows: WorkflowCardData[] = (workflowsData || []).map(
    (workflow: any) => ({
      id: workflow.id!,
      name: workflow.name!,
  ...
  const handleCreateCanvasSubmit = async (data: {
    name: string;
    description?: string;
  }) => {
    if (organizationId) {
      ...
      if (result) {
        setShowCreateCanvasModal(false);
        navigate(
          `/${organizationId}/canvas/${result.data?.canvas?.metadata?.id}`
        );
      }
    }
  };

  const handleCreateBlueprintSubmit = async (data: {
    name: string;
    description?: string;
  }) => {
    if (organizationId) {
      ...
      if (result?.data?.blueprint?.id) {
        setShowCreateBlueprintModal(false);
        navigate(
          `/${organizationId}/custom-components/${result.data.blueprint.id}`
        );
      }
    }
  };

  const handleCreateWorkflowSubmit = async (data: {
    name: string;
    description?: string;
  }) => {
    if (organizationId) {
              ...
              <>
                              ...
                              <div>
                                <div className="flex items-center mb-4">
                                  <div className="flex items-center justify-between space-x-3 flex-1">
                                    <IconComponent
                                      size={24}
                                      className={getColorClass(blueprint.color)}
                                    />
                                    <div className="flex flex-col flex-1 min-w-0">
                                      <button
                                        onClick={() =>
                                          navigate(
                                            `/${organizationId}/custom-components/${blueprint.id}`
                                          )
                                        }
                              ...
                              <div className="flex items-start justify-between space-x-3 flex-1">
                                <div className="flex flex-col flex-1 min-w-0">
                                  <button
                                    onClick={() =>
                                      navigate(
                                        `/${organizationId}/workflows/${workflow.id}`
                                      )
                                    }
                                    className="block text-left w-full"
                                  >
                ...
                {currentItems.length === 0 && (
                  <div className="text-center py-12">
                    {activeTab === "canvases" && (
                      <Bot className="mx-auto text-zinc-400 mb-4" size={48} />
                    )}
                    {activeTab === "blueprints" && (
                    ...
                    {activeTab === "workflows" && (
                      <GitBranch
                        className="mx-auto text-zinc-400 mb-4"
                        size={48}
                      />
                    )}
                    <Heading
                      level={3}
                      className="text-lg text-zinc-900 dark:text-white mb-2"
                    >
                      {searchQuery
                        ? `No ${activeTab} found`
                        : `No ${activeTab} yet`}
