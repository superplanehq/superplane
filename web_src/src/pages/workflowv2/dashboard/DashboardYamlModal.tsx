...
export function DashboardYamlModal({
  ...
  const editor = useDashboardYamlEditor({ open, exportedYaml, filename, onImport, onOpenChange });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="large" className="flex max-h-[90vh] w-[90vw] h-full flex-col gap-0 overflow-hidden p-0">
        ...
        <DashboardYamlEditor text={editor.text} readOnly={!onImport} onChange={editor.handleEditorChange} />

        <DashboardYamlError message={editor.parseError} />
...
function useDashboardYamlEditor({
  open,
  exportedYaml,
  filename,
  onImport,
  ...
  return (
    <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button variant="outline" size="sm" onClick={onCopy} data-testid="dashboard-yaml-copy">
          <Copy className="mr-1 h-3.5 w-3.5" />
          Copy
        </Button>
        <Button variant="outline" size="sm" onClick={onDownload} data-testid="dashboard-yaml-download">
...
function DashboardYamlError({ message }: { message: string | null }) {
...
function DashboardYamlFooter({
  ...
  return (
    <DialogFooter className="border-t border-slate-200 px-4 py-3">
      ...
      {canImport ? (
        <Button onClick={onApply} disabled={!isDirty || isImporting} data-testid="dashboard-yaml-apply">
...
function ReplaceDashboardDialog({
  open,
  isImporting,
  onOpenChange,
  onConfirm,
