 ...
 *
 * Filter expressions accept the same dialects as row `show` clauses:
 * CEL `{{ ... }}`, the legacy `field == "value"` shorthand, and the
 * dashboard mini-expression language. Routing through `evaluateRowShow`
 * keeps `render.filters` consistent with row-level visibility evaluation.
 */
