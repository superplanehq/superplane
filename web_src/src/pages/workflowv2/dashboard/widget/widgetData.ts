 ...
 *
 * Filter expressions go through the shared row-show dispatcher so authors can
 * mix the mini language (`row.status == "failed"`), the legacy shorthand
 * (`status == "failed"`), CEL templates (`{{ … }}`), and bare CEL expressions
 * with function calls and arithmetic (e.g. `int(started_at) >= now() - 604800`).
 */
