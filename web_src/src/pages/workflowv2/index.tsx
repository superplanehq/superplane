...
export function WorkflowPageV2() {
  ...
  const createChangeRequestVersion = useMemo(() => {
    if (activeCanvasVersionId && selectedCanvasVersion && !selectedCanvasVersion.metadata?.isPublished) {
  ...
  const hasEditableVersion =
    ...
    (isVersioningDisabled && !activeCanvasVersionId);
  ...
  useEffect(() => {
    ...
    const canvasNameForTitle = canvas?.metadata?.name || "Canvas";
    setCreateChangeRequestTitle(`Update ${canvasNameForTitle}`);
    setCreateChangeRequestDescription("");
    setCreateChangeRequestDescriptionMode("write");
  }, [isCreateChangeRequestMode, canvas?.metadata?.name]);
  ...
  const handleCreateVersion = useCallback(async () => {
    ...
    if (isVersioningDisabled) {
      showErrorToast("Versioning is disabled.");
      return;
    }

    if (hasEditableVersion && hasUnsavedChanges) {
      const shouldCreate = window.confirm("You have unsaved changes. Create a new draft anyway?");
      if (!shouldCreate) {
        return;
      }
    }
  ...
  const handleSaveWorkflow = useCallback(
    async (workflowToSave?: CanvasesCanvas, options?: { showToast?: boolean }) => {
      ...
      if (!activeCanvasVersionId && !isVersioningDisabled) {
        if (options?.showToast !== false) {
          showErrorToast("Enable edit mode first.");
        }
        return;
      }
  ...
  const versioningDisabledTooltip = "Versioning is disabled.";
  ...
  const runDisabledTooltip = canvasDeletedRemotely
    ? "Canvas was deleted."
    : isViewingDraftVersion
      ? "Publish draft to run this canvas."
      : !isViewingCurrentLiveVersion
        ? "Only the live version can execute."
        : !canUpdateCanvas
          ? "No permission to emit events."
          : isTemplate
