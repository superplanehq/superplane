...
export function CreateChangeRequestModal({
  ...
  const handlePublish = () => {
    if (isDraftOutdated) {
      const confirmed = window.confirm("This draft is outdated. Create anyway?");
      if (!confirmed) {
        return;
      }
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="min-w-[70vw] max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Snapshot your current draft as a change request.</DialogDescription>
        </DialogHeader>

        {!version ? (
          <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">
            Save your draft before creating a change request.
          </div>
        ) : (
