  ...
  return (
    <div className="min-h-0 w-full min-w-0 flex-1 overflow-auto">
      {Object.entries(groupedEntries).map(([namespace, values]) => (
        <div key={namespace} className="m-4 rounded-md border border-slate-300 bg-white">
          <div className="border-b border-slate-300 px-3 py-2 font-mono text-sm text-gray-600">
            Namespace: {namespace}
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-950/15 bg-slate-50">
              {columns.map((column) => (
                <th key={column} className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-600">
                  {column}
                </th>
              ))}
              <th className="w-12 px-3 py-2 text-right text-xs font-semibold uppercase text-gray-600"></th>
            </tr>
          </thead>
          <tbody>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-950/15 bg-slate-50">
            <th className="px-3 py-2 text-left text-xs font-semibold uppercase text-gray-600">Value</th>
            <th className="w-12 px-3 py-2 text-right text-xs font-semibold uppercase text-gray-600"></th>
          </tr>
        </thead>
        <tbody>
