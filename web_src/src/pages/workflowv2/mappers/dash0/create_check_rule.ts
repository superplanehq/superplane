...
export const createCheckRuleMapper: ComponentBaseMapper = {
  props(context: ComponentBaseContext): ComponentBaseProps {
    ...
    return {
      ...
      title: context.node.name || context.componentDefinition.label || "Create Check Rule",
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, string> {
    ...
    if (!payload?.data) {
      details["Create Response"] = "No data returned";
      return details;
    }

    try {
      details["Create Response"] = JSON.stringify(payload.data, null, 2);
    } catch {
      details["Create Response"] = String(payload.data);
    }
