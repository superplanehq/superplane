...
interface DeleteObjectConfiguration {
  region?: string;
  bucket?: string;
  key?: string;
}
...
function deleteObjectMetadataList(node: NodeInfo): MetadataItem[] {
  const metadata: MetadataItem[] = [];
  const configuration = node.configuration as DeleteObjectConfiguration | undefined;
