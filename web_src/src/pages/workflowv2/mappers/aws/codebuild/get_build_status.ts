...
interface GetBuildStatusConfiguration {
  region?: string;
  buildId?: string;
}

interface GetBuildStatusOutput {
  id?: string;
  arn?: string;
  buildNumber?: number;
  buildStatus?: string;
...
export const getBuildStatusMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, string> {
    ...
    const result = outputs?.default?.[0]?.data as GetBuildStatusOutput | undefined;
    ...
    if (result) {
      ...
      if (result.buildNumber) {
        details["Build Number"] = String(result.buildNumber);
      }
    }
...
function getMetadataList(node: NodeInfo): MetadataItem[] {
  ...
  const configuration = node.configuration as GetBuildStatusConfiguration | undefined;

  if (configuration?.buildId) {
    metadata.push({ icon: "file-text", label: configuration.buildId });
  }
