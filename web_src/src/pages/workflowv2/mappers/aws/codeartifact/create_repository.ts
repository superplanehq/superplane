...
interface CreateRepositoryConfiguration {
  domain?: string;
  repository?: string;
}
...
export const createRepositoryMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, string> {
    ...
    const createdAt =
      repo.createdTime != null ? formatTimestampInUserTimezone(new Date(repo.createdTime * 1000).toISOString()) : "-";
...
function createRepositoryMetadataList(node: NodeInfo): MetadataItem[] {
  const config = node.configuration as CreateRepositoryConfiguration | undefined;
