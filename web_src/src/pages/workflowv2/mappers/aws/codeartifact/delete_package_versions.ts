...
interface DeletePackageVersionsPayload {
...
export const deletePackageVersionsMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, string> {
    ...
    const data = outputs?.default?.[0]?.data as DeletePackageVersionsPayload | undefined;
    if (!data) return {};
    const successful = data.successfulVersions ?? {};
    const failed = data.failedVersions ?? {};
    return {
      "Successful deletes": String(Object.keys(successful).length),
...
function deletePackageVersionsMetadataList(node: NodeInfo): MetadataItem[] {
  const config = node.configuration as DeletePackageVersionsConfiguration | undefined;
  const items: MetadataItem[] = [];
  if (config?.domain) items.push({ icon: "database", label: config.domain });
  if (config?.repository) items.push({ icon: "boxes", label: config.repository });
  if (config?.package) items.push({ icon: "package", label: config.package });
  return items;
}
