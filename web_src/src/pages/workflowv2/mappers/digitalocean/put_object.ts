...
import type { PutObjectConfiguration, PutObjectOutput } from "./types";
import { DEFAULT_STATE_REGISTRY } from "../stateRegistry";
import type { EventStateRegistry } from "../types";

type PutObjectOutputs = {
  default?: OutputPayload[];
};

export const PUT_OBJECT_STATE_REGISTRY: EventStateRegistry = DEFAULT_STATE_REGISTRY;
...
export const putObjectMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, unknown> {
    ...
    const outputs = context.execution.outputs as PutObjectOutputs | undefined;
...
function metadataList(node: NodeInfo): MetadataItem[] {
  ...
  const configuration = node.configuration as PutObjectConfiguration;
  ...
  if (configuration?.filePath) {
    metadata.push({ icon: "file-up", label: configuration.filePath });
  }
