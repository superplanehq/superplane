...
export const baseMapper: ComponentBaseMapper = {
  props(
    ...
    const componentName = componentDefinition.name ?? "openai";

    return {
      iconSrc: openAiIcon,
      iconSlug: componentDefinition?.icon ?? "sparkles",
      collapsedBackground: "bg-white",
      collapsed: node.isCollapsed,
      title: node.name || componentDefinition?.label || componentDefinition?.name || "OpenAI",
