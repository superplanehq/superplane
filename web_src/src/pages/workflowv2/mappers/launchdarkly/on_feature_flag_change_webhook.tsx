...
interface OnFeatureFlagChangeConfig {
  events?: string[];
...
const SetSigningSecretSection: React.FC<{ nodeId: string }> = ({ nodeId }) => {
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="ld-signing-secret" className="text-xs font-medium text-gray-700 dark:text-gray-300">
        Webhook signing secret
      </Label>
      <div className="flex items-center gap-2">
        <Input
          id="ld-signing-secret"
          type="password"
          autoComplete="off"
          placeholder="Paste signing secret from LaunchDarkly"
          value={secret}
...
export const onFeatureFlagChangeCustomFieldRenderer: CustomFieldRenderer = {
  render: (node: NodeInfo) => {
    const config = node.configuration as OnFeatureFlagChangeConfig | undefined;
      ...
      return (
        <div className="border-t-1 border-gray-200 dark:border-gray-700 pt-4">
          <div className="text-xs text-gray-600 dark:text-gray-400 rounded-md border border-gray-200 dark:border-gray-600 px-3 py-2.5 bg-gray-50 dark:bg-gray-800/50">
            <p className="font-medium text-gray-700 dark:text-gray-300 mb-1">LaunchDarkly webhook is configured</p>
            <p>
              For security, the webhook URL and signing secret are not shown. To use a different URL or secret, add a
              new <strong>On Feature Flag Change</strong> trigger and configure it there.
            </p>
          </div>
        </div>
      );
    ...
    return (
      <div className="border-t-1 border-gray-200 dark:border-gray-700 pt-4">
        <div className="space-y-3">
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">LaunchDarkly Webhook Setup</span>
          <div className="text-xs text-gray-800 dark:text-gray-100 border-1 border-gray-300 dark:border-gray-600 px-2.5 py-2 bg-gray-50 dark:bg-gray-800 rounded-md space-y-2">
            <div className="flex items-center justify-between gap-2">
              <span className="font-medium text-gray-700 dark:text-gray-200">URL for LaunchDarkly</span>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              In LaunchDarkly go to <strong>Integrations &rarr; Webhooks</strong>, add a webhook with this URL. Check{" "}
              <strong>Sign this webhook</strong> and copy the generated secret. Then use{" "}
              <strong>Set signing secret</strong> below to store it securely.
            </p>
          </div>
          ...
          <div className="rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 px-3 py-2.5">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300">
              Save the canvas to generate the webhook URL, add it in LaunchDarkly with signing enabled, then use{" "}
              <strong>Set signing secret</strong> above. Until then, no events will be received.
            </p>
          </div>
        </div>
      </div>
