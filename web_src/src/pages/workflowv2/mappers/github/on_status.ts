...
export const onStatusTriggerRenderer: TriggerRenderer = {
  ...
  getRootEventValues: (context: TriggerEventContext): Record<string, string> => {
    ...
    return {
      State: stringOrDash(eventData?.state),
      Context: stringOrDash(eventData?.context),
      SHA: stringOrDash(eventData?.sha || eventData?.commit?.sha),
      Description: stringOrDash(eventData?.description),
      "Target URL": stringOrDash(eventData?.target_url),
      Branches: stringOrDash(statusBranchNames(eventData).join(", ")),
      Repository: stringOrDash(eventData?.repository?.full_name),
      Sender: stringOrDash(eventData?.sender?.login),
      "Commit URL": stringOrDash(eventData?.commit?.html_url),
    };
  },
...
function statusTitle(eventData: OnStatusEventData | undefined): string {
  const context = eventData?.context || "Commit status";
  const sha = shortSha(eventData?.sha || eventData?.commit?.sha);
