...
interface CreateCaseConfiguration {
  title?: string;
  severity?: string;
  owner?: string;
}

interface CreateCaseOutputData {
  id?: string;
  title?: string;
  status?: string;
  severity?: string;
...
export const createCaseMapper: ComponentBaseMapper = {
...
function metadataList(node: NodeInfo): MetadataItem[] {
  ...
  const configuration = node.configuration as CreateCaseConfiguration | undefined;

  if (configuration?.title) {
    metadata.push({ icon: "file-text", label: configuration.title });
  }
