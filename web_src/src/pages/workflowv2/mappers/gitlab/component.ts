...
function metadataList(node: ComponentsNode): MetadataItem[] {
  ...
  if (nodeMetadata?.project?.path_with_namespace) {
    metadata.push({ icon: "book", label: nodeMetadata.project.path_with_namespace });
  }
  ...
  return [
    {
      ...
      eventSubtitle: execution.metadata?.customMessage || "Executed",
