...
export const sendEmailMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(execution: WorkflowsWorkflowNodeExecution, _node: ComponentsNode): Record<string, string> {
    ...
    return {
      To: stringOrDash(Array.isArray(data?.to) ? data.to.join(", ") : data?.to),
      Subject: stringOrDash(data?.subject),
      "Sent At": stringOrDash(data?.sentAt),
      From: stringOrDash(data?.fromEmail),
    };
  },
};
...
function sendEmailSpecs(node: ComponentsNode): ComponentBaseSpec[] {
  ...
  const subject = nodeMetadata?.subject || configuration?.subject;
  if (subject) {
    specs.push({
      title: "subject",
