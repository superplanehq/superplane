...
import { LokiLogEntry } from "./types";
...
export const pushLogsMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, string> {
    ...
    const entry = outputs.default[0].data as LokiLogEntry;
...
function metadataList(node: NodeInfo): MetadataItem[] {
  ...
  if (configuration?.labels) {
    metadata.push({ icon: "tag", label: configuration.labels });
  }
...
function getDetailsForEntry(entry: LokiLogEntry): Record<string, string> {
