...
export const onIssueEventTriggerRenderer: TriggerRenderer = {
  ...
  getRootEventValues: (context: TriggerEventContext): Record<string, string> => {
    ...
    return {
      ...
      Events: issue?.count || "",
      Users: issue?.userCount != null ? issue.userCount.toString() : "",
  ...
  getTriggerProps: (context: TriggerRendererContext) => {
    ...
    // Show project from metadata or configuration
    const projectName = metadata?.project || configuration?.project;
    if (projectName) {
      metadataItems.push({
        icon: "book",
        label: projectName,
    ...
    // Show event types filter
    const eventTypes = metadata?.eventTypes || configuration?.eventTypes;
