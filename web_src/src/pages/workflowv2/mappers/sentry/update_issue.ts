...
export const updateIssueMapper: ComponentBaseMapper = {
  ...
  getExecutionDetails(context: ExecutionDetailsContext): Record<string, any> {
    ...
    addDetail("Title", payload.title);
    addDetail("Issue ID", payload.id);
    addDetail("Short ID", payload.shortId);
    addDetail("Culprit", payload.culprit);
    addDetail("Level", payload.level);
    addDetail("Status", payload.status);
    addDetail("Project", payload.project?.name);
    addDetail("Events", payload.count);
    addDetail("Users", payload.userCount != null ? payload.userCount.toString() : undefined);
    ...
    if (payload.firstSeen) {
      addDetail("First Seen", new Date(payload.firstSeen).toLocaleString());
    }

    if (payload.lastSeen) {
      addDetail("Last Seen", new Date(payload.lastSeen).toLocaleString());
    }
