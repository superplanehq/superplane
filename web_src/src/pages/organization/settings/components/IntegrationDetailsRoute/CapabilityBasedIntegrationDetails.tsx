...
export function CapabilityBasedIntegrationDetails({ organizationId, integration }: CapabilityBasedIntegrationDetailsProps) {
  ...
  const handleCapabilitiesSubmit = async () => {
    ...
    try {
      ...
      if (providerName && updated?.status?.setupState?.currentStep && resolvedIntegrationId) {
        navigate(`/${organizationId}/settings/integrations/${resolvedIntegrationId}/setup`, {
