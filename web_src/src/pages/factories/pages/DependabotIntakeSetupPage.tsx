...
export function DependabotIntakeSetupPage() {
  const layout = useFactoriesLayout();
  const { canAct } = usePermissions();
  const { lineId } = useParams<{ lineId?: string }>();
  const navigate = useNavigate();
  const model = resolveDependabotIntakeSetupModel(layout, canAct("factories", "update"), lineId);
  ...
  return (
    <div className="h-full min-h-0" data-testid="dependabot-intake-setup-page">
