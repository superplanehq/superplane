...
import { createNewColumnAutomationCanvas } from "./createNewColumnAutomation";
...
export function LinesPage() {
  ...
  const canCreateAutomation = canAct("canvases", "create");
...
function LineDetail({
  ...
  const automationsFor = useCallback(
    (key: ColumnKey, columnTitle: string) => {
      const base = buildColumnAutomations(key, {
        ...
        apps,
        workOrders,
        columnAutomations: line.columnAutomations,
      });
      return applyColumnAutomationsOverlay(base, overlay.extra[key], overlay.disabledIds, overlay.removedIds);
    },
    [apps, board, factoryIntakes, line.columnAutomations, overlay, prFeedbackHandlers, workOrders],
  );
  ...
  const handleRowAction = (automation: ColumnAutomation, action: ColumnAutomationRowAction) => {
    ...
    if (action === "remove") {
      if (automation.kind === "custom" && automation.canvasId && line.id && automationsColumnKey) {
        void updateLine
          .mutateAsync({
            lineId: line.id,
            columnAutomations: removeColumnAutomation(
              line.columnAutomations,
              automationsColumnKey,
              automation.canvasId,
            ),
          })
          .catch((error) => {
            showErrorToast(getApiErrorMessage(error, "Failed to remove automation"));
          });
        return;
      }
  ...
  const handleAddAutomation = async () => {
    if (!canCreateAutomation || creatingAutomation || !automationsColumnKey || !line.id) {
      return;
    }
    setCreatingAutomation(true);
    try {
      const canvasId = await createNewColumnAutomationCanvas({
        factoryId,
        existingNames: apps.map((app) => app.name ?? ""),
        createCanvas: createCanvas.mutateAsync,
      });
      await updateLine.mutateAsync({
        lineId: line.id,
        columnAutomations: addColumnAutomation(line.columnAutomations, automationsColumnKey, canvasId),
      });
      navigate(
    ...
    <div className="flex min-h-0 flex-1 flex-col" data-testid="lines-detail">
      {steps.length === 0 && backlogOrders.length === 0 && doneOrders.length === 0 ? (
        ...
        <PhaseBoard
          ...
          onAddAutomation={() => void handleAddAutomation()}
          onAutomationRowAction={handleRowAction}
          addAutomationDisabled={!canCreateAutomation || creatingAutomation}
        />
      )}
