...
describe("intakeConnectionModel", () => {
  ...
  it("returns health banners for repair states", () => {
    ...
    expect(intakeHealthBanner("HEALTH_WEBHOOK_NOT_READY")).toBe("SuperPlane is still registering the webhook.");
    expect(intakeHealthBanner("HEALTH_WEBHOOK_FAILED")).toBe(
      "SuperPlane could not register the webhook. Save the intake again to retry.",
    );
