...
describe("Line board job popup", () => {
  ...
  it("matches popup content to the card state", async () => {
    ...
    expect(waitingNote).toHaveAttribute("data-variant", "pull-request");
    expect(within(waitingNote).queryByRole("button", { name: "More actions" })).not.toBeInTheDocument();
    expect(within(waitingNote).queryByRole("button", { name: "To Backlog" })).not.toBeInTheDocument();
    expect(within(waitingNote).queryByRole("button", { name: "Reject" })).not.toBeInTheDocument();
