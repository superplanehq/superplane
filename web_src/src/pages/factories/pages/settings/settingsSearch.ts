...
export const FACTORY_SETTINGS_SEARCH_ENTRIES: FactorySettingsSearchResult[] = [
  ...
  // Workspace · General
  entry("workspace", "general", "General", "factory-settings-general-form", [
    "name",
    "workspace key",
    "description",
  ]),
  ...
  entry("organization", "integrations", "Integrations", undefined, [
    "connect",
    "filter integrations",
    "integration",
  ]),
...
export function searchFactorySettings(
  ...
  return dropRedundantPageHits(scored.map((row) => row.result));
}
 ...
 */
function dropRedundantPageHits(results: FactorySettingsSearchResult[]): FactorySettingsSearchResult[] {
  ...
  return results.filter((result) => {
    ...
    return !pagesWithSectionHits.has(`${result.scope}/${result.section}`);
  });
}
