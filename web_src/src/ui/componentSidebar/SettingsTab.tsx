...
/**
 * True when `name` still looks like a node that was created with its default name
 * (the raw component key, possibly with the unique-name ordinal suffix generated by
 * `generateUniqueNodeName`, e.g. "github.onPullRequest" or "github.onPullRequest 2").
 * When no canonical component key is available we cannot prove a rename, so we treat
 * the node as default-named.
 */
function isGeneratedDefaultName(name: string, componentKey?: string): boolean {
  if (!componentKey) return true;
  const trimmed = name.trim();
  if (trimmed === componentKey) return true;
  const escapedKey = componentKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return new RegExp(`^${escapedKey}\\s+\\d+$`).test(trimmed);
}
...
interface SettingsTabProps {
  ...
  nodeName: string;
  nodeLabel?: string;
  /** Canonical component key (e.g. "github.onPullRequest"); used to detect a genuine rename. */
  blockName?: string;
...
export function SettingsTab({
  ...
  // The component's original display name (from the component definition), shown as secondary
  // metadata only when the node has genuinely been renamed away from its default component name.
  // Hidden when the name is still the generated/default name, when the label is missing, or when
  // the custom name already equals the human-readable label (no redundant/duplicate text).
  const originalComponentName = useMemo(() => {
    const label = nodeLabel?.trim();
    if (!label) return undefined;
    if (label.toLowerCase() === currentNodeName.trim().toLowerCase()) return undefined;
    if (isGeneratedDefaultName(currentNodeName, blockName)) return undefined;
    return label;
  }, [nodeLabel, currentNodeName, blockName]);
      ...
      <div className="overflow-y-auto p-4 pb-24" style={{ maxHeight: "80vh" }}>
        <div className="space-y-6">
          {originalComponentName && (
            <p className="text-xs text-gray-500" data-testid="original-component-name">
