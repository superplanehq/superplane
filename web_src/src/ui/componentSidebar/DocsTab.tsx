...
function ConfigTable({ fields, showTopBorder }: { fields: ConfigurationField[]; showTopBorder?: boolean }) {
  return (
    <div className={cn("w-full px-4 pt-3 pb-3", showTopBorder && "border-t border-edge-default")}>
      <span className="text-[13px] font-medium text-content-secondary">Configuration</span>
      <div className="overflow-x-auto mt-2">
        <table className="text-xs w-full border-collapse">
          <thead>
            <tr>
              <th className="text-left py-1.5 px-2 border-b border-edge-subtle font-medium text-content-secondary bg-surface-default">
                Field
              </th>
              <th className="text-left py-1.5 px-2 border-b border-edge-subtle font-medium text-content-secondary bg-surface-default">
                Type
          ...
          <tbody>
            {fields.map((field) => (
              <tr key={field.name}>
                <td className="py-1.5 px-2 border-b border-edge-subtle font-mono text-content-secondary">
                  {field.label || field.name}
                  {field.required && <span className="text-red-400 ml-0.5 dark:text-red-400">*</span>}
                </td>
                <td className="py-1.5 px-2 border-b border-edge-subtle text-content-secondary">
                  {field.type || "string"}
                </td>
                <td className="py-1.5 px-2 border-b border-edge-subtle text-content-secondary">
                  {field.description || "—"}
...
function DocsReferenceSection({ documentationUrl }: { documentationUrl: string }) {
  return (
    <div className="bg-surface-subtle px-4 py-2.5">
...
function DocsDescriptionSection({ description, showBottomBorder }: { description: string; showBottomBorder: boolean }) {
  return (
    <div className={cn("w-full px-4 pt-3 pb-3", showBottomBorder && "border-b border-edge-default")}>
      <span className="text-[13px] font-medium text-content-secondary">Description</span>
      <p className="text-[13px] text-content-primary mt-1 leading-relaxed">{description}</p>
    </div>
  );
}
...
function DocsPayloadSection({
  ...
  return (
    <div className={cn("w-full px-2 py-2", showBottomBorder && "border-b border-edge-default")}>
      <div className="px-2">
        <PayloadPreview
  ...
  if (!hasContent) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
        <p className="text-sm text-content-secondary">No documentation available for this component.</p>
      </div>
    );
  }
