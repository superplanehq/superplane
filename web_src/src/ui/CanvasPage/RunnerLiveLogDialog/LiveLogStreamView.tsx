...
export function LiveLogStreamView({ execution }: { execution: ExecutionInfo }) {
  ...
  return (
    <div ref={scrollRef} className="h-full min-h-0 overflow-y-auto bg-surface-default">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isLast && "sticky top-0 z-10 border-b border-edge-subtle bg-surface-default",
      )}
      onClick={() => onToggle(section.index)}
    >
      <div className="flex items-center gap-2">
        <span>{isCollapsed ? openChevron : closedChevron}</span>
        <span className="font-medium text-content-primary">{section.text}</span>
      </div>
...
function CommandSectionContent({ section }: { section: CommandSection }) {
  ...
  return (
    <pre className="px-4 py-2 text-left font-mono text-xs leading-relaxed whitespace-pre-wrap text-content-primary bg-surface-raised border-t border-edge-subtle">
