...
describe("runner live log state", () => {
  ...
  it("keeps reconnecting while an execution is still in flight", () => {
    expect(shouldReconnectLiveLogSession(true)).toBe(true);
  });

  it("stops reconnecting when an execution is terminal", () => {
    expect(shouldReconnectLiveLogSession(false)).toBe(false);
  });
});
