...
function editingNavClassName(tone: DraftEditTabTone): string {
  if (tone === "uncommitted") return "bg-orange-200 dark:bg-orange-900/55";
  if (tone === "ready") return "bg-blue-50 dark:bg-blue-950/40";
  return "bg-slate-100 dark:bg-gray-800";
}

/** Edit-mode inactive tab text — light tints match draft badges; dark mode keeps orange/blue cues. */
function editingInactiveClassName(tone: DraftEditTabTone): string {
  ...
  if (tone === "ready")
    return "bg-transparent text-blue-800/80 hover:text-blue-900 transition-none dark:text-blue-300/80 dark:hover:text-blue-200";
...
function editingActiveClassName(tone: DraftEditTabTone): string {
  if (tone === "uncommitted")
    return "rounded-full bg-orange-100 text-orange-950 dark:bg-orange-700/60 dark:text-orange-50 dark:shadow-none";
  if (tone === "ready")
    return "rounded-full bg-blue-100 text-blue-950 dark:bg-blue-700/60 dark:text-blue-50 dark:shadow-none";
  return "rounded-full bg-white text-slate-900 shadow-sm dark:bg-gray-700 dark:text-gray-100 dark:shadow-none";
}

function editModeTabTone(editing: boolean, editTabTone: DraftEditTabTone): DraftEditTabTone {
  return editing ? "uncommitted" : editTabTone;
}
...
export function CanvasModeToggle({
  ...
  const tabTone = editModeTabTone(editing, editTabTone);
  ...
  return (
    <nav
      ...
      className={cn(
        ...
        editing ? editingNavClassName(tabTone) : "bg-slate-100 dark:bg-gray-800",
      )}
    >
      <Link
        to={tabHref()}
        onClick={(e) => handleTabClick(e, selected === CANVAS_TAB, () => void onSelectLive())}
        className={tabClasses(selected, CANVAS_TAB, editing, tabTone)}
      ...
      {showConsole ? (
        <Link
          ...
          className={tabClasses(selected, CONSOLE_TAB, editing, tabTone)}
      ...
      {showMemory ? (
        <Link
          ...
          className={tabClasses(selected, MEMORY_TAB, editing, tabTone)}
      ...
      {showFiles ? (
        <Link
          ...
          className={tabClasses(selected, FILES_TAB, editing, tabTone)}
