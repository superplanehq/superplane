  ...
  return (
    <div
      ...
      className={
        ...
        (index < totalSections - 1 ? " border-b border-slate-950/25" : "") +
        ` ${className}`
      }
    >
...
export const ComponentBase: React.FC<ComponentBaseProps> = ({
  ...
  return (
    <SelectionWrapper selected={selected}>
      <div
        className={`group relative flex flex-col outline-1 outline-slate-950/25 rounded-md w-[23rem] bg-white ${hasError ? "!outline-orange-500" : ""}`}
        ...
        {isCompactView ? null : (
          <>
            ...
            {specs && specs.length > 0 && (
              <div className="px-2 py-1.5 border-b border-slate-950/25 text-gray-500 flex flex-col gap-1.5">
            ...
            {eventSections?.map((section, index) => (
              <EventSectionDisplay
                className={"pb-3" + (!!includeEmptyState || !!customField ? " border-b border-slate-950/25" : "")}
                key={index}
                section={section}
                index={index}
