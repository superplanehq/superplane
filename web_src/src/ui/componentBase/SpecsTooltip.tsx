  ...
  return (
    <Tippy
      render={(attrs) => (
        <div
          {...attrs}
          className="bg-white dark:bg-slate-800 outline-1 outline-slate-300 dark:outline-slate-600 shadow-md rounded-md max-w-[800px]"
          style={{ zIndex: 10000 }}
        >
          ...
          {specValues.map((value, index) => (
            <div
              key={index}
              className={`flex flex-wrap max-w-[800px] items-center gap-2 p-2 ${index === specValues.length - 1 ? "border-b-0" : "border-b border-slate-300 dark:border-slate-600"}`}
            >
