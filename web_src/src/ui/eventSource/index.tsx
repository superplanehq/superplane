...
export const EventSource: React.FC<EventSourceProps> = ({
  ...
  if (collapsed) {
    return (
      <div className={cn("flex w-fit flex-col items-center", className)}>
        ...
        >
          {badgeImageSrc ? (
            <img src={badgeImageSrc} alt={badgeImageAlt ?? ""} className="h-12 w-12 object-contain" draggable={false} />
          ) : (
  ...
  return (
    <div className={cn("relative w-[23rem]", className)}>
      ...
      >
        {badgeImageSrc ? (
          <img src={badgeImageSrc} alt={badgeImageAlt ?? ""} className="h-9 w-9 object-contain" draggable={false} />
        ) : null}
        {badgeLabel ? <span className="sr-only">{badgeLabel}</span> : null}
      </div>
      <Card
          ...
          <CardDescription className="flex flex-col items-start gap-2 py-2 text-sm text-neutral-900">
            ...
            {(eventType || (filters && filters.length > 0)) && (
              <div className="flex items-center gap-2">
                {eventType ? (
                  <Button variant="linkSubdued" className="justify-start" onClick={eventType.onClick}>
                    <Funnel />
                    {eventType.label}
                  </Button>
                ) : null}
                ...
                {meta.map(({ icon, label: metaLabel, value }, index) => {
                  ...
                  return (
                    <div key={`${metaLabel}-${index}`} className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span className="text-sm text-muted-foreground font-medium">{metaLabel}</span>
