...
export const UserFieldRenderer = ({ value, onChange, domainId }: UserFieldRendererProps) => {
  ...
  if (isLoading) {
    return <div className="text-sm text-gray-500 dark:text-zinc-400">Loading users...</div>;
  }
  ...
  if (!users || users.length === 0) {
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-gray-500 dark:text-zinc-400">No users found in this organization.</p>
      </div>
    );
  }
