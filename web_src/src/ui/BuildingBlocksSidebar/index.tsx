...
function OpenBuildingBlocksSidebar({
  ...
  const integrationConnectionStateByName = useMemo(() => {
    ...
    integrations.forEach((integration) => {
      ...
      const nextState =
        integration.status?.state === "ready" ? "ready" : integration.status?.state === "error" ? "error" : "pending";
      const currentState = map.get(normalizedName);
      ...
      if (currentState === "pending" && nextState === "error") {
        return;
      }
      map.set(normalizedName, nextState);
    });
  ...
  const integrationsModalItems = useMemo(
    () => [...availableIntegrations].sort((a, b) => (a.label || a.name || "").localeCompare(b.label || b.name || "")),
    [availableIntegrations],
  );

  const connectedIntegrationItems = useMemo(
    () =>
      integrationsModalItems.filter((integration) =>
  ...
  useEffect(() => {
    const wasOpen = previousIntegrationDialogOpenRef.current;
    if (wasOpen && !integrationDialogOpen && shouldReopenModalAfterIntegrationDialogRef.current) {
...
function IntegrationGridSection({
  ...
  onSelect,
}: IntegrationGridSectionProps): ReactElement {
