...
describe("RunInspectorPanel", () => {
  ...
  it("stops queued items that are fresher than the inspected run cache", async () => {
    mockedExecutions = executions;
    listNodeQueueItemsMock.mockImplementation((options) => {
      if (options.path.nodeId !== "action-2") {
        return Promise.resolve({ data: { items: [], hasNextPage: false } });
      }
