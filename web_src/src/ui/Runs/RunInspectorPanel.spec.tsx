...
describe("RunInspectorPanel", () => {
  ...
  it.each([
    [
      "direct user",
      { index: 0, state: "pending", type: "user", user: { id: "account-1", email: "me@example.com" } },
      {},
    ],
    ["role", { index: 1, state: "pending", type: "role", roleRef: { name: "release_manager" } }, {}],
    ["group", { index: 2, state: "pending", type: "group", groupRef: { name: "release-approvers" } }, {}],
  ])("shows approval actions for actionable pending %s approval records", async (_label, record, overrides) => {
    mockedExecutions = [
      {
    ...
    await waitFor(() => {
      expect(invokeExecutionHookMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          body: {
            parameters: { index: record.index, comment: "" },
          },
        }),
      );
    });
