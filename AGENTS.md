...
- Agent eval scorers (unit tests, no LLM): `make test.agent.evals` (running `agent` container).
- Agent live eval (real model): `make agent.eval.manual_run_two_noop ARGS='--canvas-id <uuid> --model gpt-4o-mini'` with `OPENAI_API_KEY` (or your provider) and Superplane env in `agent/.env`.
- Same eval with **stubbed Superplane API** (no backend): `ARGS='--stub-api --model gpt-4o-mini'` or `EVAL_STUB_SUPERPLANE=1` (catalog is `start` + `noop` only).
- Eval CLI output defaults to **`--format plain`** (line-oriented). Use `ARGS='--format rich'` for the Pydantic Evals table; add `--rich-full` to include the full model output column.
